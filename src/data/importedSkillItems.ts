...
export const importedSkillItems: RadarItem[] = [
  {
    ...
    "methodCore": "Trace2Skill 采用全局并行分析 + 分层归纳整合的范式，通过三个阶段实现高质量技能的自动生成：阶段 1：轨迹批量生成（Trajectory Generation）。使用初始技能（人工编写或 LLM 生成的草稿），让智能体在目标领域任务集上并行执行，生成带标注的成功轨迹（T⁺）和失败轨迹（T⁻，含推理、工具调用、结果），构成技能提炼的原始素材库。阶段 2：并行多智能体补丁提议（Parallel Multi-Agent Patch Proposal）。调度一组专门的分析子智能体，并行处理每条轨迹，独立生成技能修改补丁：成功分析器：从成功轨迹中提炼可复用的有效行为模式，生成优化补丁。错误分析器：以 ReAct 多轮交互方式定位失败根因、验证修复方案，生成避坑补丁。阶段 3：无冲突补丁整合（Conflict-Free Consolidation）。通过分层合并 + 归纳推理，把海量补丁整合成一套无冲突、通用的技能：分层合并：按批次逐层合成补丁，去重、解决冲突、保留核心洞察。归纳推理：优先保留多次出现的通用规则，丢弃单轨迹的个性化特例，确保技能可迁移。格式校验：通过三道确定性护栏保证技能格式合法、无冲突。关键设计。所有子智能体基于同一版初始技能工作，保留轨迹经验的多样性，避免顺序更新带来的\"经验漂移\"。最终输出完整的 SKILL.md + 辅助资源，推理时直接加载，无需任何检索模块。支持两种工作模式：技能深化（优化现有技能）和从零创建（从 LLM 草稿开始迭代生成）。在能力层面，从成功轨迹中提炼可复用的有效行为模式。分层合并 + 归纳推理整合海量补丁为无冲突的通用技能。框架具双重能力：既能深化已有人工撰写技能，也能从较弱的 LLM 生成草稿创建有用技能。其核心机制是把反复出现的失败与规避手法压缩为标准操作流程（SoPs），使技能不再是训练轨迹的记忆性副本，而是可跨模型规模、跨模型家族、以及迁移到分布外设定的可移植资产。abstract 强调进化后技能无需参数更新、也无需测试时检索即可复用，并与顺序式技能编辑和 ReasoningBank 式检索记忆形成对比。整体把执行经验从零散轨迹提炼为组织化、可携带的过程性知识库，覆盖办公流程、数学推理、视觉问答等多样领域。在边界层面，需要足够多的执行轨迹才能提炼出高质量技能。跨领域迁移效果与领域结构相关性高。abstract 明确在办公流程、数学推理、视觉 QA 等多领域验证有效，并报告从 Qwen3.5-35B 轨迹进化出的技能可使 Qwen3.5-122B 智能体在 WikiTableQuestions 上最多提升 57.65 个百分点。进一步分析显示其优于顺序式技能编辑与 ReasoningBank 式检索记忆，技能可跨模型规模与家族迁移。除该项外的绝对数值需查原文。",
  ...
  },
  {
    ...
    "methodCore": "D2Skill 的核心是双粒度技能建模与策略-技能库协同进化的联合训练范式，整体框架由三大核心模块构成：带技能注入的 RL 联合训练。在每一轮训练中，对每个任务同时采样两组平行轨迹——技能组轨迹的每一步检索并注入对应技能，基线组使用相同策略但不注入技能。基于两组性能差异，同时完成三个环节的计算：事后技能效用更新：技能组与基线组的成功率差值作为任务级技能效用信号；事后内在奖励塑形：技能注入轨迹表现优于基线组时获得额外奖励；策略优化：将内在奖励融入总回报，基于 GRPO 算法完成策略更新。反思驱动的技能生成。当任务组表现低于阈值时自动触发反思机制，分析技能组的失败轨迹和成功轨迹，从中提炼出新的任务技能和步骤技能，经去重后加入技能库。技能检索与库管理。设计两阶段检索流程：第一阶段：根据语义相似度检索 top-m 候选技能；第二阶段：结合归一化语义相似度、技能效用和 UCB 探索项的综合分数排序，选出 top-k 技能注入策略上下文。同时通过效用导向的定期剪枝防止技能库无限膨胀，新生成的技能享有保护期以确保充分评估。在能力层面，双粒度技能抽象：任务技能提供高层战略指导，步骤技能提供细粒度战术纠错。动态效用评估与技能库剪枝维护。D2Skill 针对现有技能方法多停留在轨迹级指导、缺乏演化式技能记忆维护机制的问题，提出面向 agentic RL 的动态双粒度技能库。其把可复用经验组织为两类粒度：任务技能（task skills）提供高层指导，步骤技能（step skills）提供细粒度决策支持与纠错。框架采用配对回合（paired rollouts）——在同一策略下分别执行基线回合与注入技能回合，用两者的性能差（performance gap）导出事后效用信号（hindsight utility signals），同时驱动技能更新与策略优化，实现策略与技能库联合训练。技能库完全从训练期经验构建，通过反思持续扩充，并以效用感知的检索与剪枝（utility-aware retrieval and pruning）维护。消融表明双粒度建模与动态维护对增益均属关键，学得技能效用更高、可跨评测设定迁移，且仅引入适度训练开销。D2Skill 是面向智能体强化学习的动态双粒度技能库（任务技能 + 步骤技能），通过配对轨迹性能差距构建效用信号驱动策略与技能库协同进化，辅以反思驱动生成、效用感知检索与剪枝，持续维护高可用动态技能库。在边界层面，技能库管理机制仍有优化空间。跨领域迁移能力有限。",
  ...
  },
  {
    ...
    "methodCore": "SkillX 的核心是将经验拆解为三层互补的技能体系，无需人工参与，通过三步闭环流程全自动构建并迭代技能库：三层技能架构：规划技能（Planning Skills）：高层任务组织模块，将完整轨迹压缩为有序的核心执行步骤与依赖关系，过滤试错、回溯等无效操作，给出\"做什么\"的宏观方案。功能技能（Functional Skills）：工具化子任务模块，封装可复用的工具组合宏操作，每个技能包含名称、输入输出说明、工具调用模式，解决\"怎么完成子任务\"。原子技能（Atomic Skills）：单工具执行规范层，补充工具使用约束、参数配置、失败场景等信息，修正工具调用幻觉。三步闭环构建流程：技能提取：骨干智能体对训练任务多轮滚动执行，仅保留成功轨迹；对轨迹\"降噪压缩\"后按三层架构自动提取对应技能。技能迭代优化：通过合并（语义相似度聚类、聚合优势、拆解复杂技能）与过滤（通用过滤 + 质量筛选）双操作迭代升级技能库，以\"新增/修改/保留\"三种操作完成更新，多轮执行直到性能不再提升。主动扩展：采用经验引导的探索策略——优先探索低利用率、高失败率的工具，从探索轨迹中合成新任务样本，自动发现并补充缺失技能，扩充覆盖场景。即插即用赋能。先检索规划技能并重写为伪计划，再以伪计划每步为查询检索功能与原子技能，去重后一次性注入系统提示词，无需多轮交互。在能力层面，三层技能自动提取：从成功轨迹中自动提取规划、功能、原子三层技能。技能合并与过滤迭代优化：语义聚类合并 + 两轮质量筛选，技能库越用越好。即插即用跨模型赋能：弱模型接入后任务成功率平均提升约 10%。SkillX 针对自进化范式中智能体孤立学习、重复重新发现相似行为、导致冗余探索与泛化差的低效问题，提出全自动构建即插即用（plug-and-play）技能知识库的框架，可跨智能体与环境复用。其流水线建立在三项协同创新上：（i）多层次技能设计（Multi-Level Skills Design），将原始轨迹蒸馏为策略性计划、功能性技能、原子技能三层层级；（ii）迭代式技能精炼（Iterative Skills Refinement），基于执行反馈自动修订技能以持续提升库质量；（iii）探索式技能扩展（Exploratory Skills Expansion），主动生成并验证新技能，将覆盖范围扩展到种子训练数据之外。作者以强骨干智能体 GLM-4.6 自动构建可复用技能库，并在挑战性的长程、用户交互式基准上评估其可迁移性。abstract 强调结构化、层级化的经验表示对可泛化的智能体学习至关重要，代码将开源。",
  ...
  },
  {
    ...
    "methodCore": "Memento-Skills 基于 Stateful Reflective Decision Process（有状态反思决策过程） 理论，核心运行逻辑是 Read-Write Reflective Learning（读写反思学习） 闭环：读取（Read）：面对新任务时，检索当前最相关的技能来指导冻结的大语言模型执行，Agent 不再依赖静态提示词或原始历史轨迹，而是靠一个不断主动优化的技能库来行动。写入（Write）：根据执行结果直接修改可复用的技能工件，而非被动记录日志：执行成功 → 提升对应技能的效用评分，增加后续被选中概率。执行失败 → 先进行失败归因，定位到具体技能的哪个环节出了问题，然后针对性优化（添加边界条件、修改执行逻辑、调整 Prompt 模板）。现有技能无法覆盖 → 自动生成全新技能，经过测试后加入技能库。技能路由器：优化目标从传统检索的\"语义匹配\"改为\"任务成功\"，采用多正例 InfoNCE 损失做硬负例对比学习，精准区分语义相似但执行逻辑不同的技能。混合检索架构结合稀疏 BM25 召回、稠密向量检索和分数感知的倒数秩融合，可选交叉编码器重排。自动单元测试关卡：每次技能更新后，系统自动生成相关测试用例，验证修改后的技能在历史任务上的表现，确保技能不会越改越差。在能力层面，读写反思学习闭环：从经验中学习，犯错后总结教训，提炼成可复用技能。失败归因与针对性技能优化：精准定位失败根因并修改对应技能工件。自动生成全新技能并通过单元测试验证，确保不会越改越差。系统建立在带状态提示（stateful prompts）的记忆式强化学习框架上，可复用技能以结构化 markdown 文件存储，充当持久且不断演化的记忆，同时编码行为与上下文，使知识跨交互延续。系统从简单基础技能（如网页搜索、终端操作）起步，借助 Memento 2 的读-写反思学习机制持续改进：读阶段由行为可训练的技能路由器依当前状态提示选取最相关技能；写阶段基于新经验更新与扩展技能库。该闭环实现无需更新 LLM 参数的持续学习。与依赖人工设计智能体的既往方法不同，Memento-Skills 支持通用智能体端到端地为新任务设计智能体，并通过迭代式技能生成与精炼逐步提升自身能力。在边界层面，跨领域迁移能力有限。技能库收敛需要时间。abstract 明确在 General AI Assistants 基准与 Humanity's Last Exam 上验证获得持续增益，整体准确率分别取得 26.2% 与 116.2% 的相对提升（relative improvements）。代码已开源（Memento-Teams/Memento-Skills）。除上述两项相对提升外的绝对数值需查原文。",
  ...
  },
  {
    ...
    "methodCore": "SKILLRL 由三个核心组件构成：基于经验的技能蒸馏（Experience-based Skill Distillation）。不同于先前方法仅保留成功轨迹，SKILLRL 刻意保留成功和失败两类轨迹，并进行差异化处理：成功轨迹：由教师模型识别关键决策点、正确行动背后的推理逻辑以及可泛化的行为模式，提取为策略性技能。失败轨迹：由教师模型分析失败点、错误推理/行动、应做之事以及预防类似失败的通用原则，合成为简洁的失败教训。层次化技能库（SKILLBANK）。将蒸馏出的知识组织为两层结构，推理时，通用技能始终包含在上下文中，任务特定技能通过语义相似度动态检索：通用技能：跨任务类型的通用策略原则，如系统化探索（优先未访问位置）、状态管理（执行前验证前置条件）、目标跟踪启发式等，适用于所有任务。任务特定技能：针对特定任务类别的领域知识，包含领域特定行动序列、任务特定前置条件与约束、该任务类型特有的常见失败模式等。递归技能进化（Recursive Skill Evolution）。技能库并非静态 —— 通过冷启动监督微调和 RL 期间的动态进化实现协同进化：冷启动监督微调：基础 Agent 未学会如何有效利用技能，通过教师模型生成技能增强推理轨迹，演示如何检索、理解和应用技能，对基础模型进行初始化。动态技能进化：每个验证轮次后监控各任务类别的成功率，对低于阈值的类别触发进化。采用多样性感知的分层采样策略收集失败轨迹，由教师模型识别当前技能未覆盖的失败模式，提出新技能或优化现有技能，更新技能库。在能力层面，基于经验的差异化技能蒸馏：从成功轨迹提取策略模式，从失败轨迹合成教训，10-20× Token 压缩。层次化技能库与自适应检索：通用技能始终生效，任务特定技能按语义相似度动态检索。递归技能进化：技能库随 RL 训练动态扩展，与 Agent 策略协同进化。SkillRL 针对现有记忆式方法主要存储冗余且噪声重的原始轨迹、难以提取高层可复用行为模式的问题，提出一个在原始经验与策略改进之间架桥的框架，核心是自动技能发现（automatic skill discovery）与递归进化（recursive evolution）。它引入基于经验的蒸馏机制（experience-based distillation）构建分层技能库 SkillBank；配以自适应检索策略（adaptive retrieval），区分通用启发式与任务特定启发式；并设计递归进化机制，使技能库在强化学习过程中与智能体策略协同演化（co-evolve）。这些创新在显著降低 token 占用（token footprint）的同时增强了推理效用。整体思路是把杂乱轨迹提炼为结构化、可复用的行为模式，让技能库随策略更新而自我扩充，从而缓解智能体孤立运行、无法从过往经验学习的局限，并在任务复杂度上升时保持鲁棒。",
  ...
  },
  {
    ...
    "methodCore": "Skill-IR 四层中间表示：从\"发生了什么\"到\"应该怎么做\"。摘要追求语义显著性（挑重要内容、压掉细节），而技能需要的恰好是操作细节。W2S 定义 Skill-IR 将技能拆为四个正交组件：路由头：什么条件下调用。工作流骨架：节点间的有向跳转关系与宏观流程分支。节点级语义：局部目标、执行条件、成功/失败判定标准。运行时附件：工具、脚本、参考文档、模板、配置约束和状态管理规范。四层拆分使技能生成从\"把轨迹写成一段话\"变为\"从轨迹中为每一层填充该层需要的信息\"。五步重构管线：分段 → 起草 → 对齐与合并 → 分支协调 → 冗余压缩。分段将每条轨迹按操作类型切分成过程单元。起草对每条路径单独诱导本地 Skill-IR 草案。对齐与合并在多条草案间做骨架层面对齐，识别共章节点、合并同构步骤、将路径差异收敛到同一骨架的不同分支。分支协调根据轨迹一致性和出现频率判断分支是通用规则还是偶然偏差，保留的分支标注证据来源和置信度。冗余压缩在保留关键验证/审批/回滚行为的前提下合并表述重复。在能力层面，将技能生成从文本摘要范式转换为工作流规范重构，解决摘要目标函数与技能优化方向错位。Skill-IR 四层正交表示让技能的不同组件（路由/骨架/语义/附件）各自独立可编辑。多轨迹对齐与合并：自动识别共章节点，将不同路径差异收敛到统一工作流骨架的不同分支上。分支协调按轨迹一致性+频率过滤偶然偏差，保留分支标注证据来源和置信度。行为重放一致性比 Anthropic Skill Creator 高 10.5%，在分支逻辑和恢复步骤方面漏项显著更少。其公认工程语义含几项要点：一是渐进式披露，智能体先看到技能名称与简短描述，仅当任务相关时才加载完整正文与附带文件，从而在有限上下文中挂载大量技能而不臃肿；二是人机协作的创建与迭代闭环，由人给出任务规格与示例，工具辅助起草描述、正文与触发条件，再经真实使用反馈持续修订；三是标准化的可移植封装格式，使技能可跨智能体宿主安装、检视、版本化与纠正。它强调技能是显式、可审计、可复用的产物。W2S 将技能自动生成从文本摘要重新定义为工作流规范重构：不是把多条轨迹压缩成一段文章，而是将对齐到一张有节点、有分支、有语义、有操作上下文的可执行图。W2S 定义了技​​能中间表示 Skill-IR（路由头+工作流骨架+节点级语义+运行时附件），行为重放一致性比 Anthropic Skill Creator 高出 10.5%。",
    ...
    "limitations": "技能质量与触发准确性高度依赖描述撰写与人工反馈；缺乏统一自动化质控与量化评测，大规模技能库的命名冲突与维护成本随规模上升。场景覆盖的完整性是未观测变量：技能质量受限于输入轨迹是否覆盖足够多执行场景，关键回退路径若从未出现则不会被补上。Skill-IR 四层结构预定义且固定：极简单步技能中两层几乎为空，复杂多 Agent 协作场景中四层可能不够，抽象粒度不可参数化。轨迹分段的粒度选择是启发式的：不同任务的\"一个自然步骤\"跨度差异巨大，同一套分段启发式可能切出尺度极不相同的步骤。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "SkillSynth 通过三个阶段将技能图谱转化为多样化训练数据：技能图谱构建（Skill Graph Construction）。从 ClawHub 和 GitHub 收集并过滤技能，由 LLM 推断每个技能的前置场景（何时适用）和后置场景（执行后状态），经层次凝聚聚类去重、跨技能双向语义对齐、场景合并与过滤，构建以场景为节点、技能为有向边的统一图。最终图谱包含 82,073 个场景节点、57,214 条技能边、185,529 个 LLM 验证的桥接，85.6% 节点位于最大连通分量中，涵盖编码、文档处理、DevOps、安全及音频语音、3D 仿真等长尾领域。逆频率路径采样（Inverse-Frequency Path Sampling）。从图中采样有向路径作为任务工作流抽象。采用逆频率加权策略——场景和技能被访问越少、被选中概率越高——避免路径集中在热门节点；路径内排除已访问节点以保证单调推进，长度 1~7 步，仅接受技能集未曾出现过的路径。该机制将采样分布逐步推向场景与技能空间的均匀覆盖。多 Agent 协作验证管线（Multi-Agent Harness）。规划器将路径转化为子目标序列，构造器据此生成完整任务实例（指令、文件系统快照、Docker 环境、pytest 验证脚本、参考解法）。通过执行验证（确认可解）和评分验证（LLM 检查指令-测试对齐性）双重把关，失败则反馈重试（最多 3 轮），最终产出 3,560 个验证通过的任务实例，oracle 通过率 95.7%。在能力层面，图谱驱动的多样性控制：通过逆频率路径采样在\"场景 × 技能\"空间上实现均匀覆盖，合成轨迹的唯一场景-技能对比单技能基线高 31%，比随机多技能高 19%。全自动任务实例生成：单次运行产出 3,560 个验证通过的可执行任务，95.7% oracle 通过率，包含指令、环境、验证脚本和参考解法的完整任务包。小模型超越大模型的数据效率：Qwen3-32B + SkillSynth 在 Terminal-Bench 2.0 上（29.6%）超越 Qwen3 Coder 480B（23.9%），证明多样性优先于数量的数据策略。该工作面向终端类任务智能体的训练数据瓶颈，指出大规模构建高质量终端轨迹面临两大要求：可执行性（Executability，每个实例需一个合适且往往各异的 Docker 环境）与可验证性（Verifiability，异构任务输出难以统一标准化校验）。为此提出可扩展流水线 TerminalTraj，含三步：（i）筛选高质量代码仓库以构建容器化执行环境（Dockerized execution environments）；生成与 Docker 对齐的任务实例；合成带可执行校验代码的智能体轨迹，实现自动验证。基于该流水线，作者整理出 32K 个 Docker 镜像并生成 50,733 条经验证的终端轨迹，覆盖八个领域。以 Qwen2.5-Coder 为骨干在此数据上训练的模型在 TerminalBench 上持续提升，并展现更好的测试时扩展行为。整体用可执行、可验证的环境化流水线，把真实长程终端交互沉淀为可训练的高质量轨迹数据。",
  ...
  },
  {
    ...
    "methodCore": "AdaReasoner 将工具增强的多模态推理形式化为序列决策过程，基于三项核心创新构建：高质量轨迹数据管线（TC）：三阶段生成训练轨迹——首先为每类任务设计抽象最优求解蓝图（VSP 按感知-规划-验证、Jigsaw 按迭代试错），刻意融入反思回溯和显式工具失败两种复杂场景；然后程序化执行工具调用填充输入输出；最后用强 LLM 生成连接每步的 Chain-of-Thought 推理文本，教会模型主动验证假设和从失败中恢复。Tool-GRPO 强化学习（TG）：扩展 GRPO 处理多轮工具调用轨迹。奖励由格式、工具调用质量和最终答案精度三部分加权组合，格式奖励单步错误即全轨迹归零，工具奖励按结构、名称、参数名、参数内容四维度层级评分（0-4 分）。采用不对称奖励设计——预测正确时无论是否使用工具均给满分，预测错误时按工具使用质量给部分分，使工具成为不确定性下的后备机制而非强制步骤。自适应学习机制（ADL）：贯穿 TC 和 TG 两阶段，解耦工具使用逻辑与任务绑定。Token 级随机化将工具名和参数名替换为随机字符串，迫使模型从描述和上下文推断功能；语义级重述改写工具和参数描述，保持语义但改变措辞，防止过拟合。在能力层面，自主规划多步工具调用链：动态选择感知（POINT、OCR）、操作（DRAW2DPATH、CROP）、计算（ASTAR）三类视觉工具。自适应工具调节：自主学会采纳有益工具、抑制无关工具、根据任务需求动态调节使用频率，无需显式训练这些行为。跨工具跨任务泛化：在未见过的工具定义和全新任务分布上，保持高频率和高准确率的工具使用。它由三部分使能：（i）可扩展的数据整理流水线（scalable data curation pipeline），让模型接触长程、多步的工具交互；（ii）Tool-GRPO，一种基于终任务成功（end-task success）优化工具选择与排序的强化学习算法；（iii）自适应学习机制，动态调节工具使用。三者协同使模型能从任务上下文与中间结果推断工具效用，从而协调多个工具并泛化到未见工具。实验显示 AdaReasoner 展现强工具自适应与泛化行为：自主采用有益工具、抑制无关工具、按任务需求调整工具使用频率——尽管从未被显式训练如此行事。整体把工具编排内化为模型的通用推理能力。在边界层面，当前仅验证了视觉推理领域，文本工具使用能力未评估。依赖预定义工具集，工具集扩展需要重新训练。Tool-GRPO 训练计算开销较大，需要多轮交互采样。",
  ...
  },
  {
    ...
    "methodCore": "把技能定义为一种\"干预\"——加载技能等于对 Agent 做了一次处理，效果用配对实验衡量。三阶段流水线：采集基线 → 对比归纳 → 生成-验证-精炼循环。Stage 1：基线采集。让基础 Agent 在训练集上裸跑一轮，成功和失败的轨迹都保留。只看成功不知道缺口在哪，只看失败拿不到可复用的正确做法。Stage 2：对比归纳。归纳 Agent 生成结构化诊断报告，分四块——任务摘要（这类任务在干什么）、失败模式聚类（根因 + 典型出错点 + 正确做法）、成功模式聚类（可复用操作流程 + 检查点）、逐案例对比（对每个失败实例找到嵌入空间最近的同类成功实例，逐帧对比\"成功那次做了什么、失败那次没做\"）。第四块是 SKILLGEN 最特别的地方——它锚定的是 Agent 自己已经展示过但某些场景下遗漏了的能力。Stage 3：生成-验证-精炼循环：生成 Agent 写出候选技能（任务上下文 + 成功模式 + 失败规避）；验证 Agent 在验证集上跑配对评估——同一个输入分别用有技能和无技能的 Agent 执行，拉列联表算净收益 Gm = 修复数 − 回归数；精炼 Agent 据此提四类反馈（keep/remove/add/emphasize），生成 Agent 修改后进下一轮。最终不取最后一轮而是取 K 轮候选中 Gm 最大的——论文数据显示最新候选期望 Δ 是 -3.1pp，最佳候选是 +8.1pp，差了约 11 个百分点。验证门。哪怕最佳候选，Gm 低于阈值就标记 deprecated，不部署。宁可什么都不做，也不装未经验证的技能。在能力层面，对比归纳不只从成功中提取经验，更从失败和成功的\"差集\"中定位能力缺口的精确位置。配对评估用列联表量化每个候选技能的真实净收益——不是修好几个 case 就认为有用，而是同时看它搞砸了几个原本能做对的 case。best-of-K 选择把精炼当搜索而非收敛，不假设最后一轮一定最好，效果更诚实。验证门是硬约束——τ-Bench 上多个模型生成的候选技能直接被挡掉，避免了对基线的负向干预。SkillGen 针对高质量技能仍多靠手写、且无需重训即可提升智能体能力的诉求，提出一个多智能体框架，从基础智能体生成的轨迹中合成单一可审计（auditable）、人类可读的技能产物，可在使用前先行检视。其关键并非简单总结轨迹，而是对成功与失败轨迹同时进行对比归纳（contrastive induction），以识别可复用的成功模式、反复出现的失败模式，以及那些出现在临近成功而缺席于失败中的行为；随后生成候选技能并迭代精炼。SkillGen 的核心创新在于把智能体技能建模为干预（interventions）以实证验证其对整体性能的净效果（net effect）：在相同实例上比较有技能与无技能的结果，从而同时核算修复（repairs，技能修正基线失败）与回退（regressions，技能破坏基线成功）两类影响，避免只看片面收益。整体流程强调可审计、可对比、可验证的技能生成路径。",
  ...
  },
  {
    ...
    "methodCore": "SkillEvolver 将自身定义为一项通用元技能，是标准化指导 AI 创作、修订、审核各类领域技能的程序性体系，轻量化、可插拔、无需模型微调，适配各类 CLI 智能体：策略多样化探索（Strategy Diversification）：每轮迭代中，作者为执行者生成 4 条关键决策轴上必须有实质性差异的策略，每条策略由独立执行者运行，记录成功或失败轨迹。对比式更新（Contrastive Update）：将成功轨迹与失败轨迹进行对比，提取差异作为技能需要补充或修改的内容，生成补丁式修订，仅修改被证据指向的内容而非整体重写。独立审计（Independent Audit）：启动全新 AI 会话作为审计官，仅基于技能本身判断，共 9 条审计规则覆盖三类问题——过拟合检测（是否包含训练数据具体文件名或数值）、可执行性检测（是否声明主脚本但执行者从未调用，即\"静默绕过\"）、抽象程度检测（指令是否过于模糊或包含不可追溯断言）。关键设计。角色分离：编写技能的 AI（作者）与执行技能的 AI（执行者）不同，利用信息不对称自然暴露技能缺陷。学习目标是技能文本与代码，而非模型权重，产出可直接插入任何 Agent 无需重训练。元技能本身也是一项技能，通过相同接口加载，任何协议兼容的 CLI-Agent 均可使用。在能力层面，超越人类专家：在 SkillsBench 83 项任务（覆盖 15+ 领域）上达到 56.8%，超越人类编写技能的 43.6% 和无技能基线的 29.9%。GPU 内核优化：在 KernelBench 三个 GPU 内核优化任务上，平均加速比从 1.16 提升至 1.51。低成本高效：单任务耗费 3.92 美元（相较单轮迭代仅上浮 8%），使下游 Agent token 消耗降低 19.4%，交互耗时缩减 23.8%。其学习目标是技能的自然语言正文与代码（prose and code），而非模型权重，因而产物可无需重训直接落入任何智能体；且元技能本身也只是一个技能，通过同一接口被任何协议兼容的 CLI-agent 加载。与轨迹蒸馏（trace-distillation）不同，元技能仅在部署学得技能之后才精炼——学习信号来自另一个智能体在使用该技能时遭遇的失败，而非仅靠探索性轨迹。精炼迭代由一个新鲜智能体过拟合审计（fresh-agent overfit audit）管控，用以捕捉可能的泄漏，以及部署技能特有的失败，包括“静默旁路”模式——技能内容看似有效却在运行时从未被调用。整体形成部署驱动、审计约束的在线技能进化闭环。",
  ...
  },
  {
    ...
    "methodCore": "MUSE-Autoskill 将 Skill 生命周期定义为五个耦合阶段，在统一 Agent 循环中运转。五阶段生命周期。Skill 创建（Creation）：内置 skill_create 在 Agent 的 ReAct 循环内发起创建，消除创建与执行的脱节。Skill 记忆（Memory）：构建三级记忆体系。短期记忆保存当前任务上下文并随增长自适应压缩，长期记忆跨会话持久化通用经验教训，Skill 级别记忆则为每项 Skill 维护独立的 .memory.md 文件，累积失败模式、输入陷阱和性能注意事项。Skill 管理（Management）：采用渐进式检索，启动时只注入 Skill 目录，Agent 决定使用后才加载完整内容。同时，辅以合并重叠 Skill 和裁剪失败或闲置 Skill 两层维护机制。Skill 评估（Evaluation）：单元测试充当质量守门人，创建时把关注册，复用中失败则自动触发精炼。Skill 精炼（Refinement）：基于错误反馈修订或重新生成 Skill，与评估耦合形成自动纠错闭环。关键设计。Skill 级别记忆：采用 \"Skill 与经验分离\" 的设计：每个 Skill 对应一个 .memory.md 文件，用于存储 Agent 在使用过程中沉淀的所有经验教训。该文件被刻意隐藏且不随 Skill 打包导出，从机制上保证了 Skill 是可共享的公共资产，经验则是每个 Agent 独有的私有资产，彻底避免了经验的交叉污染。自适应上下文压缩：对话建模为 DAG 节点图，每节点为 plan-action-observation 三元组。Level-1 将超标节点压缩为摘要并保留链位置，Level-2 将中间连续节点合并为合成节点。在能力层面，版本化自进化：自生成 Skill 中位数 326 行（15.8KB），为人类 Skill 的 2.2 倍，包含详细的输入输出模式、失败模式和逐步操作流程。跨 Agent Skill 迁移：MUSE 生成的 Skill 注入 Hermes（完全不同设计的 Agent）后提升 +10.51pp，弥合 79% 与人类 Skill 的差距，验证 Skill 是真正外部化知识资产。跨会话能力积累：Skill 级别 .memory.md 机制使经验在每次使用时自动激活，无需重复推导。多域覆盖：SkillsBench 覆盖科学计算、数据分析、文档处理、运维规划四个超级领域。MUSE 按需创建技能，跨任务存储，通过技能目录（skill catalog）检索，并为每个技能累积经验（per-skill experience）以供后续复用与适配。其核心主张是把技能视为长生命周期、经验感知且可测试的资产，而非一次性提示。评估阶段检验技能有效性，管理阶段维护技能目录，记忆阶段沉淀 skill 级经验，从而让生命周期治理的技能把智能体经验蒸馏为高效可复用资产，并支持向其他智能体宿主迁移。",
  ...
  },
  {
    ...
    "methodCore": "三重身份 rollout：零额外开销的技能生成。利用 GRPO 每条 rollout 独立采样的特性，RESKILL 让同一条 rollout 同时完成三件事：奖励信号按标准 GRPO 流程更新策略参数（策略优化）；完整轨迹存入 经验水库，LIFO 更新确保水库始终反映 Agent 最近行为分布（失败诊断）；rollout 所属技能版本的奖励更新 Thompson Sampling 后验（技能评测）。rollout 总数与标准 GRPO 完全一致，不额外消耗采样预算。经验水库驱动的技能生成管道。水库存储的轨迹驱动四阶段技能创建管道。断言评分：规则化断言扫过水库所有轨迹，低通过率直接标注高发失败模式，断言由 LLM 诊断器动态更新——删掉不再触发的旧断言，添加针对新失败模式的新断言。对比分析：水库按任务分组，LLM 逐组对比成功与失败案例，区分技能是被正确遵循还是被无视。推荐 → 撰写 → 验证：推荐器提出 ADD/MODIFY/DELETE 操作，撰写器生成包含触发条件（general/beginning/action_pattern）的结构化技能，最后新技能必须在水库上触发率超 50% 否则打回重写（最多 3 次）。Thompson Sampling + 自适应折扣：技能版本公平竞争。新旧技能版本作为两臂 Bandit 问题处理。每个训练步 Thompson Sampling 从 Beta 后验中采样决定 rollout 分配比例，好的版本自然拿到更多 rollout。由于策略在持续进化，旧 rollout 数据随策略漂移而\"变质\"，RESKILL 引入 自适应折扣：合并新观察前伪计数先收缩一个与当前步 rollout 数量相关的因子，记忆参数 M 从已完成的 A/B 测试数据中通过预测似然准则自动估计，无需手动调节。周期结束时新版本后验均值超过旧版本则接受，否则回滚。在能力层面，训练内技能生成：技能在 RL 训练中随策略同步进化，策略学到的新能力即时沉淀为可复用技能，过时技能自动淘汰。零额外采样开销：一条 rollout 同时完成策略优化、失败诊断、技能评测，总计算量与标准 GRPO 持平。自适应版本竞争：Thompson Sampling 动态分配 rollout 给新旧技能版本，自适应折扣让评分随策略进化保持有效。全自动技能生命周期管理：ADD/MODIFY/DELETE 操作全程自动，训练早期补基础、中期做精做细，无人干预。跨域技能迁移：冻结策略后技能库可在新领域独立进化，Agent 学会了\"怎么从技能中受益\"的元能力。ReSkill 受 Anthropic Skill Creator 启发，针对既有技能增强 RL 把技能创建与策略优化解耦、易采纳与演化中策略相冲突技能的问题，提出“RL-in-the-loop”的技能创建框架，调和技能演化与策略学习。它利用 GRPO 的组内结构，以极小额外开销嵌入三种机制：（1）断言驱动的技能创建器，从过往经验诊断失败并提出条件式、触发式修订；（2）组内回合采样，受控比较不同技能版本，捕捉哪一版本最能支撑策略当前的学习；（3）带自适应折扣的 Thompson 采样，在策略演化中于技能版本选择上平衡探索与利用。对技能生命周期的分析显示，技能随策略改进被自动创建、测试、精炼与剪枝，在近乎零额外采样开销下实现技能自生成、自评测与自进化。",
  ...
  },
  {
    ...
    "methodCore": "原型表示提取：剥离叙事的数学骨架。对每个训练问题，LLM 提取\"优化成分\"——变量角色、约束机制、目标函数方向，同时生成一个\"脱敏版\"问题描述（将具体实体替换为抽象术语），数字和数学关系保留不变。两种嵌入加权融合得到原型嵌入。聚类蒸馏建库：从案例到原型技能的质变。OPTSKILLS 不做逐样本技能生成（129 个技能中 62.8% 闲置），而是对原型嵌入做 DBSCAN 聚类（余弦距离，$\\epsilon=0.05$ 偏保守），同一簇内合并轨迹的正负分析结果蒸馏为一份技能文档（含元数据、工作流、常见陷阱），最终仅 46 个技能覆盖 11 个优化家族，闲置率仅 15.2%。增量学习：精炼 + 扩展双模式。新问题匹配到已有技能时走精炼（成功经验灌入工作流、失败模式灌入陷阱），匹配不到时走扩展——只有至少一条正向轨迹存在才构建新技能入库。这个\"无正轨迹不入库\"的保守门槛保证可靠性。在能力层面，原型级抽象：按数学结构而非表面文本组织和复用经验，从根源上解决语言表述变化导致的泛化失败。聚类蒸馏防冗余：DBSCAN 聚类合并同原型的多元轨迹分析结果，闲置技能率从 62.8% 降至 15.2%。强 OOD 泛化：在 NLCO 基准上学习后达到 72.79%，比纯基线高 10.19 个百分点，新技能利用率达 41.2%。工业级规模适用：MIPLIB-NL 上求解了决策变量维度达 87,482 的问题，技能引导策略未被规模压垮。跨基座迁移：DeepSeek-V3.2 和 Qwen3-235B 两种基座上均有效，增益来自系统设计而非堆模型。OptSkills 针对 LLM 自动建模与求解优化问题时泛化受限（对表层叙述变化敏感、经验多停留在案例级、难适配漂移或新兴题型）的问题，提出以原型为中心（archetype-centric）的技能学习与推理智能体系统。为提升鲁棒泛化，系统按问题的底层原型（archetypes）而非表层叙述聚类问题；为提升分布内泛化，在每个簇内探索多样的建模范式与求解器配置，再将成功轨迹蒸馏为可复用的工作流级技能；为提升分布外泛化，用新获轨迹精炼已有技能或扩展技能库。整体把优化经验从零散案例上升为原型级、工作流级的可复用技能，使系统对叙述变体不敏感并能适配未见题型。（说明：输入概述提及 DBSCAN 聚类与并行探索，但 abstract 正文仅明确“按底层原型聚类”与“簇内探索多样范式”，故此处严格依据 abstract 表述，不写入具体聚类算法名。",
  ...
  },
  {
    ...
    "methodCore": "归纳-演绎双智能体闭环。归纳智能体的系统 Prompt 是整个框架唯一被优化的变量，输入任务描述 + 成功 ReAct 轨迹，输出结构化技能文档。演绎智能体的 Prompt 从头到尾冻结，唯一的战略输入是归纳智能体生成的技能 + 任务描述（无原轨迹信息），在真实环境中执行 ReAct 循环产出重建轨迹。冻结演绎方确保重建质量变化只能归因到技能本身的改进，形成纯净的控制实验。三层抽象分类法约束提炼粒度。归纳智能体 Prompt 内置三层分类：程序惯例：跨任务通用但不易从指令推断的，如\"循环翻页直到返回为空\"，保留;指令可推断：从任务描述就能想到的，删掉;标准答案泄露：只有看过原轨迹才知道的，如具体参数值，删掉）。归纳智能体仅保留第一类，精确控制抽象程度。三重文本损失联合优化。全部损失由 LLM Judge 或环境执行计算，走 TextGrad 文本优化。重建损失：比策略级等价性而非步骤级文字相似。结果损失：直接跑环境看任务过没过，是框架唯一的硬地面信号。量规损失：最巧妙，从标准答案独立性、可操作性、可迁移性、完整性、简洁性五个维度评估技能文档，充当抽象级别的正则化器。三者词典序最小化：结果损失第一优先，重建损失第二，量规损失第三。在能力层面，闭环质量保证：生成 → 验证 → 优化 → 再生，技能质量可测量、可优化，不再是\"生成了就不管了\"。自训练对齐：弱模型（Qwen3.5-122B）自训技能均分 59.1，强模型（GPT-5.4）代训仅 58.9——生成器与推理模型同分布比生成器原始能力更重要。紧凑技能产出：推理时仅检索 K=3 个技能，注入 Token 数为 ACE 的 1/3 到 1/6。策略级泛化：AppWorld-Challenge SGC 场景级完成率 39.6%，显著优于 ACE（34.5%），捕捉的是场景级程序模式而非任务特定捷径。MIND-Skill（Multi-agent INduction and Deduction for Skills）针对复杂多步真实任务需要领域过程性知识、而技能整理长期依赖人工的问题，提出从成功轨迹自动归纳可泛化技能并带鲁棒质量保证的框架。它含两个智能体：归纳智能体（induction agent）从成功轨迹抽象出可复用技能；演绎智能体（deduction agent）尝试遵循归纳出的技能重建轨迹。为保证生成技能质量，作者引入三重文本损失：重建损失（reconstruction loss）比较输入轨迹与重建轨迹；结果损失（outcome loss）强制重建轨迹的正确性；评分标准损失依预定义准则评估文档质量并规整技能的抽象层级。这些文本损失以 TextGrad 联合优化，生成技能在优化期未见的留出任务上评估。整体将技能质量从主观判断转化为可测量、可优化的目标，形成归纳-演绎闭环的质量保证式技能生成。",
  ...
  },
  {
    ...
    "methodCore": "Create 操作：从原始轨迹到可复用技能的初次提取。Agent 在无技能指导下纯靠自身能力跑完任务轨迹作为输入。LLM 审视轨迹，从中抽取可复用的过程性知识：先做什么、再做什么、遇到什么情况如何处理。关键约束是区分\"这条轨迹特有的偶然操作\"和\"这类任务通用的有效策略\"，只抽出后者。每个技能包含三个不可变组件：名称（一经确定不可修改）、描述（触发条件和功能概要）、正文（markdown 格式的操作指令）。Merge 操作：通过多视图语义识别驱动泛化。技能库膨胀后，Merge 在名称、描述、正文三个维度上分别计算语义相似度并取均值，超过阈值 δ 的配对触发合并。合并后的技能需覆盖原有技能的全部有效指导，同时去除冗余和矛盾。这是框架中驱动泛化的核心机制。Improve 操作：从新轨迹注入缺失模式驱动专化。Agent 拿着已有技能执行新任务时，新轨迹会暴露原技能未覆盖的边界条件和错误模式。Improve 审视新轨迹与原技能的差异，找出\"轨迹里做到了但技能没写\"和\"轨迹里犯了错但技能没预警\"的部分，精准注入。内置防退化机制：若新轨迹没有提供有用改进信号，直接返回原技能。Delta Pass Rate 拒绝采样：无需标注的自动化质量门。三种操作的基线各不相同：Create 以\"不用技能\"为基线，Merge 以\"各自原技能\"为基线，Improve 以\"改进前原技能\"为基线。候选技能需在对应任务上的 pass@1 提升超过阈值才被保留。约 7000 条高质量样本 SFT 训练 Qwen3.5-4B 模型，让技能编排成为一种可跨任务类型迁移的元能力。在能力层面，从原始执行轨迹中自动提取可复用技能，区分偶然操作与通用策略。通过多视图语义相似度合并重复技能驱动泛化，压缩技能库规模。从新任务轨迹中增量注入缺失模式，驱动专化落地，内置防退化保护。三种部署模式覆盖不同场景：Offline（批量泛化）、Online（零先验迭代）、Hybrid（冷启动+专化）。技能编排能力跨领域、跨任务类型、跨模型规模可迁移。SkillComposer 针对当前技能构建被当作一次性抽取、忽视“规范化与泛化”内在张力（贴合具体任务的技能难迁移，而过度抽象的技能指导不足）的问题，提出将技能构建分解为三个可学习操作：创建（create）、改进（improve）、合并（merge）。框架通过系统化的拒绝采样配方（rejection sampling recipe）训练，使语言模型能在推理时自进化技能，并支持三种部署模式：离线构建通用技能库、在线做任务特定精炼、混合兼具两者。分析揭示 merge 与 improve 针对正交的质量维度，且技能组合是一种可迁移的元能力。整体把技能演化建模为可组合、可训练的操作序列，在规范化与泛化间取得平衡。（说明:输入概述提及 Delta Pass Rate，abstract 仅明确“系统化拒绝采样配方”，故此处依 abstract 表述。",
  ...
  },
  {
    ...
    "methodCore": "结构化技能合约：四要素的显式约定。每个技能是一份包含八个字段的结构化合约，核心逻辑：可执行技能必须同时说清楚什么时候触发、怎么执行、注意什么、产出什么。包含触发条件与禁忌场景、具体步骤与动作序列、约束与注意事项、期望输出，以及指向原始证据的引用与置信度分数。五种资产类型：约束性分类体系防止提取偏差。所有技能归入严格分类树：宏协议（跨任务全局流程）、会话技能（多步交互模式）、微技能（原子操作）、安全规则（危险动作和禁忌）、知识参考（事实性信息）。提取时每个候选必须能归入某个节点否则丢弃：这个约束让提取器产出过程性技能而非自由摘要，也为后续合并冲突检查提供类型安全。先规划再展开：防幻觉的两阶段提取。长文档先切分成证据窗口（附带标题路径和位置锚点）。规划阶段：LLM 扫描窗口识别潜在技能候选，只产出轻量提纲。展开阶段：每个候选独立扩充为完整草案。防幻觉关键：窗口内容若不直接支撑规划中列出的步骤，展开器返回空草案而非硬编；草案必须包含至少一个过程性组件（步骤、动作、约束）才通过过滤。三层入库管理：从局部草案到可用技能库。文档级编译将同源草案按分类键合并为规范技能；注册表级协调用混合检索（稠密语义+稀疏词汇+分类兼容）匹配已有技能，管理模型执行七种生命周期动作（CREATE、STRENGTHEN、REVISE、MERGE、SPLIT、UNCHANGED、DISCARD），更新类动作仅在类型和粒度一致时允许；可视技能树将活跃技能投影为可浏览、可检索的导航树。在能力层面，从文档、手册、日志等多种异构来源自动编译过程性技能，每个技能可追溯到原始证据。推理时提供 RAG + SkillBank 双通道检索，声明性知识与过程性技能互补覆盖。七种技能生命周期动作（含合并、拆分、强化、废弃）实现技能库的持续演化管理。五种资产类型的约束性分类体系确保提取产物是可执行过程知识而非自由摘要。可视化技能树为 Agent 提供可浏览、可检索的技能导航，底层有完整版本历史与证据链。Anything2Skill 针对 RAG 主要检索碎片化陈述性证据、迫使智能体反复从段落/手册/示例/日志/轨迹推断任务流程的问题，提出分类树引导（taxonomy-guided）的框架，把异构外部知识编译为可复用、可检索、可执行的技能。给定知识记录语料，框架先将每条记录分解为证据窗口（evidence windows），在技能树先验（skill-tree prior）下执行“计划-扩展”式技能抽取。候选技能随后被转为结构化技能约定，明确规定调用条件、禁忌、动作招式、工作流步骤、约束、输出规格、支撑证据与置信分。系统在持久化 SkillBank 中经分类感知编译、注册级协调、生命周期跟踪、版本化更新与可见的技能树投影来管理技能，构建可部署的过程性记忆。推理时智能体同时从原始知识库检索任务相关段落、从 SkillBank 检索相关过程性技能，形成 RAG 提供陈述性证据、编译技能提供可复用过程性指导的双通道。",
  ...
  },
  {
    ...
    "methodCore": "双轨蒸馏：把\"做事\"和\"说话\"拆成独立可组合轨道。技能包劈成两条独立轨道。能力轨：提取持久的工作方法、技术标准、审核准则、决策启发式：管\"做什么、怎么做\"。行为轨：提取表达模式、交互规则和风格边界：管\"怎么表达、怎么交互\"。两条轨道各自有独立可调用的入口点，可完整加载也可只加载其中一条。核心设计逻辑：把事实知识、过程性判断和表面语气拆成独立文件，避免 persona 系统中最常见的三者混淆。五步管线：从异质痕迹到标准化技能包。痕迹摄入层：统一处理飞书、钉钉、Slack、微信 SQLite 导出、邮件存档、PDF、截图等异质格式。预设路由层：提供同事预设、公众人物预设、关系预设三套配置，共享同一蒸馏核心，仅在来源范围、证据要求和治理规则上做配置级区分。双轨蒸馏层：用不同 prompt 策略分别生成能力轨和行为轨内容。产物写入层：按 schema v3 渲染输出文件。分发层：支持安装到 Claude Code、OpenClaw、Codex、Hermes 等宿主。修正即一等公民：自然语言驱动的版本化修订。修正入口是自然语言反馈：用户说\"他不会这么说\"、\"这个判断不对\"即可。能力修正生成 Markdown patch 匹配二级标题定位替换；行为修正生成标准化记录 {scene, wrong, correct}。每次修正触发版本管理：归档当前版本、应用 patch、递增版本号、重新生成派生产物，支持版本历史、备份、回滚。在能力层面，从异质痕迹（聊天记录、PR 评论、文档批注、PDF 等）自动蒸馏为结构化技能包。双轨独立架构：能力轨与行为轨可独立加载、独立修正，互不干扰。自然语言驱动的修正流程 + 完整版本管理（备份、回滚、历史记录）。三套预设覆盖不同治理场景（同事/公众人物/关系），共享蒸馏管线、配置级区分。产物兼容 AgentSkills 标准，支持跨宿主安装（Claude Code、Codex、Hermes 等）。COLLEAGUE.SKILL 针对“以人为本”的智能体难以构建——与某人或角色相关的可操作知识通常嵌于异构痕迹（traces）而非干净指令中——提出一套自动化的轨迹到技能蒸馏系统（trace-to-skill distillation），经专家知识蒸馏生成人格化（person-grounded）AI 技能。给定目标人物或角色的素材，系统产出带版本的技能包，含两条协同轨道：能力轨（capability track）承载实践、心智模型与决策启发式；有界行为轨承载沟通风格、交互规则与纠正历史。技能包可被检视、调用、通过自然语言反馈更新、回滚、跨智能体宿主安装，并可选地准备受控分发。作者描述了产物约定、生成工作流、纠正生命周期、部署面与领域预设。整体主张把人格化技能表示为可移植、可纠正的包，而非不透明提示或隐藏记忆，弥补记忆/人格系统只捕捉片段、技能框架只提供打包格式而缺乏端到端蒸馏工作流的空白。",
  ...
  },
  {
    ...
    "methodCore": "第一阶段：多模态教程到结构化技能的四层提取。VLM 技能提取器将 HTML+截图教程标准化为可编辑的 SKILL.md，每条技能拆为四层：可复用操作步骤、适用条件、预期状态线索（操作完后环境应该长什么样）、恢复知识（偏离了怎么拉回来）。四层结构的关键价值不在格式而在可编辑：后续精炼时可精确修改某一层而不碰其他层。第二阶段：带着技能执行，模型冻结不动。技能被注入 Agent 上下文，与最近 W-1 轮的观测-动作历史持久共存。每个动作在任务说明、近期历史和当前技能集的联合条件下采样。整个闭环中 VLM 模型全程冻结，只有技能文件被修改。第三阶段：基于轨迹反馈的闭环精炼。每轮 rollout 后，分析器只看任务说明和 Agent 可见轨迹（没有 benchmark 分数、没有隐藏环境状态、没有 gold action 序列），输出轨迹级问题诊断$e_k$和自判结果 $r_k$（$likely_{success}$）。精炼器综合原始教程、当前技能和所有累积诊断更新技能：补上缺失的检查条件、写清模糊的状态线索、删掉教程抄来但跑不通的建议。闭环最多 N=5 轮，自判 l$likely_{success}$ 时可提前终止，节省 25% 到 53% 试次。在能力层面，从多模态人类教程（HTML+截图+Wiki）自动提取为结构化、可编辑的 SKILL.md 技能。闭环精炼：执行→诊断→修订→重跑，每次失败轨迹转化为对技能文件的一次精确修改。分析器仅依赖 Agent 可见轨迹判断 success，不依赖 benchmark 分数，自判信号可做部署停止条件。提前终止机制在任务达成率不降反升同时节省 25%-53% 试次。覆盖三类交互模式（桌面 GUI、开放世界游戏、回合制策略），18/18 组合全部正向。MMG2Skill 针对网络上丰富但多模态、异构、含噪且默认由人类执行的过程性知识难以直接作为智能体技能的问题，把该问题形式化为“教程到技能学习”（guide-to-skill learning）：将野外教程转为可执行技能，并从智能体可观察的轨迹中持续改进。为评估现有智能体在此任务上的能力，作者提出首个基准 MMG2Skill-Bench。进一步提出闭环框架 MMG2Skill：将教程编译为可编辑技能（editable skills），在执行时以这些技能约束一个固定的视觉-语言模型（VLM）智能体，并从轨迹级根因反馈修订技能——不使用基准分数作为信号。消融显示直接用原始教程提示智能体反而可能降低性能，而结构化技能构建与轨迹驱动修订二者均为增益所必需。在可推断成功的任务上，基于分析器的提前停止还能防止后期性能回退。整体形成“教程编译-技能约束执行-根因修订”的自进化闭环。",
  ...
  },
  {
    ...
    "methodCore": "诊断发现：强模型生成 ≠ 好用，边际效用全程接近零。SAPO 首先做了一项关键诊断：训练期间定期将 GPT-5.4 生成的技能取出测边际效用。结果：平均边际效用全程接近零。将技能分为 promoted（top 20%）和 discarded（其余 80%）后效用差距显著，说明确实有好技能但被淹没在大量\"水技能\"里。技能效用还是上下文条件化且随时间衰减的：同类已存在时完全冗余，随策略变强早期有用技能也会过时。预算分劈入库前验证：Split-Half A/B Testing。核心操作是把每个任务的 rollout 预算 G 切成两半：前一半跑基线 rollout（仅用已有技能），后一半跑技能增强 rollout（多加一个候选技能）。两组共享同一任务和检索上下文，仅候选技能一个变量，奖励差距直接估计候选技能的边际贡献。正效用技能被 promote 进入长期库，效用在零附近的被丢弃。与入库后追踪的差异是质的：追踪方式下低效技能在被识别前已被多次检索和注入，策略梯度被一个\"不提供价值但系统以为提供了\"的技能所扭曲。策略自我训练为技能生成器 + 上下文条件化检索排序。将效能信号升级为训练信号：promoted 技能作为正例训练策略提高生成类似技能的概率，discarded 技能作为负例降低概率。策略学到的生成概率作为检索时的上下文条件化附加排序信号，超越纯语义相似度。同样分数用于长期维护：策略认为生成概率很低的技能被标记为过时并删除。训练后期策略自身学会生成高质量技能，API 调用数在训练中段开始下降并最终归零。在能力层面，入库前 A/B 验证：用 split-half rollout 当场估计技能边际效用，阻止低效技能进入决策环路。技能效能信号反馈训练策略，让策略学会自行生成高质量技能，替代外部 LLM 调用。策略生成概率作为上下文条件化检索排序信号，超越纯语义相似度的匹配精度。基于策略生成概率的长期维护机制：自动检测并删除过时技能。SAPO（Self-Adaptive Process Optimization）针对现有自进化方法忽视细粒度推理步骤影响、导致推理器-验证器差距（reasoner-verifier gap），以及蒙特卡洛（MC）过程监督计算低效进一步加剧该差距的问题，面向小语言模型（SLMs）的自我改进提出自适应过程优化方法。其灵感来自错误相关负波（Error-Related Negativity, ERN）——推理器能在错误决策后定位错误并快速调整。SAPO 据此自适应且高效地引入过程监督信号，通过主动最小化推理器-验证器差距，而非依赖低效的 MC 估计。整体把过程级监督从昂贵的采样式估计转为主动、自适应的差距缩小，让小模型在无需大规模过程标注的情况下获得更细粒度的自我纠错能力，从而在推理密集任务上自我提升。作者还为过程奖励模型在数学与代码两类任务上引入两个新基准，以进一步考察 SAPO 对验证器性能的影响。",
  ...
  },
  {
    ...
    "methodCore": "分层多智能体架构：Manager + 五个专业化 Sub-Agent。顶层 Manager Agent 负责全局推理、任务分解和跨 Agent 协调。下层五个专业化 Sub-Agent 各维护独立局部技能空间：Cleaner（数据清洗）、Featurizer（特征工程）、Modeler（模型训练与调参）、Visualizer（可视化）、Debugger（错误诊断修复）。每个专业 Agent 的动作空间仅含其领域操作和培育的专属技能，Agent 间互不干扰。理论分析证明，工具分配到互斥专业空间后每个 Agent 的选择错误率低于统一大空间。自主技能获取（ASA）：从成功经验中长出可复用新工具。当 Sub-Agent 执行了一段从未出现的成功操作序列，且满足条件（成功完成子任务、模式在库中无对应项、是流程级新模式而非简单参数变化），成功轨迹被送去合成候选技能。候选先在同类子任务上重跑验证真伪，通过后以\"名称+功能说明+可执行代码\"存入专属仓库。配套按使用频度淘汰：高频保留、长期未用退场，避免技能膨胀占用上下文。自适应上下文压缩（ACC）：把压缩做成可学习的 RL 动作。不同于硬截断，ACC 将压缩作为 Manager Agent 的可选动作：对上下文中每段信息做保留、摘要或丢弃。压缩决策纳入 RL 目标：压缩错误导致后续失败被惩罚，压缩后仍高效完成任务且省 token 被奖励。ASA 和 ACC 呈现互补关系：ASA 往上下文加入新技能，ACC 控制上下文总量保证新技能不挤掉关键信息，两者联合使用增益最大。在能力层面，自主技能获取（ASA）：从成功经验中合成、验证、入库可复用技能，按频度淘汰，让工具空间随任务持续进化。分层多智能体解耦全局协调与专业执行，各 Sub-Agent 独立技能空间降低工具选择错误率。两阶段训练（SFT→Agentic RL）中技能空间和上下文预算同时动态变化，训练条件与部署条件一致。EvoDS 针对现有数据科学智能体受静态动作集与缺乏原则性长程上下文管理限制、难以跨任务累积可复用经验、在多阶段迭代管线中不可靠的问题，提出通过 agentic 强化学习学会扩展技能并自适应管理长期上下文的自进化自主数据科学智能体。它引入两项关键策略：（1）自主技能获取机制（Autonomous Skill Acquisition, ASA），使智能体能合成、验证并复用可执行技能；（2）自适应上下文压缩策略（Adaptive Context Compression, ACC），把上下文管理当作可学习的控制问题，而非被动截断（passive truncation）。两者被编排在一个两阶段多智能体训练方案中，使 EvoDS 能随时间自主改进。理论上，作者证明其层级化设计降低了工具选择误差，优化目标与信息瓶颈原则一致，保证高效的上下文使用。整体形成技能自生成-验证-复用与上下文自适应压缩相结合的自进化范式。",
  ...
  },
  {
    ...
    "methodCore": "CSTS：集体技能树搜索三阶段。第一阶段复杂任务分解：将复杂任务按依赖关系分解为有序子任务序列。第二阶段 CSN-Gen（集体技能节点生成）：多个异构模型并行对同一子任务产生执行轨迹，共享技能合成器将每条轨迹总结为一个候选技能节点：每个节点是集体智慧的独立投票。第三阶段 CSN-Assess（集体技能节点评估）：双评分机制，集体质量评分（每个模型对自己的技能节点做多次 rollout 验证）+ 集体可迁移性评分（每个节点在 N-1 个其他模型上交叉测试，取平均成功率）。最终分 = 质量 + 可迁移性，每子任务选最高分节点。CSRL：跨技能条件的对比策略优化。扩展 GRPO 风格的截断优化。对每个子任务，在所有候选技能条件下分别采样 rollout，计算跨技能组的相对优势（cross-skill normalization）：不是比较同一技能下不同行动的好坏，而是比较同一子任务下不同技能条件导致的轨迹质量差异。这推动策略偏好更有效的技能条件化策略，同时避免策略坍缩到单一技能依赖。训练与规模效应。CSTS 产出 2K SFT 样本，在 8 个 H100 GPU 上训练 2 个 epoch。消融显示每个组件独立贡献：基础 34.5，+CSN-Gen 39.8（+5.3），+CSN-Gen+CSN-Assess 42.8（+3.0），Full +CSRL 44.9（+2.1）。跨类别增益不均：SVM 类从 33.2 跳至 70.9（+37.7），CS 类从 30.2 跳至 78.4（+48.2），说明技能树搜索对原模型弱势领域的增益最显著。在能力层面，CSTS 集体技能树搜索：多异构模型并行产生候选技能，通过交叉验证筛选最优节点。CSRL 跨技能条件策略优化：比较同一子任务下不同技能条件的轨迹质量，驱动策略偏好更优技能。技能树作为结构化、可导航的技能组织方式，天然支持多步复杂任务的技能路由。以 9B 开源权重在 QwenClawBench 上逼近闭源模型的竞争力。OpenClaw-Skill 面向为 LLM 智能体装备有效技能以解决 OpenClaw 等真实系统复杂任务的需求，提出集体技能树搜索（Collective Skill Tree Search, CSTS）——一个基于树搜索的技能构建框架，构建结构化、多样且可泛化的技能树。CSTS 借集体智能经两个迭代阶段联合搜索、识别与组合有效技能：集体技能节点生成（CSN-Gen）利用多模型的集体知识为每个子任务探索多样候选技能；集体技能节点评估（CSN-Assess）以多模型作为评判，用两种评分机制筛选节点——集体质量评分（collective quality scoring）聚合独立评估得出稳健的有效性估计，集体可迁移性评分显式验证技能是否跨模型良好泛化。据此构建技能树与技能增强训练数据。此外引入集体技能强化学习，主动从树中选取多个相关技能以拓宽解空间探索，避免被单一技能困住而产生同质或次优解。",
  ...
  },
  {
    ...
    "methodCore": "Stage 1 — Skill Creator：从任务规范和生产轨迹中引导技能。从任务规范和用户交互轨迹中提取初始技能，每个技能是一个 (Description, Body) 元组：Description 管路由匹配，Body 管响应质量。技能创建后经人工反思门控（human reflection gating）过滤低质量产物，确保初始技能库达到生产基线。Description 和 Body 的架构解耦是整个链式设计的基石，后续两个阶段各自修改一个组件，互不干扰。Stage 2 — Route Optimizer：挖掘路由失败并修复 Description。线上流量持续变化导致意图到技能的映射退化（C2 路由漂移）。Stage 2 挖掘路由失败案例，对技能 Description 执行三种操作：Update（修正单个技能的 Description）、Merge（合并路由混淆的技能对）、Discard（移除不再匹配流量分布的技能）。路由修复是下游质量优化的前提：如果技能没被正确触发，Body 质量再高也用不上。Stage 3 — Body Refiner：双路径评估 + 跨样本归因修复 Body。单条轨迹的 LLM Judge 评分方差太高，直接按单样本修改有过拟合风险。Stage 3 改用跨样本归因：先对每个技能的所有生产响应跑双路径评估（规则路径检测结构性违规 + LLM Judge 在 TCR/CCC/CQ/CA 四维度打分），将分数离散化为 Good/Average/Poor 三档，按技能聚合计算各维度的 Poor 比例，仅当超过阈值 θ 时才触发定向修复。更新接受条件为 $J̄(B_{k+1}) ≥ J̄(B_k)$，构成单调质量保证。在能力层面，三阶段解耦闭环：Skill Creator（引导）、Route Optimizer（路由修复）、Body Refiner（质量迭代），各自修改技能的不同组件。单调质量保证：每个阶段仅在接受改进时才更新，确保技能质量不回退。跨五个视觉意图类别的严格递增加成：S1 62.5→S1+S2 67.2→Full 72.2（NoSkill 基线 59.1）。线上 A/B 验证用户行为指标全面正向：交互 UV +1.92pp，全文阅读率 +4.98pp，七日回访率 +1.15pp。SkillChain 面向已在电商平台生产规模部署的图像 AI 助手场景——一张上传图片可触发商品搜索、风格推荐、视觉百科或工具调用等根本不同的用户意图，各自要求不同的响应格式、工具调用与领域知识；缺乏按意图的行为约束会使 LLM 系统混淆这些异构模式而达不到领域质量标准，而意图空间的广度与动态性又使人工工程不可行。为此 SkillChain 闭合技能进化的生产反馈循环，通过三阶段自动化技能生命周期：技能创建器（Skill Creator）从任务规格与轨迹自举技能；路由优化器（Route Optimizer）做路由对齐（routing alignment），确保意图正确映射到技能；正文精炼器（Body Refiner）经双路 LLM-Judge 评估迭代精炼技能正文。整体把技能的创建、路由与正文精炼串成自动闭环，以应对广而动态的意图空间并保证按意图的行为质量，是面向真实生产部署的技能自进化框架。",
  ...
  },
  {
    ...
    "methodCore": "两层技能结构与图文交错加载。每个应用一套技能，顶层SKILL.md索引加底层话题文件夹（guide.md+PNG截图）。Agent推理时只持索引，按需调load_topic MCP工具拉取对应话题。关键设计：文字和图片按引用顺序交错推送——截图紧跟描述句子出现，Agent在同一上下文窗口同时看到描述和图像，不需自己匹配。纯文本对照版由同一LLM调用同源内容生成，唯一区别是视觉信息以图片还是文字呈现。两阶段技能构建。Stage 1挖文档：解析官方手册（PDF/HTML）目录结构复用为话题集合，提取文字和vendor配图原样保留。控制要点：继承维护者已有的结构比另起炉灶更省钱且更准确。Stage 2跑真实应用，两种互补子模式：Free Explorer：Opus planner分8个区域，Sonnet worker逐控件操作、截图、记录状态转换行为。Trajectory-Targeted Explorer：reviewer agent读失败rollout找出系统性UI知识缺口，针对性派worker补截图和操作说明。控制要点：两个子阶段覆盖互补的UI表面——free覆盖静态可见区域（工具栏、侧边栏），targeted覆盖只有在任务交互中才出现的动态表面（两层深的模态对话框、状态转换行为）。配对对照与模态归因。每个VISUALSKILL配一个从同源内容生成的纯文字对照版，共享话题结构和操作内容。任何性能差异只来自模态差异——不是内容差异。八个领域177个任务上验证。MCP工具 vs 直接Read。消融实验揭示加载方式决定技能能否被真正使用：MCP方式每任务读7.9张图、最后一次查阅在10.4步（边做边查）；直接Read只读0.8张图、最后一次查阅在1.5步（开头翻一下就再也不回去）。MCP把图文打包一次返回，直接Read每张图要单独调一次。在能力层面，截图作为一等公民：保留UI截图而非改写为文字，消除\"点击格式刷\"这类文字描述的歧义。MCP on-demand加载：Agent只持索引，边做边查，不是一次读完所有技能。两阶段互补覆盖：文档覆盖+真实UI探索，Free和Targeted各补不同的盲区。干净模态归因：配对纯文字版隔离模态贡献，差距可精确量化。视觉密集型应用受益最大：GIMP+16.6%，Tableau+15.0%，OpenToonz+8.9%。VisualSkill 针对计算机使用智能体（CUAs）在长程任务与未见软件上仍吃力、而既有技能库把技能产物仅表示为纯文本（忽视 GUI 交互的视觉本质）的问题，提出一种分层多模态技能（hierarchical multimodal skill）：针对每个目标应用定制，组织为一个中央索引（central index）之下的按主题文件（per-topic files），智能体通过 load_topic 这一 MCP 工具按需抓取相关主题的文本与图（figures）。每个技能以两阶段流水线构建，结合撰写的文档与实时应用 UI 探索。其关键主张是在技能产物中保留视觉图示、而非把它们“言语化抹除”，这既帮助智能体识别 UI 元素，也帮助其在每步动作后验证工作流状态。整体形成“中央索引 + 按主题多模态文件 + MCP 按需加载”的可复用技能组织方式，直接对比同源生成、仅模态不同的纯文本技能以隔离视觉信息的贡献。",
  ...
  },
  {
    ...
    "methodCore": "SkillRouter 采用两阶段检索-重排架构，两个阶段均以完整技能文本作为输入：阶段 1：双编码器检索（粗排）：微调 Qwen3-Emb-0.6B 作为嵌入模型，将用户查询与全量技能库进行向量嵌入，通过余弦相似度检索 Top-20 候选，将候选集从 8 万缩减至 20。训练数据包含 37,979 条（查询，技能）训练对，采用困难负例挖掘（语义负例 4 个 + 词汇负例 3 个 + 分类负例 2 个 + 随机负例 1 个），并通过三层假阴性过滤（名称去重、代码重叠、嵌入相似度）过滤约 10% 的负例。使用批内 InfoNCE 损失通过对比学习优化。阶段 2：交叉编码器重排（细排）：微调 Qwen3-Reranker-0.6B，将每个候选技能的\"名称+描述+代码\"平铺拼接后输入，通过交叉注意力机制对每个（候选技能，用户查询）对联合处理，生成精细化相关性得分后重排。采用列表式交叉熵损失对整个候选列表联合排序（比逐点方式 Hit@1 提升 30.7 个百分点）。推理流程：技能通过离线方式预编码，推理阶段仅需查询编码、近似最近邻搜索及对 20 个候选重排，实现高效在线服务。在能力层面，大规模技能库精准匹配：8 万技能库场景下端到端 Hit@1 达 74.0%。重排序修复：交叉编码器可修复 12.7% 的检索失败案例，仅造成 4.0% 性能下降。跨模型迁移：架构可针对不同垂直领域进行端到端微调，适配不同技能库特性。作者先在源自 SkillsBench、含约 8 万候选技能且大量重叠的基准上审视主流的渐进式披露（progressive disclosure）设计——该做法只暴露技能名与描述而隐藏完整实现体。跨稀疏、稠密与重排三类代表性基线的实验发现，隐藏技能体会导致路由准确率下降 31–44 个百分点，说明技能全文并非可有可无的元数据细化，而是该场景下的关键路由信号。基于此结论，论文提出一个紧凑的 1.2B 全文「检索-重排」（retrieve-and-rerank）管线：先以全文检索召回候选，再经重排模块排序，从而在有限算力下利用完整技能文本这一强信号。整体流程强调用小模型承载全文语义，兼顾准确率与推理效率，避免依赖更大规模的基础管线。在边界层面，基准测试仅来自有限数量的技能源，结论适用于大规模、存在大量重叠的注册表；在小型目录中，纯元数据路由可能更具竞争力。仅在四种编程 Agent 和单一执行预算下完成评估。在其基准上 SkillRouter 取得 74.0% Hit@1，为所评估基线中最强的平均 Top-1 路由表现，同时参数量少 13 倍、运行速度快 5.8 倍于最强基础管线。排序增益还在独立构建自三个技能来源的补充基准上得到泛化。此外在覆盖四个编码智能体的端到端研究中，路由增益可转化为任务成功率提升，且对更强的智能体收益更大。",
  ...
  },
  {
    ...
    "methodCore": "Skill Handbook 三层结构：模式洞察 + 技能注册表 + Agent 画像。手册由三层可复用知识构成。模式层（Mode-level Insights）：从执行轨迹中蒸馏出的经验规则，指导何时搜索、编码或直接回答。技能注册表（Skill Registry）：自动挖掘的细粒度能力抽象，如\"符号逻辑推理\"、\"Pell 方程求解\"、\"约束配对计数\"——每个技能含自然语言描述、适用场景指示信号和典型样例，支持从粗到细的分层展开（如 data_processing → symbolic_logic）。Agent 画像（Agent Profiles）：每个 Agent 在每个技能上维护一个 Beta 分布估计的成功概率，附带成本统计、优劣势总结和使用注记，路由可直接查表决策。技能发现与精炼：轨迹对比 + 统计合并拆分。技能不是专家标注的，而是从执行轨迹中自动挖掘。Skill Discovery 阶段：对同一查询和模式，取一条成功轨迹和一条失败轨迹做对比，两者间的能力差距由 LLM 抽象为可复用技能定义并加入注册表。Handbook Refinement 阶段：定期依据 Agent 表现统计——高方差技能触发拆分（标识多个底层能力），统计不可区分的技能对触发合并（消除冗余）。Pareto 最优手册筛选 + 技能感知路由。手册建完后不是全量使用。细粒度技能要求 orchestrator 具备足够的推理能力来准确识别当前激活的子技能，能力弱的 orchestrator 反而会因误判而选错 Agent。系统在验证集上评估候选手册子集，选出使目标 orchestrator 在性能和成本间达到 Pareto 最优的版本 —— 强者拿精细版，弱者拿粗粒度版。推理时，orchestrator 依次执行模式选择、技能识别，最后用 Bayesian 后验均值聚合 Agent 在所需技能上的能力得分，减去成本惩罚项，选分最高者执行。在能力层面，根治路由坍塌：将 RL 路由器的单模型集中度从 98% 降至均衡分布，各模型按能力特长被合理调用。极致数据效率：仅需几十条探索轨迹构建手册，训练样本比 Router-R1 少约 700 倍，比 ToolOrchestra 少约 300 倍。跨 orchestrator 零成本迁移：手册与 orchestrator 模型解耦，一次构建可复用至不同 backbone，全部涨点无例外。白盒可解释路由：每次路由决策有技能层面的明确理由，而非隐状态权重。Pareto 最优的性能-成本权衡：在模型路由和 Agent 编排双场景中均位于 Pareto 前沿，准确率更高、成本更低。SkillOrchestra 针对复合 AI 系统编排的两大局限:(1) 输入级路由器做粗粒度的查询级决策,忽略任务需求的动态演变;(2) 强化学习训练的编排器适配成本高,且在多轮场景中易出现路由坍塌(routing collapse)——反复调用某个强但昂贵的选项。框架不直接端到端学习路由策略,而是提出技能感知编排:先从执行经验中学习细粒度技能(skills),再对每个智能体在这些技能下的能力(competence)与成本(cost)建模。部署时,编排器推断当前交互的技能需求,并在显式的性能-成本权衡下选择最能满足需求的智能体。这种显式技能建模将路由从数据密集的 RL 策略学习,转为可解释、样本高效且可扩展的能力-成本匹配过程,并支持跨 orchestrator 迁移。",
  ...
  },
  {
    ...
    "methodCore": "GoS 的核心设计是把所有 Skill 及其依赖关系组织成一张结构化的有向图，检索时不仅返回语义相关的 Skill，还会自动补全所有前置依赖。整套体系分为两个阶段，全程不需要人工标注依赖关系：离线建图：自动生成 Skill 依赖网络。将所有本地 Skill 包解析成标准化节点（名称、能力描述、输入输出格式、依赖工具、脚本路径等），自动生成四种类型的边：依赖边：A 的输出格式匹配 B 的输入要求A → B，权重最高。工作流边：两个 Skill 经常在同一业务流程中先后使用。语义边：两个 Skill 能力高度相似，属同一功能领域。替代边：两个 Skill 可互相替换实现同一功能。在线检索：反向扩散生成完整可执行 Skill 包。混合语义-词汇种子检索：将用户查询重写为结构化检索 schema，同时用语义向量和词汇匹配找到最相关的核心 Skill 作为种子。反向加权个性化 PageRank：以种子 Skill 为起点沿依赖边反向扩散评分，不同边类型分配不同权重（依赖边 1.0、工作流边 0.5、语义边 0.2、替代边 0.1），自动补全所有前置依赖。预算重排与水化：按上下文窗口预算限制重排候选 Skill，每个 Skill 附带本地路径和执行说明，Agent 可直接使用。在能力层面，自动生成 Skill 依赖网络（四种边类型，无需人工标注）。依赖感知的结构化检索（反向扩散算法自动补全前置依赖）。Graph-of-Skills(GoS)针对技能库扩展至数千条时的两大挑战:(1) 加载全量技能会撑满上下文窗口,推高 token 成本、幻觉与延迟;(2) 单纯语义检索能召回主题相关技能,却遗漏其上下游前置依赖链,形成「前置缺口」(prerequisite gap),导致检索出的技能包无法执行完整。GoS 是一个面向大型技能库的推理时结构化检索层:离线阶段从技能包构建可执行技能图(executable skill graph);推理阶段通过三步检索出有界、依赖感知的技能包——混合语义-词法种子选取(hybrid semantic-lexical seeding)、反向感知个性化 PageRank(reverse-aware Personalized PageRank)扩散依赖关系,以及上下文预算受限的填充(context-budgeted hydration)。该设计将语义检索升级为结构感知检索,兼顾依赖完整性与上下文预算。Graph of Skills (GoS) 是推理时结构感知检索系统，通过构建 Skill 依赖图捕获技能间依赖、工作流、语义和替代关系，采用反向加权 PageRank 一次性返回完整可执行 Skill 包，兼顾成功率与 Token 效率。",
  ...
  },
  {
    ...
    "methodCore": "能力树构建：层次化技能组织与增量管理。系统从根节点出发，采用广度优先策略逐层递归划分技能集。每层划分分两步：Group Discovery：由 LLM 生成类别分组（名称+描述）；Skill Assignment：由 LLM 将各技能分配至最匹配类别。解耦两步骤可显著降低遗漏概率。根节点使用五个人工固定类别（内容创作、数据处理、软件开发、自动化、领域特定）保证顶层稳定。单技能类别直接合并至兄弟节点，未分配技能触发重试或兜底入最大类别。超大规模（超过阈值 K）时启用使用频率队列（按安装量排序）只取 Top-K 建树，其余进入休眠索引（向量语义检索，按需唤醒）。新技能加入支持增量更新——从根节点逐层分配至叶节点后，自底向上刷新路径节点描述。任务驱动技能检索：树遍历 + 向量补充 + 修剪排序。LLM 从能力树根节点出发，逐层选择相关类别子节点直至叶节点，所有触及叶子构成候选技能集。树内未覆盖技能通过向量相似度搜索从休眠索引中补充。候选集经过去重、按任务相关性排序、丢弃无关项，最终保留 Top-M（实验中 M=8）的紧凑列表。用户可在此基础上手动增选私有技能。DAG 技能编排与分层执行。确定技能集后，LLM 将用户任务拆解为子任务、分配对应技能、明确子任务间依赖关系，产出一个有向无环图（DAG）。提供三种编排策略：Quality-First 最大化质量（最深、节点最多、依赖最密）；Efficiency-First 最大化并行度（宽而浅）；Simplicity-First 只保留必需节点（最紧凑）。执行时同层节点并行、跨层按依赖顺序串行。每个节点的执行 Prompt 包含任务描述、技能调用、上游制品、期望输出和下游消费方式。支持编排复用——相似任务可直接复用历史 DAG 方案。在能力层面，大规模技能生态系统组织：将 28 万+技能层次化组织为能力树，支持从粗到细的逐级定位。DAG 多技能协同编排：三种编排策略（质量/效率/简洁优先）产生拓扑结构根本不同的执行方案。增量树更新：新技能加入无需重建整棵树，沿路径插入并自底向上更新。使用频率队列 + 休眠索引：平衡树规模与覆盖度，高频技能进树，低频技能按需唤醒。编排方案复用：基于任务向量相似度匹配历史 DAG，跳过检索与编排阶段。AgentSkillOS 面向 Claude agent skills 快速增长带来的生态利用、管理与扩展难题,提出首个系统化的技能选择、编排与生态级管理框架。框架含两阶段:(i) Manage Skills——通过节点级递归归类(node-level recursive categorization)将技能组织为能力树(capability tree),以支持高效发现;(ii) Solve Tasks——基于 DAG 管线(DAG-based pipelines)检索、编排并执行多个技能。为评估智能体调用技能的能力,作者构建了含 30 个产物丰富(artifact-rich)任务的基准,覆盖数据计算、文档创建、动态视频、视觉设计与网页交互五类。任务输出质量用基于 LLM 的成对评估(pairwise evaluation),并经 Bradley-Terry 模型聚合为统一质量分。整体强调结构化组合是释放技能潜力的关键。",
  ...
  },
  {
    ...
    "methodCore": "SkillNet 不只是一个技能仓库，更像一个技能操作系统——创建管线自动化、评估体系多维度、本体论三层组织、分发集成闭环。四源自动化创建：从执行轨迹/对话日志、GitHub 开源仓库、半结构化文档（PDF/PPT/Word）、自然语言描述四种来源，用 LLM 驱动自动提取结构化技能。生成后经过去重、过滤、分类、打标签、多维评估，只有通过质检的才入库。五维质检体系：每个技能从安全性（有无危险操作、抗注入能力）、完备性（流程步骤是否完整、前置条件是否写清）、可执行性（沙箱里能否实际跑通、有无幻觉工具调用）、可维护性（模块化程度、局部修改是否影响全局）、成本意识（延迟、算力、API 费用）五个维度量化打分。评估由 GPT-5o-mini 自动完成，可执行性维度还会在沙箱里实际跑。人类专家盲评验证：所有维度 MAE < 0.03，QWK 接近满分 1.000。三层技能本体论：分类层按功能领域分类（开发/AIGC/科研/安全等），关系图层定义四种类型化关系（similar_to/compose_with/belong_to/depend_on），技能包层将相关技能打包成模块化集合（如 data-science-visualization、e2e-browser-testing）。关系图谱用语义嵌入做候选匹配再用 LLM 确认关系类型。闭环集成：与 OpenClaw 集成后形成三种自动行为——遇到复杂任务自动搜索 SkillNet 下载匹配技能、定期分析本地技能库生成多维评估报告、完成任务后主动将成功经验打包入库。在能力层面，全链条覆盖：从创建到评估到连接到分发，其他平台更像技能市场，SkillNet 更像技能操作系统。四源自动炼制让技能库可以持续自我演化，不只是静态仓库。五维评估的自动化结果与人类专家几乎无差别，为大规模技能质量管理提供了可落地的方案。三层本体论把孤立技能变成有类型关系的知识网络，检索不再是纯语义匹配而是图结构上的导航。SkillNet 针对当前智能体缺乏系统化技能积累与迁移、频繁「重造轮子」的问题,提出一个用于大规模创建、评估与组织 AI 技能的开放基础设施。其核心是将技能纳入统一本体(unified ontology):支持从异构来源创建技能、建立丰富的关系连接,并进行多维度评估,涵盖安全性(Safety)、完整性(Completeness)、可执行性(Executability)、可维护性(Maintainability)与成本感知(Cost-awareness)五个维度。基础设施整合了一个逾 20 万技能的仓库、一个交互平台与一个通用 Python 工具包。通过将技能形式化为可演化、可组合的资产,SkillNet 让智能体从一次性经验走向持久掌握,为技能巩固提供统一机制。",
  ...
  },
  {
    ...
    "methodCore": "GraSP 不换检索器、不改执行器，在检索和执行之间插入四个模块：双维度检索：在语义相似度之外乘上历史成功率 —— 这个技能过去好不好使，加权融合后每个技能附带置信度分数，执行中低于阈值就降级。DAG 编译：将检索到的技能列表转成类型化有向无环图。状态边传递前置条件（\"找到苹果\"是\"拿起苹果\"的前提），数据边传递输入输出（\"读文件\"→\"解析 JSON\"），顺序边绑定不可颠倒的步骤。编译器通过前置匹配、输入输出匹配、历史共现三条规则自动建边，并做循环检测。遵循最小执行集原则，无关技能全部剪掉。局部修复：节点执行失败不重跑整条链路，修复限定在失败节点的 $h$ 跳邻居（默认 $h=2$），已成功节点不动。五种算子：REBIND（改参数重试）、INSERTPREREQ（补前置步骤）、SUBSTITUTE（换等效技能）、REWIRE（修正边方向）、BYPASS（跳过已满足节点）。重规划成本从 $O(N)$ 压到 $O(d^h)$。置信度路由：技能置信度太低时自动退回 ReAct 模式走传统推理路径，作为安全兜底。在能力层面，语义相似度 × 历史成功率联合打分，每个技能附带置信度分数，解决\"看着相关但实际没用\"的问题。将扁平技能列表转成带状态边/数据边/顺序边的有向无环图，技能间的依赖关系被显式编码为图结构，不再靠 LLM 自己猜。遵循最小执行集原则，DAG 只保留和当前任务直接相关的技能，无关的全部剔除——技能库再大，塞给 Agent 的始终是精简后的子集。执行失败时只修受影响子图（h 跳邻居），已成功节点不动，重规划复杂度从 $O(N)$ 压到 $O(d^h)$。GraSP 针对近期基准发现的反常现象:给智能体更多技能并不单调提升性能——聚焦的 2-3 个技能反而胜过完备文档,过多技能甚至有害;瓶颈已从技能可得性转向技能编排。GraSP 提出首个可执行技能图架构,在技能检索与执行之间引入编译层(compilation layer):将扁平技能集转化为带前置-效果边(precondition-effect edges)的类型化有向无环图(typed DAG),以节点级校验(node-level verification)执行,并通过五类类型化算子(five typed operators)进行局部受限修复(locality-bounded repair),把重规划复杂度从 O(N) 降至 O(d^h)。该设计以显式因果依赖组织技能的选择、组合与执行,强调结构化编排而非扩大技能库。",
  ...
  },
  {
    ...
    "methodCore": "核心决策：不给 Agent 看原子技能列表，而是看带有明确角色的技能组。离线建组图，在线走五步流水线。离线阶段：从技能图中挖技能组。把技能库表示成带类型标签的技能图（依赖、工作流、语义、替代等边类型），从中提取锚点中心技能组，每个组包含一个锚点技能（执行入口）、至多两个支撑成员（各有明确角色：预处理器/解析器/格式化器/检查器/回退方案）、适用性标签、可见需求线索。再在组之间建一张组图（support/artifact/visible-check/fallback/incompatibility 边），作为推理时的检索基板。在线阶段：五步流水线。查询模式提取（7 个维度描述任务）→ 锚点组选择（固定权重线性评分，跨任务跨模型同一套权重）→ 支持组扩展（贪心，最多 1 锚点 + 2 支持）→ 瓶颈化（精筛原子技能负载，控 token 预算）→ 覆盖回填 + 约定渲染。覆盖回填是最后一道保险：计算查询中必须覆盖的高置信度特征减去已呈现的，剩余债务显式回填，而不是静默丢弃。执行约定格式。START（入口 + 负责技能）、SUPPORT（辅助技能 + 何时用）、CHECK（必须满足的可见条件）、AVOID（绝对不能做的事）。Agent 看到的是有先后顺序和角色标注的执行参考，不用自己从一堆技能描述里推断\"从哪开始、谁是辅助、哪些约束不能碰\"。在能力层面，检索单元从技能升级为技能组，交付物从\"你可以用这些技能\"变成\"你应该这样用这些技能\"。执行约定中的角色标签（锚点/支撑/检查/禁止）省掉了 Agent 从技能描述里反向推断调用顺序的整段推理开销。覆盖回填在 token 预算紧张时显式兜底关键技能——去掉后 must-hit 从 1.00 暴跌到 0.73。固定权重评分策略不依赖学习，跨任务跨模型一致，行为可预测——出问题时能定位到是被检索层还是执行层。SkillRet 针对技能检索这一被低估却关键的系统挑战:小库中用户可按名显式调用技能,但随生态在紧张的上下文与延迟预算下增长,该假设失效,而技能检索缺乏基准与对真实库检索行为的理解。为填补空白,作者构建大规模技能检索基准 SkillRet:含 17,810 条公开智能体技能,以结构化语义标签(structured semantic tags)与两级分类法(two-level taxonomy)组织,跨 6 大类与 18 子类;提供 63,259 个训练样本与 4,997 条评估查询,且技能池互不相交(disjoint),既支持基准评测也支持面向检索的训练。作者在多种检索器上评测,并通过 SkillRet 上的任务专属微调(task-specific fine-tuning)提升性能,分析指出增益源于微调模型能更好聚焦长而嘈杂查询中的少量技能相关信号。",
  ...
  },
  {
    ...
    "methodCore": "三阶段闭环：从轨迹中蒸馏技能建图 → 图感知检索输出有序技能序列 → 每轮验证后图自行演化（节点+边调整）。图和策略互相推动——策略变强产生更丰富轨迹，轨迹精炼图，更好的图提供更精准的检索，加速策略学习。技能图骨架。节点是技能（标题 + 核心原则 + 适用条件 + 运行时统计），边分三种，每个节点有拓扑层级，0 级是地基技能，高层级站在低层级肩膀上：Prerequisite：A 必须在 B 前，有向；Enhancement：通用技能增强特定技能效果，有向；Co-occurrence：成功轨迹里反复共现，对称。图感知检索。种子选择（匹配当前任务的通用 + 特定技能）→ 后向 BFS（找回种子依赖的前置技能，最大深度 2）→ 前向 Beam Search（发现后续技能，束宽 3，扩展分数 = 前驱分数 × 边权重乘积）→ 拓扑排序输出有序序列（上限 8 个）。Agent 拿到的不是一堆技能名，而是从基础到高级、尊重前置条件的执行参考。图演化：节点级。四种操作，插入（新失败模式催生新技能，最多 3 个）、合并（邻居 Jaccard ≥ 0.85 合并为一个）、拆分（使用 ≥ 10 次但成功率 15-40% 的拆成 2-3 个子技能）、废弃（使用 ≥ 20 次且成功率 < 15% 移除）。图演化：边级。路径强化（成功轨迹走过的边权重 +0.05）、共现发现（≥ 2 次自动加边）、衰减剪枝（每 checkpoint 乘以 0.99，低于 0.05 移除）。每次边更新后重算节点层级。渐进式解锁。初始只开放 0 级技能，前 5 步热身期不解锁。之后每个 checkpoint 检查当前最高活跃层级平均成功率，超过 0.6 就解锁下一层，可跨级连续解锁。在能力层面，图感知检索用拓扑排序输出有序技能序列，直接解决扁平检索\"给你四个技能但不说先执行谁\"的组合性规划问题——Clean、Heat 类严格顺序任务吃到最大收益。节点四种操作 + 边三种操作构成技能质量自循环：差的淘汰、泛的细分、重复的压缩、新的失败催生新技能。边权重沉淀了跨轨迹的集体经验——被反复验证的路径权重高，长期不用的自动衰减，比静态规则或单次启发式推断更健壮。7B 模型靠图结构组织反超大规模闭源模型，知识组织方式的差距可以部分弥补参数量的差距。SkillGraph 针对 LLM 智能体从大型 API 库中选工具并正确排序的问题:现有方法对检索与排序都用语义相似度,但排序依赖工具间的数据依赖,而这在工具描述中缺失,导致纯语义方法在结构化工作流域产生负的 Kendall-τ。作者从 49,831 条成功的 LLM 智能体轨迹中挖掘出一个有向加权的执行转移图(directed weighted execution-transition graph),将工作流先后规律编码为可复用的图基础先验(graph foundation prior)。基于该先验,提出两阶段解耦框架:GS-Hybrid 检索(GS-Hybrid retrieval)负责候选选取,学习式成对重排器(learned pairwise reranker)负责排序。该设计把工具选择与顺序决策分离,用图先验补足描述中缺失的先后依赖信号。",
  ...
  },
  {
    ...
    "methodCore": "离线建一张三层技能图（社区-技能-子单元），在线双方向检索后用四步编译器把结果组织成可执行上下文。编译器是 advisory 的——只改 Agent 看到什么，不改执行逻辑，可叠加在任何现有检索器或执行器上。离线三层图：L2 技能社区（KMeans 聚类，一个社区的技能解决同一类问题）、L1 技能节点（保留为执行器兼容单位，不拆碎）、L0 子单元节点（过程性代码行/文件模式/约束说明，标准化去重后通过提取边连回源技能）。子单元跨技能共享，同一子单元可以属于多个技能。双方向检索：自上而下——任务请求和技能社区做 embedding 匹配，得分最高社区里的技能获得社区加分，保证领域一致性。自下而上——任务请求和子单元做匹配，将子单元证据反向投射到源技能上，投射得分考虑子单元区分度（被越少技能共享，区分度越高）。最终技能得分 = 技能相似度 + 子单元证据 + 名称匹配 + 社区加分。四步编译：Rescue（打捞落选技能中高相关但被淹没的子单元，预算 384 tokens）→ Affiliate Attachment（把救援子单元挂到已选技能上，不独立暴露，避免引入新执行单元）→ 任务特定指导生成（再筛一次，要求具体、对齐、不冗余）→ 组织上下文（按任务优先格式打包——先明确要干什么，然后列出用哪些技能、每个技能里的关键子单元是什么、跨技能补充提示）。在能力层面，编译层不改源技能、不改规划器、不改执行器，只改变 Agent 在执行前看到的信息组织形式，可叠加到任何现有系统上。三层图同时支持粗粒度社区路由和细粒度子单元证据聚合，两个方向的信号在检索时交叉验证。救援子单元机制打捞了因整技能落选而被埋没的关键局部信息——在技能选对的前提下靠更好的上下文编译又提了 3 个点。SkillRAE 针对检索增强执行(Retrieval-Augmented Execution, RAE)中被忽视的一环:现有工作多聚焦优化技能检索与任务执行,却很少关注如何把选出的技能证据组织成紧凑、有据(grounded)、可直接使用的形式供下游执行器完成任务。SkillRAE 是聚焦技能式上下文编译(skill-based context compilation)的两阶段方法,含离线与在线阶段:离线索引阶段在技能社区(skill communities)、技能(skills)与可复用子单元(reusable subunits)三个层级上构建多级技能图(multi-level skill graph)捕获其关系;在线检索阶段先做技能排序检索并导出选定子单元证据,再施加救援感知的紧凑编译(rescue-aware compact compilation)恢复关键证据,把粗排技能集编译为任务专属、紧凑、有据且可即用的上下文。",
  ...
  },
  {
    ...
    "methodCore": "百万级技能画像：三个维度把静态文档变成有执行证据的行为档案。对百万级技能逐一构建三维护画像：运行时需求画像：OS、写入权限、sudo、网络、API Key、MCP Server 依赖；质量画像：描述与脚本一致性、引用完整性、任务是明确可执行还是开放式模糊描述；可验证性画像：是否可客观判断\"确实成功了\"，环境可否在沙箱中复现。通过验证的技能额外获取一组合成测试任务（含标准任务描述、可复现环境和可执行验证器），真实跑 Agent 记录成功率、Token 消耗和执行轨迹。偏好驱动或需特殊硬件的技能保留画像但不强行塞评测。Agentic Library Search：推荐不是检索，是搜索。推荐阶段独立于任务执行。Agent 不解决任务，只做一件事：在结构化技能库里像程序员一样 grep、glob、read，浏览技能目录、按需打开 SKILL.md 和资源文件、判断哪些技能覆盖当前任务需求、哪些适配目标环境、哪些互补。输出不是 Top-K 排序列表，而是一套紧凑的技能暴露集 + 简洁的使用指南，告诉执行 Agent\"这三份技能相关，它们这样配合，注意环境要求\"。推荐记录同时作为后续归属的锚——执行后回溯检查推荐技能是否被实际使用、效果如何，下一次推荐的依据是上一轮真实的执行证据而非语义相似度。子任务级归属：三道归因决定技能是帮了忙还是添了乱。将整条执行轨迹拆解为最细的语义完整子任务（有独立目标、有主要评估信号、最多关联一份技能上下文），每个子任务过三道归因：结果证据：成败来自客观环境反馈还是主观判断还是无明确信号；责任归属：成功归功于技能指导还是 Agent 自己探索还是 Agent 看了技能后自行纠正，失败归咎于技能写错还是环境故障还是评估信号问题；可复用增量：提取技能中真正影响执行的那部分知识，缺失的前置条件、错误的步骤顺序、遗漏的恢复模式，丢弃随机试错和任务特定硬编码。只有成功子任务、且归因为技能贡献、且提取出可复用增量，才允许提出进化建议。证据门控的受控进化：离线 + 在线双模式。Offline Evolution 在历史任务数据上离线优化技能库，在 Terminal-Bench 2.0 上帮 GPT-5.2 提升 7.9 个百分点。Online Evolution 在测试时任务流中边跑边进化，在 SWE-Bench Pro 上提升 2.6 个百分点。治理过的技能库比原始社区技能库、比不加归属直接进化、比仅靠语义检索——都要好。在能力层面，百万级技能画像：168 万+ Skills 收录，三维画像（运行时/质量/可验证性）将静态文档转为执行证据档案。子任务级归属：三道归因（结果证据 → 责任归属 → 可复用增量），精确区分技能贡献和 Agent 自主探索，防止错误归因污染技能库。证据门控进化：只有通过归属三道检验的可复用发现才写入技能库，离线 + 在线双模式验证（+7.9 / +2.6）。一行命令接入：npx skills add，支持 Codex/Claude Code/OpenClaw 等主流客户端，Playground 可可视化推理轨迹。SkillsVote 针对开放技能生态中的冗余、参差、环境敏感与「无差别更新污染未来上下文」问题,提出覆盖收集、推荐、归因与演化的生命周期治理框架。它先对百万级开源语料(million-scale corpus)按环境需求、质量与可验证性画像,并为可验证技能合成任务。执行前,进行结构化技能文件夹上的智能体式库搜索(agentic library search)以暴露指导性上下文;执行后,把轨迹分解为技能关联的子任务,将结果归因(attributes)到技能引导执行、智能体探索、环境与结果信号四类因素,并仅接纳成功且可复用的发现进入证据门控更新(evidence-gated updates)。该设计实现两条互补路径:测试时对任务流的在线演化,以及通过冻结库(frozen libraries)的离线迁移。",
  ...
  },
  {
    ...
    "methodCore": "SRA vs RAG：可执行能力检索 vs 陈述性知识检索。RAG 检索的是陈述性知识（这段文字在说什么），评估标准是语义相似度。SRA 检索的是 可执行能力（这个技能能不能被 Agent 加载、理解和实际用于完成任务），评判标准是下游效用——检索到的技能有没有真的帮 Agent 变强。技能生态爆炸到百万级时，全量塞入上下文不可能也不该做——SRA 走的是按需检索路线，只在需要时加载相关技能。三阶段管道拆解：检索 → 融合 → 应用。SRA 将技能使用全链路拆为三段：检索，从海量外部技能库中找到相关候选技能，测的是 Top-K 里有没有黄金技能；融合，Agent 看到候选列表后能不能识别并正确加载真正有用的那几份，测的是 $loaded ∩ gold / gold$；应用，加载了的技能能不能实际在推理和执行中被使用从而提升任务表现，测的是端到端成功率提升）。三个阶段任何一个出问题，整个链路就断了。检索到了但 Agent 没加载 = 没用。加载了但 Agent 不知道怎么用 = 还是没用。四个关键实验发现。发现一：Agent 根本不知道什么时候该加载技能，同一模型在\"检索结果里有黄金技能\"和\"检索结果里全是噪音\"两种情况下，加载技能的概率几乎完全一样。发现二：Agent 加载技能的意愿跟任务是否真需要技能无关，Agent 自己能做的任务和必须靠外部技能的任务，加载概率差不多，完全缺乏\"需求感知\"。发现三：不同模型加载行为差异巨大但没有一致规律，更大的模型并不比小模型更理性，\"知道何时用技能\"这一判断能力与模型规模无单调关系。发现四：只要检索对了技能确实能帮到 Agent，这验证了 SRA 路线正确，当前瓶颈在 Agent 的融合和应用能力跟不上检索的进步。在能力层面，三阶段独立诊断：将\"Agent 用不好技能\"的模糊直觉拆解为检索、融合、应用三个可分别测量和定位的瓶颈。SRA-Bench 首个技能检索增强评测基准：5400 测试实例 + 636 手工黄金技能 + 2.6 万网络干扰技能，模拟真实嘈杂技能库场景。按需技能检索范式：替代全量上下文注入，RAG 处理知识检索 + SRA 处理能力检索形成双轮驱动。Skill-RAG 针对现有 RAG 把检索后失败当作「重试」信号而非「诊断」信号、未解决查询-证据结构性错位(alignment gap)的问题。作者观察到大量持续失败并非缺乏相关证据,而是查询与证据空间之间存在对齐鸿沟。框架耦合一个轻量隐状态探针(hidden-state prober)与一个基于 prompt 的技能路由器(skill router):探针在管线的两个阶段门控(gate)检索;一旦检测到失败态,技能路由器诊断底层原因,并在四种检索技能中择一以在下次生成前修正错位——查询改写(query rewriting)、问题分解(question decomposition)、证据聚焦(evidence focusing),以及针对真正不可解情形的退出技能(exit skill)。该设计将失败视为可归类(typed)现象而非单一整体,按类型对症下药。",
  ...
  },
  {
    ...
    "methodCore": "五类型有向边：建模相似度看不见的关系。SkillDAG 用五类有向边替代模糊的\"相关\"概念：depends_on（A 需要 B 作前提）、specializes（D 是 A 的特化版本）、composes_with（组合使用效果更优）、similar_to（功能重复可互换）、conflicts_with（同时使用会失败）。同一对技能可有多条边。有向性让 Agent 能区分\"我需要你\"和\"你跟我很像\"——这两者在 Embedding 空间里几乎不可区分。冷启动建图：双视图 + LLM 分类器。冷启动采用 两视图策略补全互补技能的盲区：e_self：视图对技能自我描述做 Embedding，捕获话题相近的技能；e_needs：视图让 LLM 想象调用场景并总结共享前置条件（如\"需要物品在手中\"），捕获功能互补的技能 —— 一份技能需要的东西恰好是另一份的产出。两张视图各自拉候选、过自适应阈值，再由 LLM 分类器分配边类型。conflicts_with 被有意留空：冲突只有执行后才能发现，留给在线演化阶段处理。Agent 可调用的图检索接口：三分路信号。Agent 调用 search(query, K, D) 一次获得三个独立字段，不融合排序：matches（Top-K 余弦相似技能）、neighbors（沿正向边 BFS 深度 D 遍历的邻居技能，带边类型标注）、conflicts（一跳冲突边）。Agent 自行决定取舍——上下文紧张时可丢弃 neighbors，需要全局视野时展开，并在推理 Trace 中记录选择理由。另有 show(skill) 按需加载技能完整内容，不提前占满上下文。在线演化：Agent 执行中写回图。Agent 发现新关系后通过两步动作写回图：propose-edge：预览提交后果，展示同对技能的历史边和编辑记录；edit-edge：正式提交，附带自然语言理由和执行 Trace 证据。提交时强制执行 无环性（depends_on/specializes 不能成环）、非矛盾性（正向边和 conflicts_with 互斥）、可回滚性（追加日志可按时间或任务 ID 回滚）。一次 Agent 判决即提交，不需要投票或人工 curator。在能力层面，结构化技能检索：将技能间关系从隐式语义相似度提升为显式的五类有向边，Agent 依结构而非分数做技能选择决策。双视图冷启动建图：e_self 捕获话题相近、e_needs 捕获功能互补，补全单一 Embedding 视角的盲区。Agent-in-the-loop 图推理：图不是离线预处理步骤，而是 Agent 执行中随时可查、可展开、可编辑的活工具。抗规模衰减：技能池从 200 扩至 2000（10×），Ret@5 仅降 3.5 个点（78.2→74.7），typed-graph embedding 排序天然比 PageRank 扩散更抗膨胀。在线自演化：Agent 发现新关系当场写回图，图随使用次数增长而持续完善。SkillDAG 认为在大型技能库中选子集是结构问题而非相似度匹配:技能之间存在依赖、冲突、特化(specialize)、重复(duplicate)等关系,这一结构对全量枚举与嵌入相似度都不可见。SkillDAG 将技能间关系建模为类型化有向图(typed directed graph),作为推理时、可被智能体调用的结构化检索接口暴露——图在执行中被查询与演化,而非固化进固定检索管线。每次搜索返回向量匹配、类型化边邻居(typed-edge neighbors)与冲突信号(conflict signals);「先提议后提交」(propose-then-commit)协议让智能体注册由执行支撑的边,使图跨回合累积结构。增益可追溯到候选排序在池扩大时保持稳健,以及集合单调(set-monotone)的在线编辑扩大真值召回而不驱逐既有命中。",
  ...
  },
  {
    ...
    "methodCore": "Relation Analyzer：从粗到细的两阶段关系分析。Analyzer 采用 粗分组 → 细分析 策略，避免全库逐对扫描的噪音和成本:粗分组阶段：仅看技能名称和简短描述，将功能相近、有潜在复用可能的技能归入同组；细分析阶段：打开每组内技能的完整内容，深入识别具体关系类型，输出结构化的 关系构建任务，指定哪些技能之间存在什么关系及其证据。两阶段设计让分析成本随技能库增长的曲线显著平缓于全对扫描。Relation Builder：向下原子提取 + 向上抽象归纳。Builder 执行两种互补的结构化操作。向下原子提取：从一组技能中提取共享的最小可复用能力（如\"定位并获取目标物体\"），作为原子技能放入金字塔更低层，同时重写原始技能，在关键步骤显式引用原子技能。向上抽象归纳：从一组技能中提取共享的高层解题模式（如\"获取→处理→验证→放置\"），作为抽象技能放入金字塔更高层，提供可泛化的任务框架而非僵化步骤。两种操作共同将平铺技能库重建为有层级、有引用、有方向的图结构。Skill Creator：框架构建 → 细节实例化的两步组合。遇到新任务时，Skill Creator 不检索匹配技能，而是两步组合：先用抽象技能勾勒高层解题结构、子目标、决策点和成功标准（框架构建）；再用原子技能填充每一步的具体操作、输入检查和输出验证（细节实例化）。新技能每层都显式引用金字塔中的已有技能，只有真正新增的部分才从零生成。Self-Evolution：增量更新，技能长回金字塔。每次任务执行结束后，新生成的技能作为新节点 增量长回金字塔。Analyzer 仅检索与新技能相关的已有技能，Builder 仅构建新技能与它们的关联，无需重建整个结构。Builder 还会检查新技能是否能被已有原子或抽象技能覆盖，若能则不重复构建，天然防止冗余膨胀。在能力层面，层级化技能重构：向下提取共享原子能力、向上归纳抽象任务框架，将平铺技能库重建为可复用的金字塔结构。组合式新技能生成：遇到新任务时从抽象框架 + 原子操作中组合出新技能，无需从零探索或生成。跨任务经验迁移：原子技能被多个上层技能共享引用，\"找苹果\"和\"找土豆\"共享同一个\"获取目标物体\"原子。增量自演化：每次任务执行后将新技能增量长回金字塔，系统随使用次数增长持续扩展且防止冗余。结构抗退化：金字塔层级结构天然区分稳定底层和灵活高层，模型升级或环境变化时仅需更新受影响层。SkillPyramid 针对智能体长期改进受限于缺乏系统化技能构建、积累与迁移的问题:没有统一的技能巩固框架,智能体会在不同任务间冗余构建相似能力,无法把经验有效转化为可复用资产,也难以将任务专属技能泛化到新场景。SkillPyramid 是一个复用已有技能经验以实现更广泛任务泛化的技能巩固框架,运行于层级技能拓扑(hierarchical skill topology)之上,并进一步引入自演化机制(self-evolution mechanism),使智能体能在任务执行过程中组合(compose)、验证(validate)并纳入(incorporate)新技能。整体思路是向下做原子提取、向上做抽象归纳,将平铺技能库重构为可组合、可演化的层级金字塔,把技能集合从静态资源池转变为动态演化系统。",
  ...
  },
  {
    ...
    "methodCore": "三阶段统一架构：选择→利用→蒸馏。Skill1 将技能选择、技能利用、技能蒸馏全部塞入一个策略模型 $\\pi_\\theta$，按顺序跑三阶段：先由 $\\pi_\\theta$ 生成自然语言查询并重排序 Top-K 候选技能、选择工具最优；再条件化于所选技能与环境交互最多 T 步；最后从成功轨迹中蒸馏出新技能并写入库。选择过程的查询生成和重排序都可被梯度优化，而非冻结的检索器。单信号分拆：低频趋势 vs 高频变化。系统只有一个二元环境奖励 $r(\\tau)$，但三项能力处于不同时间尺度：利用信用直接用任务结果；选择信用对每个候选技能维护 EMA 效用分（$\\alpha=0.05$），用 NDCG 衡量重排序质量；蒸馏信用计算 $r(\\tau)$ 与当前候选最佳效用 $\\hat{U}_i$ 的差值——正差代表新技能有增量价值，负差代表冗余。三项损失加权求和一次梯度更新。技能库淘汰策略。只有 $r(\\tau)=1$ 时新技能才入库。库满时按 $U(s) \\cdot \\log n(s)$ 分数淘汰末尾——长期高效用且使用频繁的技能保留，低效用且无人问津的淘汰。库上限为 5000 条。在能力层面，统一信号驱动：选择、利用、蒸馏全部使用同一个任务结果奖励，无需为不同阶段设计不同奖励函数。变化驱动蒸馏：蒸馏奖励为任务结果减去库中最佳已有效用，迫使策略探索库未覆盖的更优策略，防止技能冗余。可优化选择：查询生成和重排序由 $\\pi_\\theta$ 产生，选择过程本身可被梯度优化，而非依赖冻结检索器。多样性保持：t-SNE 可视化验证高频技能覆盖更广的策略空间且分布均匀，变化驱动蒸馏主动填补未覆盖场景。Skill1 针对维护持久技能库所需的三项耦合能力——选择相关技能、执行中利用技能、从经验中蒸馏新技能——现有方法孤立优化或用分离奖励源,导致进化片面且相互冲突。Skill1 训练单一策略(single policy),在共享的任务结果目标(shared task-outcome objective)下协同进化(co-evolve)选择、利用与蒸馏三种能力。策略的推理流程为:生成查询以搜索技能库、对候选重排以选出一个、在其条件下求解任务、并从轨迹中蒸馏出新技能。全部学习都源自单一任务结果信号(single task-outcome signal):其低频趋势(low-frequency trend)记功于选择,高频变化(high-frequency variation)记功于蒸馏。该信号拆解设计让单一奖励同时驱动三能力的协同进化,避免多奖励源冲突。",
  ...
  },
  {
    ...
    "methodCore": "五阶段转换流水线。Parser 按 Markdown 标题将技能散文拆为程序单元 → 对每个单元提取框架元组（动词、宾语、代码语言、脚本）并做单链聚类（故意保持高召回，假阳性由后续验证滤掉）→ 对每个聚类调用一次 LLM 产出类型化合约 κ（触发条件、I/O schema、前后置条件、副作用）→ 四重确定性校验 → 通过验证的合约写回父技能，对应段落替换为 invoke(κ, {arg=value}) 占位符。四重确定性校验——全部基于规则。Coverage：合约是否覆盖原段落 Token。Binding：每个输入参数在原文中是否有对应绑定。Replacement：原段落结构上是否允许被 invoke 占位符替换。Risk：AST 扫描附带资源是否含危险调用（裸 rm -rf、未声明网络出口等），加权风险分 ≥ 0.80 直接驳回。四条检查各有过滤重点且近乎互斥，用 30 个合成负样本标定决策阈值，确保零假阳性。三层级运行时呈现。Agent 拿到三合一套餐：具体动作模板：环境直接接受的动词和参数格式，先看；重写的父技能骨架：段落被 invoke 替换保留结构位置，再看；内联子合约：类型签名，需要时看。消融证明子合约作为独立检索条目浮出而非内联嵌入——得分反掉 27%——说明 Agent 需要的是\"在父技能上下文中被替换好的类型化信息\"而非多一个检索条目。在能力层面，命中率突破：82/402 配对比分 win GoS（47/402），输入 Token 减少 22.8%，LLM 调用次数减少 14.5%。\"散文→动作\"死循环的剪断：类型签名的\"what\"和动作模板的\"how\"分开提供，Agent 不再反复重新推导同一段散文。确定性质量保证：四重规则校验确保 LLM 生成的合约不偏离原文、不制造幻觉。与 SkillSmith 互补：SkillSmith 优化技能加载后的执行效率，SaP 优化技能被 Agent 读取时的格式。Skill-as-Pseudocode(SaP)针对 markdown 技能库以自由散文(free-form prose)形式发布、迫使智能体每次检索都重新推导输入模式与具体调用语法的问题。作者观察到这常产生「困惑→重检索→仍困惑」的死循环:智能体发出部分正确的动作、收到无信息量的环境反馈、再重检索同一散文。SaP 自动将 markdown 技能库转换为带类型伪代码(typed pseudocode)并施加确定性质量控制:对来自一个或多个技能的相似过程性段落聚类,为每簇抽取类型化合约(typed contract),并通过四重确定性校验器(coverage、binding、replacement、risk 四项检查)过滤;通过校验的合约连同复原的具体动作模板(concrete action templates)一起内联进重写后的技能骨架,从而给智能体两个互补信号:说明「技能做什么」的类型化签名,与说明「如何调用」的具体模板。",
  ...
  },
  {
    ...
    "methodCore": "Context Planner：学习预测执行效用而非语义相似度。训练一个打分器 $f_\\theta(t, s)$，监督信号来自真实执行效用 $\\Delta(t, s)$——在同一个任务上有技能和没技能的表现差异。训练数据从任务池中采样 Agent 的实际执行结果，用所有候选技能的 leave-one-out 评测标注执行效用的真实值。语义匹配和执行效用之间的相关性极弱（τ2-bench 上一份语义高度匹配的技能执行效用为 -0.20，而四份语义中等匹配的技能执行效用为 +0.40），学出来的打分器能区分\"看着像\"和\"真有用\"。自适应预算：阈值截断而非固定 K。每个任务的每份候选技能由打分器给出效用分，通过验证集上调出的阈值截断——超过阈值的全要、低于的全部不要。预算 $B_t$ 自然涌现不预设。简单任务可能只需要 1 条技能，复杂任务可能需要 6 条，每个任务拿到恰好它需要的数量。Set-Aware Renderer：技能描述互感知重写。当选中的技能超过 1 条时，Renderer 读取每条技能的原始描述、当前任务和其他同时注入的技能描述，产出改写后的描述——在原描述基础上区分边界（\"这条管这个，那条管那个\"）和减少重叠（\"两条都提到退款，但权限范围不同\"）。Renderer 被蒸馏为小模型，每次 Agent 调用前仅推理一次。单条技能时直接跳过。在能力层面，执行效用驱动的技能选择：学会预测\"真的帮到 Agent\"而非\"文本看起来像\"，打破语义匹配和执行效用之间的弱相关。任务自适应预算：每个任务按实际需要自动决定注入几条技能，不需人工预设 K。技能互感知描述：多条技能同时注入时改写描述区分边界、减少重叠，防止 Agent 被互相矛盾的描述搞混。轻量推理开销：Renderer 被蒸馏为小模型，每次调用仅推理一次。SkillsInjector 针对注入更多技能未必提升、甚至可能损害任务完成的问题,指出现有方法把技能注入当作静态步骤——用固定标准选技能、预先固定预算、且不改描述。作者主张暴露哪些技能、包含多少、如何呈现都影响下游表现,因此提出两阶段自适应方法联合处理这三项决策。第一阶段:上下文规划器(context planner)学习执行接地(execution-grounded)的技能偏好,并为每个任务准入自适应数量(adaptive number)的技能,以替代固定 Top-K。第二阶段:集合感知渲染器(set-aware renderer)相对于其共注入邻居(co-injected neighbors)定制被选描述的呈现方式,以凸显技能间的边界差异。该设计将「选哪些、选多少、怎么呈现」统一优化,直接优化注入的上下文本身。",
  ...
  },
  {
    ...
    "methodCore": "形式化定义：将技能兼容性分解为可操作的检索因子。给定查询 q 和目标技能集 S*，联合检索概率被分解为两个因子：每个技能独立被检索的相关性概率乘积 + 查询条件化的技能兼容性修正项 C(q, S*)。C≈1 恢复文档检索的独立性假设；C<1 表示技能集在查询下存在冲突（功能重叠、生态系统不兼容）；C>1 表示技能相互增强。C 是查询条件化的，同一对技能在不同查询下兼容性完全相反。 这个分解将文档检索与技能检索的本质差异锁定在 C 项上。数据构建：将 LLM 拒绝信号从废料变为兼容性监督。现有技能合成管线中，LLM 判断一组技能\"不自然可组合\"就将样本丢弃。R3-Skill 反向保留这些 SKIP 注释，作为 C 的二进制监督信号。数据生成管道对每个技能组先做两阶段评估（CoT 推理→WRITE/SKIP 判定），WRITE 分支生成 6 种风格×4 语言方向的查询，SKIP 分支归档进入 8 类拒绝原因分类体系（domain_mismatch 占 45.9%，forced_chaining 占 33.9%）。训练/测试集在技能池级别完全不相交，测试集由多位专家交叉验证。两阶段检索：Bi-encoder 粗召回 + Cross-encoder 兼容性重排。R3-Embedding（bi-encoder，Qwen3-Embedding-0.6B 微调）用多正例 InfoNCE 目标做粗召回，将 GT 中的同辈技能从分母中掩码掉避免互相排斥。R3-Reranker（cross-encoder）在候选池中施加三级兼容性标签：GT 技能=3，SKIP 伙伴=1，其他=0，用 graded ListNet 优化 C 条件化的排序。梯度分析证明 bi-encoder 下双边平衡使兼容性信号被稀释，而 cross-encoder 因其对每个 (q,s) 独立评分的能力可有效利用该信号。在能力层面，查询条件化技能兼容性被形式化定义并作为可训练监督信号，从机制上区分于文档检索。R3-Skill 是首个双语（中英）四方向技能检索基准，测试查询使用 LLM 改写逼近真实用户请求。8 类拒绝原因分类体系提供精细化的兼容性失败分析，揭示跨语言方向间拒绝模式的结构性差异。Set-Compat 指标直接衡量整个 GT 技能集是否被同时检索到 top-m 位置。R3-Skill 指出技能检索不同于文档检索:Top-K 正确性不仅取决于每个查询-技能对的相关性,还取决于检索出的技能能否在该查询下协同工作,这种查询条件化的「技能兼容性」(skill compatibility)无法仅从独立相关性恢复。作者发现 LLM 合成管线本身已产出有用信号——LLM 自身的拒绝决策(rejection decisions),指明哪些技能在给定查询下不应被一起检索,但通常被当作低质数据丢弃。论文提出 Reject-as-Resource Retriever(R3),把这一被丢弃的拒绝信号保留并用作兼容性监督(compatibility supervision),并构建双语(中英)技能路由基准 R3-Skill。作者训练由 R3-Embedding 与 R3-Reranker 组成的两阶段检索器;梯度分析解释为何该查询条件化信号在双编码器目标下较弱,而交叉编码器可将其用作分级排序监督。",
  ...
  },
  {
    ...
    "methodCore": "双流知识架构：Skill + Experience。传统Agent只用Skill（结构化工作流），XSkill首次引入Experience（动作级战术洞察）作为互补知识流。Skill是\"怎么做\"的模板（步骤、工具调用序列），Experience是\"在这种具体情况下该怎么调整\"的上下文敏感记录。两者从同一轨迹中自动提取，互相增强：Skill提供结构化框架，Experience填补执行细节。Phase I 积累：视觉接地轨迹提炼。每批rollout后，XSkill通过三个步骤提炼知识：轨迹摘要（Trajectory Summary）：视觉接地的方式将Agent行为序列压缩为可复用知识条目，保留关键视觉线索而非纯文本描述。跨rollout批评（Cross-Rollout Critique）：比较多次执行结果，识别成功vs失败的关键差异，提取经验教训。层次化整合（Hierarchical Consolidation）：将零散经验整合为结构化Skill文档，消除冗余，保持知识库精简。Phase II 推理：分解-检索-适配-注入。对每个测试任务：任务分解：将复杂任务拆解为子目标。知识检索：从双流记忆库中检索相关Skill和Experience。视觉适配：将检索到的知识适配到当前视觉上下文（而非简单文本拼接）。Prompt注入：将适配后的知识注入系统提示词，引导Agent执行。关键设计。零参数训练：所有知识以文档形式存储和检索，不修改模型权重。视觉接地：知识提取和适配均以视觉信息为锚点，适配多模态Agent场景。在线积累：随执行批次持续积累，知识库动态增长。在能力层面，双流协同进化：Skill库提供\"怎么做的模板\" + Experience库提供\"具体怎么调的经验\"，两者从同一轨迹自动提取互相增强。零样本跨任务迁移：在一个任务上积累的知识可直接迁移到新任务，平均性能提升11%。多模态benchmark验证：在VisualToolBench、TIR-Bench、MMSearch-Plus、AgentVista、MMBrowseComp五个benchmark上超越强基线。XSkill 针对直接从非结构化人类视频中提取可复用机器人操作技能的难题:巨大的形态差异(embodiment gap)与未观测的动作参数使迁移困难。XSkill 是一个模仿学习框架,分三步弥合形态鸿沟:1) 纯粹从无标注的人类与机器人操作视频中发现称为技能原型(skill prototypes)的跨形态表征(cross-embodiment representation);2) 通过条件扩散策略(conditional diffusion policy)将该技能表征迁移为机器人动作;3) 组合(compose)已学技能以完成由人类提示视频(human prompt video)指定的未见任务。整体形成一个更通用、可扩展的模仿学习框架,以跨形态技能原型为枢纽,支撑技能的迁移与组合。",
  ...
  },
  {
    ...
    "methodCore": "构造性控制实验设计：串行生成链条确保归因可靠。先创建一个扁平 Baseline：将源技能中所有功能性内容压平到一个文件里。再从这个 Baseline 出发，把同样内容按结构拆分成轻量入口+多个按需加载的参考文件组成渐进式展开（PD）变体。PD 不是从原始源技能独立写出的：它是对 Baseline 的约束式重组。串行生成链条保证两组变体知识内容完全相同，让\"差异来自结构而非内容\"的归因牢固。82 个 SkillsBench 任务，每个任务跑 5 轮，总计 1,230 次试验，基座模型固定为 GPT-5.4。三层验证关卡 + 运行时控制。构造完成的变体通过三层检查才进入执行评估：确定性门控程序化检查目录布局、文件路径、资源可达性和行为单元 diff；基于 Rubric 的语义审计自动扫描变体是否保留了任务范围、执行约束、输入输出约定；人工审核覆盖处理验证异常。运行时控制方面，所有任务-范式组合在同一 Harbor 沙箱、同一 Agent harness、同一超参数下执行，唯一变量是暴露给 Agent 的技能产物。行为层与结果层的分离发现。行为层差异是数量级的：Agent 每个轨迹平均触达的独立技能资源从 1.18 跳到 3.85，有效资源吸收事件从 1.33 跳到 3.92。结果层仅有 +4.1% 增益且分布不均：核心障碍是\"不知道怎么做\"的任务 PD 优势明显，核心障碍是\"做出来必须精确匹配规范\"的任务 PD 边际收益有限。两层的分离意味着只看 pass rate 会丢失完整的信号层。在能力层面，首次将技能组织方式确立为可隔离、可控制、可测量的独立实验变量。构造性控制实验设计（串行生成 + 三层验证 + 运行时固定）实现高归因置信度。揭示行为层与结果层分离：行为层系统性变化（资源触达 3.85×、有效吸收 3.92×）仅部分转化为 pass rate。提供对技能作者的直接指导：PD 不是普适最优，任务属性决定组织方式何时有用。SkillJuror 针对当前基准很少区分「技能说了什么」与「技能如何组织」的问题,通过渐进式披露(Progressive Disclosure)这一范式来研究——简洁的根文件按需指向支撑资源——并与归一化的扁平基线(normalized flat baseline)对比。SkillJuror 是一个通过语义受控变体(semantically controlled variants)、匹配多试验评估(matched multi-trial evaluations)与轨迹证据(trajectory evidence)来评估技能写作范式的框架,同时保持任务知识固定。其方法论核心是把技能组织方式作为独立可控变量,在保持内容不变的前提下度量组织对运行时搜索与应用行为的影响,从而分离「呈现方式」与「知识本身」的效应。",
  ...
  },
  {
    ...
    "methodCore": "离线解析：六种语义类型节点 + 四类边。一条自动流水线将原始 Markdown 技能文档拆分为六种语义类型节点：step：可执行操作或指令。example：使用演示和代码片段。param：可配置参数。precondition：执行前约束。error_handling：失败恢复步骤。concept：定义性背景段落。前五种为可执行节点，concept 保留在候选池中但不可执行。同时自动推断四种边（sequence、semantic、reference、condition）构建节点图，但在线检索阶段完全不依赖这张图：离线管线对 395 个技能一次性解析后永久缓存，在线只需查询嵌入和 MMR 循环。在线检索：全局 MMR 不打图遍历。给定查询 q 和缓存的节点集合，MMR 从空已选集开始，每次迭代选一个节点最大化 $λ·sim(e_q, e_n) - (1-λ)·max sim(e_s, e_n)$，$λ=0.7$。第一项把与查询最相关的节点前推，第二项把已有高相似替代品的节点后压。当最佳剩余候选 MMR 转负时提前终止。选中节点按原文原始顺序重排后拼成上下文。预算分两档：节点 ≤100 取 20 个，更大取 60 个：来自 k 扫描分析。放弃图遍历的关键决策。实验中自动解析出的图稀疏且不可靠：执行关键节点之间经常未被连接。局部图遍历上下文充分性仅 66.73%，全局 MMR 达 78.89%，差 12.16 个百分点。在 MMR 结果上加一阶图扩张反而让充分性从 69.8% 降到 67.4%、节点膨胀、token 节省率缩水。结论是图对这个场景是噪音源而非信号源：一旦节点被正确按语义类型标注，扁平化全局 MMR 比任何局部图遍历方案都更准也更省 token。在能力层面，技能内检索：在已知该用哪个技能的前提下按查询挑出最小够用上下文，填补生态空白。六种语义类型节点离线解析：step/example/param/precondition/error_handling/concept，一条流水线一次性缓存。全局 MMR 检索：仅依赖查询嵌入和数值计算，在线阶段不调用 LLM，中位数查询嵌入约 498 token。concept 节点动态保留在候选池中，让 MMR 在查询级别自适应判断是否需要，优于静态一刀切删除。SkillPager 针对基于技能的 LLM 智能体日益依赖长过程性文档、而全文档提示浪费 token 并稀释执行关键信息的问题,将此设定形式化为技能内检索(intra-skill retrieval):目标是在给定查询下,从一个已知技能文档中选出最小、执行充分(execution-sufficient)的上下文。SkillPager 是两阶段框架:离线阶段把每个 Markdown 技能解析为类型化语义节点(typed semantic nodes,含六种语义类型);在线阶段利用最大边际相关性(Maximal Marginal Relevance, MMR)执行全局、查询条件化的节点选择(global query-conditioned node selection)。粒度消融表明,对原始定长分块施加同一检索算法虽能达到相近充分性,但 token 成本更高,说明效率增益源于类型化语义粒度而非检索算法本身。",
  ...
  },
  {
    ...
    "methodCore": "三段式组合路由框架。给定复杂查询和2209个真实技能库：拆：decomposer（Qwen2.5-7B）拆成原子子任务序列，每个子任务恰好配一个技能。检：对每个子任务用双编码器+FAISS做语义检索，仅用技能元数据，CatR@10达69.0%。组：按技能间兼容性（I/O类型匹配、类别Jaccard、关键词共现）选最优组合形成DAG。SAD：检索增强的分解反馈回路。标准分解→检索top-15候选技能名→作为提示词喂回decomposer→重新分解。控制要点：SAD是粒度修正器而非词汇对齐器：修好了75个vanilla步数错误的查询，DA都正确的查询上CatR@1完全一致（$p = 0.97$）。直接告诉vanilla decomposer ground truth步数，DA飙到99.3%，CatR@1到39.8%，跟SAD的DA=1条件下几乎一样。Qwen2.5-14B过度分解更严重（平均4.72步），SAD同样压回。COMPSKILLBENCH基准与迁移验证。从公开MCP生态构建，300个组合式查询、2209个真实技能、24个类别，分Easy/Medium/Hard三级。迁移实验验证SAD非模板记忆：类别级留出：+35.6%相对DA提升。技能级留出：+23.2%。人类查询DA_±1：+66%。控制要点：上下文从88.4万token降到约1,160 token，降低99.9%。在能力层面，组合式技能路由：支持一个查询需要多个技能串行协同，不再局限于单选。瓶颈定位与精准攻击：DA条件分析揭示分解是瓶颈（DA=1时CatR@1从34.2%跳到41.2%），SAD集中解决。跨模型粒度修正：14B过度分解比7B更严重，SAD同样压回有效。上下文压缩：将2209技能暴露压到平均2.9个/子任务，token从88.4万降到约1,160。表征层可扩展：listwise reranker +10.3%、BGE-base +14.5%，gap在表征层不在架构层。SkillWeaver 针对自主网页智能体缺乏关键自我提升能力——难以做过程性知识抽象、技能精炼与技能组合——提出以技能为中心(skill-centric)的框架,让智能体通过自主合成可复用技能为 API 实现自我提升。给定一个新网站,智能体自主发现技能(discover skills)、执行以练习(execute for practice),并把练习经验蒸馏(distill)为健壮的 API。迭代探索持续扩充一个轻量、即插即用(plug-and-play)的 API 库,显著增强智能体能力。该框架借鉴人类通过环境探索、把经验层级抽象为可复用技能、并协作构建不断增长的技能库来自我提升的机制,将多样的网站交互打磨成可在不同网页智能体间无缝共享的 API。",
  ...
  },
  {
    ...
    "methodCore": "技能文件夹结构：知识封装层。每个 Skill 是一个标准化的文件夹，包含三部分：YAML 元数据：定义技能名称与触发条件；Markdown 指令：描述工作流程与最佳实践；脚本和参考资料：作为可执行资源。该结构将分散的领域知识收敛为可复用、可组合的模块单元，Agent 在沙盒虚拟机内通过文件系统直接访问这些内容。渐进式披露：上下文加载控制层。Skills 加载分为三个级别：元数据在智体启动时始终预加载至系统提示词，仅占用极少 Token；指令文本仅在 Claude 判定技能相关时，通过 Bash 工具读取加载；脚本与资源仅在指令明确引用或执行需要时才被调用。这种\"用多少读多少\"的策略从根本上解决了上下文窗口膨胀问题。沙盒执行模型：代码隔离层。脚本在沙盒虚拟机中执行，LLM 通过 Bash 运行脚本并仅接收输出结果，脚本代码本身不进入上下文窗口。这使得技能可捆绑任意数量的 API 文档、代码脚本和数据集，未调用内容不产生任何 Token 消耗，实现了知识与执行的解耦。> 核心链路：技能文件夹封装知识 → 元数据预加载触发匹配 → 指令按需注入上下文 → 脚本沙盒执行返回结果。在能力层面，按需文件访问：仅加载任务实际需要的文件，未被引用的文档保留在文件系统中且不占用 Token。高效脚本执行：脚本代码本身不进入上下文，仅输出结果消耗 Token，比动态生成代码更高效。无限制内容捆绑：技能可捆绑大量文档与数据，未使用内容不产生上下文浪费，理论上无容量上限。MCP 互补协作：Skill 提供\"如何使用工具\"的指令，MCP 提供\"连接工具\"的管道，两者可组合使用。开源模板生态：Anthropic 已开源十几个技能模板，覆盖 PDF 处理、品牌指南等常见场景。渐进式披露(Progressive Disclosure)是 Agent Skills 生态的基础加载范式。其核心在于:一个 Skill 以文件夹形式封装,入口 SKILL.md 的 YAML frontmatter 仅暴露 name 与 description 等极少元信息,常驻于系统上下文;当模型判断任务与该 Skill 相关时,才把正文指令加载进上下文;正文中引用的脚本、参考文档、模板等资源(references、scripts、assets)则进一步延后到真正需要时才读取或执行。由此形成“元信息 → 正文 → 附属资源”的三层按需展开结构。这一设计直接回应了长上下文成本与注意力稀释问题:无关能力不占用 token,模型无需在海量指令中做检索式定位,从而可挂载大量 Skill 而不拖累单次推理。它同时把知识组织权交还给文件系统,使 Skill 可像代码仓一样被复用、版本化与共享,是当下把“通用助手”改造为“专业智能体”的主流工程约定。",
  ...
  },
  {
    ...
    "methodCore": "节点层：离散操作步骤的二元分类。每个节点是不可再分的操作步骤，分为两类：确定性脚本节点承载可运行代码；自然语言节点仅用于需要判断或交互的环节。这种分类本身标记了哪些步骤已被工程化固定、哪些仍依赖 Agent 实时推理。边层：带类型的输入输出数据流。节点间通过显式类型标注的边连接，步骤 A 输出文件路径、步骤 B 期望接收文件路径，这个约定写在边的类型标签上而非依赖 prompt 的自然语言约定。类型系统在编译期即可捕获数据流不匹配。Schema 校验层：YAML 规范的严格约束。整个图结构由 schema 校验的 YAML 规范约束，类型错误、字段缺失、结构不一致在编译阶段被捕获。这层校验是自由文本技能不具备的结构化质量保障。编译器层：meta-skill 的自动化转换。编译器本身是一个 meta-skill，输入任意形式的人类编写材料，输出符合 AIP schema 的执行图。编译过程是质量门：源材料中的模糊之处必须被解析才能产生有效图，自由文本中隐式的内容在图结构中被迫显式化。在能力层面，将自由文本技能编译为类型化、schema 校验的有向执行图。支持节点级可寻址调试：故障可精确追踪到具体节点，修复不波及其他部分。支持全库结构化审计：按节点类型或拓扑模式对技能库进行图查询。支持基于图结构的技能检索：可按节点类型或拓扑模式检索，而非仅依赖语义匹配。为技能优化提供类型化、有边界的 RL 动作空间，有向图拓扑天然构成动作掩码。执行流图(Execution Trace)是 Agent Skills 生态中面向可观测性的工程概念。其核心是把智能体一次任务中“何时匹配到哪个 Skill、加载了哪些正文与资源、调用了哪些脚本或工具、产生了哪些中间文件与状态转移”完整记录下来,并还原成一张可视化的执行路径图。相比只看最终输出,执行流图暴露了中间决策链:哪一步触发了错误分支、哪个 Skill 被误召回、哪次工具调用消耗了大量 token。它通常以结构化事件流为底座,按时间与调用层级组织成节点与边,支持逐步回放。这一能力对调试多 Skill 协作、定位技能间冲突、复盘失败任务尤为关键,也是让“技能执行”从黑盒变为可审计过程的前提。它与渐进式披露、Skill Pipeline 等概念互补:前者提供加载与调度机制,执行流图则提供事后追溯的证据链。AIP 将 Agent 技能从自由文本 Markdown 重构为一张有向执行图：节点是离散的操作步骤（确定性脚本或自然语言描述），边是带类型的输入输出数据流，整体由 schema 校验的 YAML 规范严格约束，让技能从\"每次重读重理解\"变成按图执行、按节点调试、按类型审计。",
    ...
    "limitations": "全量追踪会带来存储与性能开销;若记录粒度不当,要么信息过载淹没关键节点,要么过粗而漏掉真正的故障点。目前仅为 spec 阶段，运行时协议尚未实现，论文将其列为 future work。实验规模有限（27 个任务），需在更大规模任务和更多模型上验证泛化性。编译器本身是 meta-skill，编译质量依赖底层 LLM 的推理能力。对于简单或篇幅较短的技能，图结构的工程开销可能超过收益。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "风险剖析：不判断好坏，推断潜在风险接口。RSA 先用 LLM 扫描技能产物，不通盘判断好坏，而是推断该技能可能调用哪些平台工具、以什么方式与 Agent 环境交互。然后将这些工具映射到预定义安全能力组：文件访问、shell 执行、Web 交互、知识/记忆读写、会话控制。这个映射告诉后续探测阶段：该技能的潜在风险接口在哪些地方，应该设计什么样的任务来触达这些接口。定向探测：造敏感环境让它真的跑。根据风险剖析输出，RSA 为每份技能生成一组定向探测任务。任务设计把该技能的风险相关接口放在一个包含真实敏感资产的本地环境中（如文件管理类技能旁放 auth.json、config.yml），Agent 被提示完成一个看似普通的任务，执行在隔离沙箱中完成。基于 trace 证据的安全判决：行为链替代文本模式。RSA 不依赖技能内容文本做判断，而是读执行 trace：系统调用日志、工具调用序列、文件访问记录、网络请求、进程生成。一份 trace 显示\"工具A → 读取auth.json → 工具B → 网络外发包含文件内容的消息\"被标记为恶意，不是因为\"看起来可疑\"，而是因为 trace 显示了从文件读取到网络外发的完整行为链条。自进化攻击下静态检测器一两轮后全面失效（检出率跌至个位数），RSA 在所有轮次中持续检出 19-20 个（共 20 个）：词可以换，行为换不了。在能力层面，运行时行为审计：用执行 trace（系统调用、工具调用、文件访问、网络请求）替代代码文本分析做安全判决。风险剖析驱动的定向探测生成：每个技能用自己的风险接口映射定制探测任务，而非通用测试集。对自进化攻击免疫：攻击者改写文本后静态检测器失效，但 RSA 依赖行为 trace 持续检出。准确率 90.0%（+13pp vs 最优静态基线），假阳性 8.0%（-4pp）。Permission Sandboxing(权限沙箱)是 Agent Skills 生态中面向安全的执行机制。由于 Skill 常携带可执行脚本并被智能体自动调用,一旦技能来源不可信或被污染,就可能读写敏感文件、发起外部网络请求或执行破坏性命令。权限沙箱的核心是把每个 Skill 的执行置于受限环境中,依据最小权限原则显式声明并约束其可触达的资源边界:可访问的目录与文件、允许的网络出口、可调用的系统能力等,越界操作被拦截或需显式授权。它通常与渐进式披露、执行流图配合——加载技能前先审阅其声明的权限,执行时在隔离环境内运行,过程再由追踪机制留痕。这样即便技能行为异常,损害也被限制在沙箱内,不会外溢到宿主系统。对于共享、公开分发且“隐式受信”的技能,这一隔离层是让智能体敢于自动执行第三方技能的安全前提。",
    ...
    "limitations": "沙箱粒度与可用性之间存在张力:限制过严会破坏正常技能功能,过松则失去防护意义;跨平台的一致隔离实现与配置维护成本较高。风险接口推断受 LLM 能力约束：对动态拼接、间接引用等 LLM 无法推断的接口可能被错误标为非关键。探测未覆盖风险接口之间的交互组合：两个单独无害组合起来有害的情况不在当前探测范围内。沙箱环境的现实性有天然边界：真实部署中恶意技能可能利用沙箱无法预料的业务特定敏感资产类型触发外泄。每个技能需要定制探测任务并实际执行，评估成本高于静态扫描。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "三层证据架构：从候选到确认的递推测量。第一层：个体标记，单个技能自身满足禁止模式，与现有扫描方案一致。第二层：结构性组合风险候选，两个技能单独通过安全检查，但能力并集触发禁止模式，这是 SkillReact 真正的测量对象。第三层：模型实际发出的工具调用尝试。三层递推让测量边界干净：静态候选是 recall-oriented 上限，人类审计将过报量校准为真实风险率。静态基准：确定性组合分析框架。从 ClawHub 采集 1,520 个技能，正则抽取器扫描每个技能的 SKILL.md 和附带代码，归入 8 种能力标签。869 个（57.2%）单独触发禁止模式后，剩余 651 个组成 211,575 个配对。对每对计算能力并集，匹配来自四个来源交叉精炼的 10 种禁止组合模式。47,075 对被标为候选（22.25%）。人类审计校准管线：双 LLM 标注 + 人类终判。47,075 个候选对按 10 种模式分层抽样，每模式 20 单元组成 200 单元黄金集。每个单元先由 Claude Sonnet 4.6 和 OpenAI Codex CLI + gpt-5.5 独立标注。人类审计员填九字段表单做出终判。各模式真实率从 5% 到 45%，总体人口加权有效率为 18.2%（95% CI [11.3%, 27.4%]）。安装时组合检查器。将测量框架打包为双模式安装时组合检查器，对每次安装操作报告集合级违规和配对级证据，耗时亚毫秒级。判定边界限制在 10 种禁止模式内，不声称覆盖全部风险，不替代运行时沙箱。在能力层面，确定性静态组合分析：对技能库所有配对计算能力并集，与 10 种禁止组合模式匹配。三层证据架构（个体标记→组合候选→模型行为）提供边界清晰的递推测量。双 LLM + 人类终判的审计管线，将过报量校准为可置信的真实风险率。安装时组合检查器：亚毫秒级，对每次安装报告集合级和配对级违规证据。Skill Pipeline(技能流水线)是 Agent Skills 生态中面向编排的执行概念。当一个复杂任务需要多个 Skill 协同——例如先抽取、再分析、再生成报告——朴素做法是让模型在推理循环里逐个临时召回技能,既低效又难以复用。Skill Pipeline 的核心是把技能之间的依赖与数据流显式组织成流水线:定义各技能的输入输出约定,识别彼此独立的分支从而并行调度,存在依赖的环节则按序串联,中间结果在环节间传递。这样多技能协作从一次次即兴决策变为可预先编排、可复用的流程模板。它与执行流图天然互补:流水线定义“应当如何跑”,执行流图记录“实际如何跑”;与渐进式披露互补:后者管单个技能的加载,前者管多个技能的组合与并发。并行化则直接缩短端到端时延,是把技能从孤立能力升级为可组合系统的关键一环。",
    ...
    "limitations": "流水线一旦固化,面对需要动态改变步骤的开放式任务会显得僵硬;编排约定的定义与维护本身需要人工投入,且并行调度对环节幂等性有要求。10 种禁止模式的分类体系不声称覆盖所有可能风险，新型攻击模式需持续更新。静态分析是 recall-oriented，过报率高（22.25%→18.2%），仍需人类审计过滤。组合风险利用率高度依赖宿主模型的危险门槛：同一组合在不同模型上表现截然不同。不替代运行时沙箱，仅覆盖安装时检查。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "File-as-State 由两个互补层面构成：四层文件系统执行架构。编译层（TaskCompiler）：将自然语言任务转换为标准化 YAML 任务包，解析为可执行的步骤定义。编排层（DAGOrchestrator）：将任务包解析为 DAG 有向无环图，按依赖关系调度执行，支持循环与分支。执行层（AgentRunner）：调用 Agent 执行单步任务，将结果写入状态文件。状态层（File System）：每步执行结果以文件形式持久化，天然支持断点恢复。AGFS 聚合文件系统。AGFS将异构后端统一为文件系统接口，开发者可用 Rust、C 或任意语言编写新的文件系统实现，编译为 WebAssembly 后动态挂载。多 Agent 可通过 QueueFS 分发任务、S3FS 收集结果、标准 grep 等工具过滤——无需额外应用层。核心模式凝练为一行管道：cat context.txt | llm > output.txt && exec action.sh——LLM 能理解、人类能理解，秒级即可验证。在能力层面，文件即状态：所有中间状态、执行结果、调度信息均通过文件持久化，任务中断后从状态文件恢复，无需额外检查点机制。零重型框架依赖：不依赖 LangGraph、Temporal 等编排框架，Shell 脚本 + cron 即可实现生产级调度，任何能读写文件的 Agent 均可直接接入。异构后端统一文件接口：通过 AGFS 将数据库、消息队列、对象存储、流式数据等统一为文件系统，Agent 间通信和协作通过标准文件操作完成。智能体每一步读取相关文件了解当前进度,执行后把新状态写回文件,下一步再基于文件继续——状态在磁盘上持久化,而非驻留于易失的上下文窗口。这一做法带来几点直接收益:其一,状态天然可续跑,任务中断后可从文件恢复;其二,人可随时打开文件审阅与修改,人机协作透明;其三,与渐进式披露的文件夹哲学一致,复用现有文件工具即可,无需专用运行时。它与 Skill Pipeline、执行流图形成呼应:流水线描述流程,文件承载流经的状态,执行流图追溯状态如何演变。本质上,它主张用“最小机制”——文件——承担原本需要框架承担的状态管理职责。AGFS 将存储、数据库、消息队列等异构后端统一为文件接口，Agent 通信、记忆、调度皆通过标准文件操作完成。在边界层面，不适合需要强一致性的分布式多节点场景。文件系统 I/O 性能瓶颈在高并发下可能显现。尚无开源实现，仅有技术文章描述（AGFS 亦处于早期阶段，正在寻求贡献者）。传统文件系统缺乏结构化数据支持和原生消息传递能力，AGFS 需要为每种后端单独实现。缺乏成熟的状态版本管理和回滚机制。",
  ...
  },
  {
    ...
    "methodCore": "源码形态分类：四类技能分别降级。编译前先判断技能的\"源码形态\"：workflow 型（有序步骤、明确控制流）编译为带依赖关系的步骤级工作流图；dispatcher 型（打包脚本和 API 描述）编译为类型化可调用算子；reference 型（表格和示例为主）索引化按需检索；insufficient 型（内容模糊）不编译、退回使用原始文档。分类基于结构性特征（标题层级、有序列表、代码块等）并由 LLM 最终判定，保守优先级：workflow > dispatcher > reference > insufficient。边界合约：编译产物的声明式 ABI。编译产物不是工作流 IR，而是一份声明式的边界合约，包含七个字段：边界类型 τ、可调用算子 O（含输入/输出 schema、底层绑定、源码引用）、输入/输出合约、风险标记 + 验证级别、动作策略 + 选择策略、以及无损降级胶囊 F——当编译产物无法处理当前请求时，完整保留原始技能内容，Agent 可回退到原始材料。带护栏的运行时状态机。Agent 使用编译技能时渐进式披露：先看到紧凑的 run_{skill} 操作柄和边界摘要，详细内容不占持久 Prompt。每次调用走三路径：执行路径（算子直接执行返回证据）、指引路径（返回参考指引让 Agent 自主推理）、阻塞路径（合约判断超出能力返回降级提示）。编译是非破坏性的——任何时候可从合约回到原始技能。在能力层面，Token 减半：推理和 LLM 调用次数减少 43%，解决时间缩短 51%（约 2 倍加速），按 Token 计费的成本减少 57%。强模型编译、弱模型跑：用强模型编译的产物给弱模型执行，某些任务上的准确率反优于弱模型直接解读原始技能。非破坏性编译：降级胶囊完整保留原始技能，Agent 可在编译器产出不足时无损失回退。跨平台适用：SkillsBench 七个任务、三个 Agent 平台、四个模型规模上验证。SkillSmith 针对现有框架中技能被作为上下文指导整体注入推理循环所带来的两类冗余——无关上下文注入(irrelevant context injection)与重复的技能特定推理与规划(repeated skill-specific reasoning and planning)——提出一套边界优先(boundary-first)的编译-运行时框架。其核心是把技能包离线编译成最小可执行接口(minimal executable interfaces):从技能中抽取细粒度的操作边界(fine-grained operational boundaries),使智能体在运行时能够动态地只访问并执行真正相关的组件,从而最小化不必要的上下文注入与冗余推理开销。此外,该框架具备跨模型复用特性:由更强模型产出的编译产物(compiled artifacts)可被更小或更高效的运行时模型复用,在原始技能解释失败的情形下提升任务准确率。整体思路与传统“编译一次、按需执行”一致,把技能从被整段喂给模型的原始文本,转化为带明确边界、可精确取用的运行时接口。代码与数据已在 GitHub 的 AetherHeart-AI/Aeloon 开源。",
  ...
  },
  {
    ...
    "methodCore": "Stage 1 — Epistemic Directive Extraction：从实验记录中提取带确定性标签的指令。输入实验记录文本，LLM 提取指令并叠加两维标签：第一维：操作类型，FLAG_DATA、CONDITION_CHANGE、ANALYSIS_SUGGESTION、PROTOCOL_CHANGE、PARAMETER_SHIFT；第二维：确定性标签，FACT（作者确认的观测事实）、JUDGMENT（作者不确定的判断）、SUGGESTION（留给下次实验的建议）。这三个标签直接控制 Agent 后续执行权限：FACT 允许强操作（不可逆数据处理），JUDGMENT 默认退回审查保留模式，SUGGESTION 不触发任何文件级操作。Stage 2 — 确定性 MetaSkill 编译：零 LLM 参与的确定性编译。输入带标签的指令集，输出符合 SKILL.md 标准的技能胶囊文档。编译过程完全确定性：不是让 LLM 重新理解再写，胶囊的每个字段要么从 Stage 1 的 EDE 记录继承，要么由领域配置固定。让 LLM 做编译可能自行将不确定判断升级为指令，这个决定本身就引入新的不确定性源。每个胶囊还带原文片段的加密链接，Agent 执行强操作时可回溯到实验记录原文对应语句。执行时门控：确定性标签 + 信号证据联合检查。每条胶囊的确定性标签与实际文件的信号证据做 joint check。FACT 胶囊且数据特征与作者描述一致时授权强操作。JUDGMENT 胶囊仅在文件信号证据完全独立支持同样结论时才执行，否则只能标为待人工复核。SUGGESTION 胶囊不触发任何永久修改。本质上是简化贝叶斯决策逻辑：在没有观察到独立验证信号之前，不对基于不确定判断的结论做不可逆操作。在能力层面，确定性三元标签体系：FACT/JUDGMENT/SUGGESTION，每个标签直接控制 Agent 的操作许可级别。执行时 joint check 门控：确定性标签与实际信号证据双重验证后才授权强操作。在 JUDGMENT 主导和 FACT 主导的实验中，只有 Notes2Skills 同时避免了不确定性漂白和指令丢失。然而以往对科学文本的研究多聚焦论文、协议或结构化数据库,把非正式的实验室笔记这一输入留待挖掘。关键难点在于:笔记常在同一段落里混杂已验证的观察、试探性判断与可能的下一步实验;若这些信号被混为一谈,AI 智能体可能把不确定的科学判断误当作已确认的结论或可执行动作。为此,Notes2Skills 提出一个两阶段框架(two-stage framework),把实验室笔记本转化为可验证的技能(verifiable skills),同时保留作者的确定性标注。其核心主张是:确定性保持(certainty preservation)正是连接实验室笔记与可靠智能体技能之间此前缺失的一环,由此指向更安全的 AI 协作科学家(co-scientist)系统。",
  ...
  },
  {
    ...
    "methodCore": "SkillsBench 通过三阶段管线构建基准并评估 Skills 效能：基准构建。从三个来源聚合去重得到 47,150 个独立 Skills（开源仓库 12,847 + Claude Code 生态 28,412 + 企业 5,891）。并行地，105 位贡献者提交 322 个候选任务，每个任务包含指令、Docker 容器化环境、参考解法和确定性验证器。质量过滤。自动化检查（结构验证、Oracle 100% 通过、GPTZero 人类撰写检测、Skill 泄漏审计）与人工审核（数据真实性、任务现实性、Oracle 质量、Skill 质量、反作弊）结合，精选 84 个任务横跨 11 个领域。评估协议。每个任务在三种条件下运行：无 Skills、精选 Skills、自生成 Skills。在 Claude Code、Gemini CLI、Codex CLI 三个商业平台上使用七个前沿模型，pytest 确定性验证器产生二元 pass/fail，共生成 7,308 条有效轨迹。在能力层面，配对评估 Skills 效力：同一任务在三种条件下对比，直接测量 Skill 的边际贡献，而非评估 Agent 的绝对能力。跨领域跨模型分析：11 个领域 × 7 种 agent-model 配置的系统性实证证据，揭示 Skills 效力的领域异质性和 harness 差异。Skills 设计因素分析：量化 Skills 数量（2-3 个最优）、复杂度（Detailed/Compact 优于 Comprehensive）和模型规模（Skills 可部分替代模型容量）对性能的影响。其核心方法论是「配对评测」（paired evaluation）——在完全匹配的「无技能」与「精选技能」两种条件下对同一批任务运行，从而将技能带来的边际增益从模型底座本身的能力中剥离出来。最新的聚合评测覆盖了 18 种模型-脚手架（model-harness）配置，在这套 87 任务基准上以匹配条件运行。摘要还给出了关于技能设计的结论性观察：模块数不超过三个的聚焦型技能（focused Skills）表现优于更大或穷举式的技能捆绑包（exhaustive bundles），且配备技能的小模型可以匹敌不配技能的大模型。该工作将配对评测确立为严格衡量技能在高专业度智能体工作中功效的基础范式。核心发现：精选 Skills +16.2pp，自生成 –1.3pp，2-3 个最优，小模型 + Skills 可匹敌大模型。在边界层面，覆盖与泛化：仅评估终端型容器化任务，结果可能不适用于 GUI Agent、多 Agent 协调或超长周期工作流；模型和 harness 集合有限，商业平台行为可能随时间变化。因果归因与控制：Skills 注入增加了上下文长度，观察到的增益可能部分反映\"更多上下文\"而非过程性结构；需要更强的长度匹配基线（如随机/无关文本、仅检索文档对照）。确定性与生态效度：容器化提供状态隔离但非完美确定性；应评估生态代表性设置，包括低质量 Skills 和自动选择 Skills 的场景。",
  ...
  },
  {
    ...
    "methodCore": "SkillProbe 框架分为五层架构（输入层、编排层、技能层、输出层、基础设施层），核心是三阶段审计流水线：阶段 1：Gatekeeper（准入过滤）：采用\"解耦检查+集中聚合\"架构，将现有安全工具封装为独立执行器并行检测，覆盖合规性、恶意代码模式、风险依赖、权限声明合理性等维度。遵循保守原则，任一检查项不通过即全局拦截。阶段 2：Alignment Detector（语义-行为一致性检测）：将一致性问题形式化为文档声明能力集 D(s) 与代码实现能力集 C(s) 的匹配问题。通过文档提取器与代码提取器并行解析、语义归一化后，构建四类对齐矩阵实现风险分类——完全对齐（Match）、过度声明（Over-declaration）、影子功能（Under-declaration，高风险）、复杂偏差（Mixed）。阶段 3：Flow Simulator（组合风险模拟）：通过风险指纹标签化——为每个技能打上输出风险标签和输入敏感标签——将组合风险检测复杂度从指数级 O(2^N) 降至线性级 O(N×Rules)。基于 9 类核心风险链路策略（命令注入、间接提示词注入、数据泄露、意图劫持等）批量执行攻击路径模拟。引入证据强制机制，每个漏洞必须引用原始代码或文档片段作为证据。裁决机制：基于恶意模式、语义一致性、组合安全性三大维度构建安全评分卡，采用一票否决原则，将技能分为 REJECTED（存在关键漏洞）、CONDITIONAL（低风险偏差需人工复核）、APPROVED（全维度符合安全基线）三个等级。在能力层面，语义-行为一致性检测：识别\"文档合规、代码恶意\"的语义欺骗攻击。跨技能组合风险识别：通过风险指纹标签化，将组合爆炸从指数级降至线性级。结构化安全评分卡生成（REJECTED / CONDITIONAL / APPROVED 三级裁决）。SkillProbe 针对集中式技能市场面临的两类安全挑战——语义-行为不一致（semantic-behavioral inconsistency）以及技能间组合风险（inter-skill combinatorial risks，即单独看均无害的技能在协作调用时诱发恶意行为）——提出一个由多智能体协作驱动的多阶段安全审计框架。其核心创新是「Skills-for-Skills（以技能审技能）」设计范式：将审计流程本身封装为标准化的技能模块，驱动专门化的智能体走完一条严格流水线，包括准入过滤（admission filtering）、语义-行为对齐检测（semantic-behavioral alignment detection）与组合风险模拟（combinatorial risk simulation）。作者以来自 ClawHub 的 2500 个真实技能、跨 8 个主流 LLM 系列开展大规模评测。研究揭示了一个「流行度-安全悖论」：下载量并非安全质量的可靠代理。此外发现高风险技能在风险链维度上构成单一的巨型连通分量（giant connected component），说明级联风险是系统性的而非孤立事件。作者希望 SkillProbe 能为构建可信的 Agentic Web 提供可扩展的治理基础设施，系统已在 skillhub.holosai.io 公开体验。",
  ...
  },
  {
    ...
    "methodCore": "环境工程与任务自动生成。读入SKILL.md，分析16类环境依赖（工具/CLI、认证凭据、运行时等），由环境工程Agent逐一检查并自动满足。LLM从技能内容反推真实使用场景，每个技能最多生成三个任务。控制要点：任务描述不透露操作步骤和评分维度——防止Agent猜答案。质检Agent验证环境可达性、输入完整性和评分标准泄露。配对对比与双维度评分。每个任务跑两遍——有技能和无技能——共享相同指令、输入、环境和评分标准。评判Agent（固定Sonnet 4.6）对照两套隐藏评分标准打分：目标完成度：产物对不对——衡量\"能不能做\"。指令遵循度：做法对不对——衡量\"是不是按你的方式做\"。控制要点：2026年前沿模型完成度已接近饱和，真正的差异全在遵循度。Opus 4.8遵循度88.0 vs Nemotron 3 Nano 30B仅25.2。单技能诊断。框架不仅产出聚合分数，还能针对每个评分维度做精确诊断。以HuggingFace hf-cli技能为例，每个子命令的遵循情况有单独分数，技能作者能精确知道哪些指令被吸收、哪些被忽略、哪些写太模糊理解错了。成本-性能最优组合发现。19个配置的对比揭示了技能加便宜模型可逼近旗舰模型。控制要点：效用和基线强度强负相关（$r = -0.90$），强模型基线高则技能边际价值被压缩。GLM 5.1加技能91.1分成本$0.89 vs Opus 4.8的$3.26。在能力层面，边际贡献剥离：通过基线对比，把技能增量和backbone自身能力分开，PRG只反映净增益。双维度评估区分：完成度衡量能不能做，遵循度衡量做法对不对，饱和的完成度已不是有效指标。单技能诊断：精确到评分维度的前后差异，定位技能中有效/冗余/模糊的指令。成本-性能最优组合发现：GLM 5.1加技能91.1分成本$0.89 vs Opus 4.8的$3.26。工作流型vs建议型技能区分：程序化技能（媒体处理+38.1）远好于建议型技能（测试+16.7）。Cross-Evaluation 是 Agent Skills 生态中的一种工程实践思路：不满足于单个技能在单个模型、单个任务上的孤立结果，而是把评测组织成「多技能 × 多模型 × 多任务」的横向对比矩阵。其工程价值在于三点：一是通过对照同一任务在不同模型底座上的表现，帮助区分「技能贡献」与「模型自身能力」，避免把底座强度误记为技能功效；二是通过让多个候选技能在同一任务集合上竞赛，为技能选型与市场排名提供横向可比的证据；三是通过跨任务铺开，暴露某个技能的适用边界——它在哪些领域生效、在哪些领域反而拖累表现。这种矩阵化对照通常以配对/对照条件（有技能 vs 无技能）为基础单元，配合确定性或可复现的验证方式，把零散的评测结果沉淀为可持续更新的横向榜单。它面向的是技能市场治理者、Agent 平台方与技能作者，帮助他们在众多技能与模型组合中快速定位「值得部署」的组合。",
    ...
    "limitations": "作为工程实践缺乏统一基准与公开数据支撑，横向可比性依赖各方口径一致；矩阵规模增长带来的评测成本与结果可复现性是主要工程约束。单评判模型（Sonnet 4.6），风格审美类指标上偏差难免。数据集偏软件工程，500个技能绝大多数是编程/DevOps/Web开发。有技能条件告知了Agent技能存在，但真实部署中技能选择是独立变量。约20%技能因环境依赖太复杂被过滤（数据库、MCP服务器等），覆盖受限。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "SKILLFLOW 将技能终身演化评估拆为一条三阶段流水线，核心抽象是 DAEF（Domain Agnostic Execution Flow）和技能补丁（skill patch）机制。DAEF 执行流抽象：将每个技能定义为一组有序的执行步骤和对应的验证检查点，使不同领域、不同格式的技能可以在统一框架下被评估和比较。技能补丁机制：每次技能更新被视为对原始技能的一个补丁，系统追踪补丁链上的性能变化，量化每次修改带来的净收益——修了几个 bug、引入几个回归。三阶段演化协议：初始训练建立基线 → 技能更新注入变更 → 持续适应观察长期漂移，模拟真实部署中技能库不断被修改的场景。跨模型退化分析：在 ALFWorld、WebShop、ScienceWorld 三个环境上，对 GPT、Claude、Gemini、Qwen、DeepSeek 等 11 个模型做退化曲线追踪，区分\"技能本身退化\"和\"模型在执行中忽略技能\"两类根本原因。在能力层面，为 Agent 技能演化提供标准化的评测框架，把\"技能越改越差\"这个之前只能定性描述的问题变成可定量追踪的指标。用技能补丁链追踪每次技能更新的净效果——修了多少个 case、搞砸了多少个原本能对的 case。跨 11 个模型的退化曲线对比，揭示了一个反直觉发现：基座能力更强的模型，在技能持续更新中并不一定更稳定。区分两类退化根因——技能内容真的写坏了，还是模型在执行时没有正确遵循技能指令——直接指向不同的修复策略。SkillFlow 针对「加载过多、尤其是无关技能会拖累性能」的问题，把技能获取（skill acquisition）重新表述为一个信息检索问题——检索对象是从 GitHub 索引的约 36K 个社区贡献 SKILL.md 定义构成的语料库。它是首个专为 Agent 技能发现设计的多阶段检索流水线，通过四个阶段逐步收窄候选集：稠密检索（dense retrieval）、两轮交叉编码器重排（cross-encoder reranking），以及基于 LLM 的选择（LLM-based selection），在每一阶段平衡召回与精度。作者在两个编码基准上评测：SkillsBench（87 任务、229 个匹配技能）与 Terminal-Bench（89 任务、无匹配技能）。实验揭示了一个关键对比：检索本身只在语料具备高质量、可执行技能时才有效——当语料缺乏目标领域的可运行技能时，即便 Agent 乐于使用检索到的技能，也不会带来性能提升。作者强调技能增强 Agent 的实际影响取决于语料覆盖度与技能质量，尤其是可运行代码与打包产物（bundled artifacts）的密度。",
  ...
  },
  {
    ...
    "methodCore": "三阶段拆解：把技能生成从黑盒变成白盒。SkillLens 将技能全链路拆为三段并让每个阶段可以被独立诊断。阶段一：经验生成，目标模型 M 在训练任务上跑，产出包含成功和失败轨迹的经验池。阶段二：技能提取，提取器 E 从经验池中蒸馏技能，提取框架刻意最小化（仅逐轨迹分析 + 层级合并），不做任何领域特化或启发式过滤，所有抽象决策全留给提取器自己做。阶段三：技能消费，同一个目标模型 M 装上技能在留存测试任务上评估，技能效用 = 有技能时的分数减去没技能时的分数。这个设计让\"谁提得好\"和\"谁用得好\"可以被分开度量——提取效能（EE）衡量一个提取器跨不同目标模型的平均产出质量，目标可进化性（TE）衡量一个目标模型从不同提取器产出的技能中平均获益多少。六个关键实验发现。发现一：模型生成的技能平均有用，但存在系统性负迁移，相当比例的技能装上后反而让模型表现变差，且不是偶然噪音。发现二：好的提取器不一定是好的消费者，A 模型帮别人提的技能比帮自己提的更好，但 A 模型自己使用技能时受益很少，提取能力和消费能力是两个独立维度。发现三：EE 和 TE 跟模型参数规模之间没有单调关系，一个中等规模的模型可能比一个大模型产出更有用的技能。发现四：经验池的成功轨迹比例、轨迹多样性、失败案例覆盖范围，比提取方法本身更能预测技能最终好不好用。发现五：好技能的共同特征，明确的触发条件、清晰的步骤顺序、标注的常见陷阱、可操作的具体指令；烂技能的共性——内容太抽象、过度依赖示例、缺少错误处理。发现六：技能跨模型迁移极不稳定，同一份技能装给不同模型效果天差地别，因为不同模型对同一份技能的解读方式完全不同。Meta-Skill：从研究发现到可操作的技能生成指引。论文将全部发现浓缩为一份 Meta-Skill：不是给 Agent 用的技能，是给技能提取器用的技能。它指导提取器在生成技能时：从成功轨迹提炼具体可操作的步骤而非泛泛原则；从失败轨迹提炼明确的触发条件和边界 case；给每个步骤标注前置条件、预期结果和常见陷阱；避免过长的示例段。加上这份 Meta-Skill 后提取器产出的技能在多领域上一致提升，负迁移大幅减少。在能力层面，三阶段白盒诊断：将技能生成从\"最终 Agent 变没变强\"的模糊信号拆解为经验池质量、提取器能力、消费者能力三个独立账本。EE/TE 双指标量化：分别度量提取器的跨模型泛化产出能力和目标模型吸收技能的潜力，两个维度相互独立且与模型规模无关。技能负迁移的系统性暴露：25% 的技能导致表现下降，且无法靠阅读技能文本判断哪个更好。Meta-Skill 实践指南：从研究发现中提炼可操作的技能生成指引，直接改善提取器产出质量并减少负迁移。经验池-技能质量因果关系揭示：经验池的多样性比提取方法本身更能预测最终技能质量。SkillLens 针对现有系统把技能当作扁平、单一分辨率提示块所导致的「相关性与成本」张力——注入粗粒度技能会引入无关或误导性上下文，而重写整个技能则昂贵且往往不必要。它是一个分层技能演化框架，将技能组织成策略（policies）、战略（strategies）、流程（procedures）与原语（primitives）四层图，并以混合粒度进行检索。给定任务时，SkillLens 先检索语义相关的技能种子（skill seeds），通过在技能图上的度校正随机游走（degree-corrected random walk）扩展，再用验证器（verifier）判定每个被访问的单元应被接受、分解、重写还是跳过，从而让 Agent 直接复用兼容的子技能、仅对局部不匹配的组件做适配。为随时间自我改进，SkillLens 进一步精炼多粒度技能与验证器以优化其路由决策。作者提供了理论分析：在稀疏不匹配假设下混合粒度适配的成本为亚线性（sublinear），且演化更新规则单调改进验证目标直至局部最优。评测在 MuLocbench 与 ALFWorld 上进行。",
  ...
  },
  {
    ...
    "methodCore": "六风险域 × 五类别 × 八攻击手法的攻击矩阵。SkillSafetyBench 将技能安全风险组织为六个域：上下文信任风险（检索结果被污染、示例被篡改）、权限越界风险（伪造审批信号、诱导扩大行动计划）、运行时劫持风险（解释器替换、沙箱边界绕过）、输出腐败风险（产出被篡改、敏感数据静默泄露）、状态持久化风险（后门存活于后续会话）、供应链和知识中毒风险（依赖版本替换、检索语料投毒）。每个域 5 个标准化类别，对应 8 种攻击手法。干净用户请求 + 有毒技能材料的攻击模型。核心创新在于攻击面的划定：用户请求保持合法，模型推理无恶意，工具调用正常——但 Agent 加载的技能材料中含污染载荷。Agent 在忠实执行合法技能工作流时被误导，因为被替换的环节在其视角看来都是\"正常操作\"。每个案例配一个确定性 verify_attack.py 脚本，以执行产物中的实体证据（恶意痕迹、泄露载荷、污染记忆记录）而非主观判断为准。多 Agent 框架 + 多模型后端评估矩阵。评估覆盖 Claude Code、Codex CLI、Cursor CLI、Qwen3 Coder 等主流 CLI Agent，分别搭配不同的底层模型。发现不同 Agent 在同一攻击类别上的失守模式有明显差异——Agent 框架如何加载技能、管理上下文、分配信任给本地文件，这些工程决策同样决定了安全边界。在能力层面，首个技能攻击面系统化评估：155 个案例、6 风险域、30 类别，提供了技能安全评估的标准化框架。供应链级攻击建模：攻击可隐藏于技能文档、辅助脚本、Docker 模板、检索语料、依赖包中的任一层。确定性证据验证：每个案例配备 verify_attack.py 脚本，以实体证据而非 LLM 主观判断确定攻击成功。跨框架安全差异面揭示：Claude Code 和 Codex CLI 在同一攻击上失守模式不同，暴露框架设计的工程决策影响安全。它是一个可运行的（runnable）基准，专门评测这类「技能介导的安全失败」（skill-mediated safety failures）。基准包含 155 个对抗案例，横跨 47 个任务、6 个风险域（risk domains）与 30 个安全类别（safety categories），每个案例均由针对该案例的基于规则的验证器（rule-based verifier）评估。作者用多个 CLI Agent 与模型底座开展实验，结果表明非用户来源的攻击（non-user attacks）能够持续诱发不安全行为，且在不同领域、攻击方法与脚手架-模型配对（scaffold-model pairings）间呈现出各异的失败模式。研究据此指出：Agent 安全不仅取决于模型层面的对齐，还取决于 Agent 如何解读技能、信任工作流上下文，以及如何通过可执行环境采取行动。",
  ...
  },
  {
    ...
    "methodCore": "生成与执行解耦的评估架构。SkillGenBench 的核心设计是将生成器和执行器拆开。生成器负责从原始材料产出标准化技能包，执行器固定不变不参与比较，评测统一协议且以确定性执行验证为主。这消除了已有基准中\"方法 Backbone vs 执行 Backbone vs 检索策略\"的变量混淆，确保评测差异完全来自生成能力。两种生成模式 + 两类知识来源。任务已知模式：生成器拿到原始材料 + 任务描述，测的是\"知道要干什么后能不能精准提炼\"。任务未知模式：生成器只拿到原始材料，不知道下游任务，测的是\"不看考题能不能写成好教材\"，目前这一模式几乎是空白，且实验显示该模式下生成技能在部分情况不如无技能。代码仓库类：程序知识隐藏在目录结构、调用关系和配置中，最佳 Backbone pass@3 仅 14.4%。长文档类：知识显式写出但散落各处，最佳 pass@3 为 25.0%。五阶段质量保证流水线。任务不是手写的——经过知识图谱构建、场景生成、任务和测试生成、无技能验证（太简单或材料直抄都能搞定的打回）、参考技能验证（连参考技能都跑不通的打回），反复循环直到落入目标难度\"甜点\"，最后加人工审核。在能力层面，技能生成能力的独立标准化评估：首次将\"从材料到技能\"这一步从端到端评测中解耦，纯测生成质量。任务未知模式首次成体系评估：揭示不看考题写教材在代码仓库类几乎做不到的残酷现状。跨方法 × 跨 Backbone 矩阵对比：五种方法 × 六种 Backbone，目前最好方法 + 最强 Backbone 在代码仓库类仅 17.1%。静态技能质量六维分析：接口合约完整性、环境依赖覆盖等六维，揭示不同方法各有优缺、全优者为零。它在统一且受控的协议下评测技能生成流水线：生成器接收原始语料并产出标准化的技能产物（standardized skill artifacts），随后在固定脚手架（fixed harnesses）下执行，并用统一的评测程序评估。基准覆盖两种生成模式（regimes）：任务条件生成（task-conditioned generation，任务揭示后再合成特定任务技能）与任务无关生成（task-agnostic generation，需在下游任务已知前先蒸馏出可复用技能库）。它还跨越两类互补的程序来源：以仓库为依据（repository-grounded，程序分散在代码、配置与脚本中）与以文档为依据（document-grounded，程序与约束需从长文本中蒸馏）。作者提供了标准化的任务规范、锁定环境（pinned environments），以及以确定性执行检查（deterministic execution-based checks）为中心、辅以诊断信号的评测协议。实验横跨多种技能生成方法与底座，显示出显著的性能差异，并揭示了从软件仓库与从长文档生成技能时各异的失败模式。",
  ...
  },
  {
    ...
    "methodCore": "完整的技能进化弧线评测设计。六个环境（代码修改、API 编排、数据处理、文档转换、研究合成、沟通操作），每个五个任务家族，每个家族六个角色任务。三个学习任务：标准任务采集经验 → 升级版暴露缺失的子能力 → 变体版改变形式保持底层逻辑，每步 Agent 可选写/改/不改技能。三个冻结评估任务：上下文漂移（技能需正确识别但描述变了）、对抗性捷径（存在绕开正确流程的浅层方案）、组合（需与其他技能协同），评估时技能库冻结——测的是\"学到的能不能迁移\"而非\"现场再学\"。四组控制条件 + 原始轨迹复用对标。裸跑（无技能）、零样本技能（任务前生成不更新）、自主生成（从空库自我归纳）、精炼种子（从有盲区的人工技能出发修补）。更重要的是把 raw-trajectory reuse（直接注入之前任务的经验轨迹作为上下文）跟 skill-based 条件做对比，暴露蒸馏的有损程度。多平台 × 多模型矩阵。评估覆盖 Claude Code、Codex CLI、Gemini CLI 三个 Agent 框架和十个模型配置，测量技能形成能力在不同 Agent 平台和模型间的差异。在能力层面，技能进化标准化测试协议：首次将\"从经验到可复用技能\"的整条链条做成可量化评估的标准协议。迁移 vs 记忆的系统化区分：冻结评估任务（上下文漂移、对抗、组合）区分 Agent 是\"记住了\"还是\"学懂了\"。蒸馏有损性的量化证据：原始轨迹复用经常超过蒸馏技能，揭示当前技能生成本质上是一次有损压缩。跨平台能力差异暴露：不同 Agent 框架在\"从经验中提炼技能\"上表现差异显著，框架本身是变量。SkillEvolBench 针对「LLM Agent 在解题中积累了丰富的情节式轨迹（episodic trajectories），但这些经验能否被蒸馏为可复用的程序技能」这一未明问题，提出一个诊断性基准，评测从「经验复用」到「技能形成」这一步。它包含 180 个任务，横跨六个真实世界 Agent 环境，组织为共享潜在程序（shared latent procedures）的角色条件任务族（role-conditioned task families）。Agent 先从获取任务（acquisition tasks）中学习，使用压缩后的轨迹与验证器反馈更新一个外部技能库，随后面对冻结的部署任务（frozen deployment tasks），后者测试上下文迁移、对抗性捷径与组合能力。通过将自生成与精选起点（curated-start）的技能演化，与「无技能」和「原始轨迹」两种对照相比较，SkillEvolBench 得以把程序抽象（procedural abstraction）从基础能力、精选先验知识、以及对情节轨迹的直接复用中分离出来。评测横跨十个模型配置与三个 Agent 脚手架。此外作者还做了容量与成本分析，考察写入更多技能或更大 Tier-3 资源库的效果。",
  ...
  },
  {
    ...
    "methodCore": "Reward-Evaluation Skill：二元组架构。核心形式为 $(M_{RM}, U_{RM})$ 二元组。$M_{RM}$：技能说明书，显式规定评估涉及的维度、资源调用协议、证据收集格式规范、以及输出约定（最终结构化判决由证据记录集 E 和结论字段 d 组成）。$U_{RM}$：资源库，将评估所需外部材料组织为五类可加载资源：Rubric & Criterion（评估维度和优先级权重）、Reference（答案标准和官方解法）、Checklist & Constraint（指令遵循逐项检查条件）、Verifier & Tool（可执行确定性工具：Python 沙箱、代码检查器、精确匹配器）、Calibration & Aggregation Rule（证据冲突解决和分数映射规则）。四步评估流程：诊断、选取、验证、汇总。诊断阶段分析 (query, response) 对需要激活哪些评估维度：数学题激活参考答案和代码执行验证器，开放式写作只激活 rubric 和格式 checklist。选取阶段从资源库按类型标签和适用范围抽取对应资源加载。验证阶段逐条调用被激活的资源，每个验证步骤产出结构化证据记录 $e_m = (c_m, q_m, s_m)$。汇总阶段将所有维度证据记录按预定义 Aggregation Rule 合成为结论字段 d，再映射为最终奖励分。在能力层面，结构化证据记录让奖励分可追溯、可审计：每分背后都可翻出对应的被调用资源和局部判定。五类可加载资源在技能控制下按需调用，不在当前评估范围的资源不进入上下文。评估流程四步标准化：诊断→选取→验证→汇总，执行顺序和冲突解决规则由技能规格显式控制。在 best-of-N 选择和 RFT 等下游应用中，Skill-RM 作为训练信号训练出的策略优于传统 Judge 对照组。Skill-RM 针对当前奖励评估依赖异构标准——基于规则的验证器、真值参考、程序化清单（procedural checklists）与复杂 rubric——却缺乏统一机制整合所有类型证据的问题，提出一个统一框架，将奖励建模重新表述为执行一项可复用的「奖励-评估技能」（Reward-Evaluation Skill）。通过把奖励计算当作一个结构化的智能体任务（structured agentic task），Skill-RM 提供了一致的接口来编排（orchestrate）异构资源，针对每个输入的具体需求动态选择与聚合证据。这一做法使奖励模型超越静态评估，在多样任务间确保一致性与透明性。作者在奖励基准与下游应用上开展了大量实验，下游应用包括 best-of-N 选择与强化学习。结果显示 Skill-RM 一致优于传统的 judge 基线，作者据此认为其价值不仅在于为奖励建模提供统一方案，还在于通过对证据的策略性、动态化编排获得更优性能。代码开源于 GitHub（Qwen-Applications/Skill-RM）。",
  ...
  },
  {
    ...
    "methodCore": "故障归因：rubric 生成的三个系统性失败模式。Qwen3-8B 裸评准确率 57.04%，加上自生成 rubric 后跌到 50.63%（-6.4pp）。用 RIFT 分类法对 200 个失败案例归因发现三类模式：缺失判别标准（34.2%）：rubric 从查询文本生成，无法感知候选回答之间的实际差异；错位或僵化（24.1%）：rubric 过度指定表面要求，将 Judge 引向无区分度的维度；criterion-list 格式本身表达力瓶颈：无法表达条件化决策逻辑。根源一致：rubric 是从查询推断而非从实际候选回答差异分布中归纳。Workflow + Principles 双组件技能结构。将评估标准从\"每个查询一次\"改为\"每个领域一次\"。Workflow：告诉 Judge 怎么做评估（先看什么、后看什么、维度不相上下时如何分岔、优先级和排名处理），不是一列\"检查要点\"而是一套\"操作流程\"。Principles：列出领域内反复出现的关键判别维度，每条从实际候选回答对的差异中提炼而非从查询推断。Workflow 和 Principles 交叉迭代进化：Workflow 多分支搜索不同评估路径结构在进化集上评分选优，Principles 从最佳 Workflow 路径 rollout 中提取并再筛选。离线一次性计算 + 推理时零额外开销。整个进化过程离线一次性完成，产出的 .md 技能文件本地缓存。推理时技能直接注入 Judge 的 prompt 上下文，每轮评估不需要额外 rubric 生成或额外模型调用。进化时间缩放行为显示增加进化样本数（50→100→200）技能质量持续提升，曲线未触顶：离线投入越多、推理收益越大，且推理成本完全不变。技能编码\"对一个领域的好评估标准\"而非\"某个模型的偏好\"，天然跨底座可迁移。在能力层面，领域级离线进化：100 个样本离线搜索和筛选，产出可复用的 Workflow+Principles 评估技能。Workflow（操作流程）替代 criterion-list（检查要点列表），支持条件化分岔决策逻辑。跨底座可迁移：一个模型进化出的技能注入另一个模型的推理上下文，效果优于目标模型裸评。下游 best-of-N 推理中 Eval-Skill 引导的选择在胜率、平局减少和质量一致性上均优于基线。进化时间缩放曲线未触顶，离线计算投入与在线收益呈持续正相关。Eval-Skill 针对开放式奖励建模需要能遵循细微、领域特定偏好的 judge，而现有 rubric 方法为每个查询在线生成标准、带来额外推理开销并可能产生僵硬或错位引导的问题，提出一种探索引导（exploration-guided）方法：合成可复用的评估技能（evaluation skills）用于奖励建模，并把奖励引导重构为上下文演化（context evolution），而非参数训练或逐查询 rubric 生成。它每个领域仅用 100 个案例进行技能演化，通过两个渐进阶段合成可复用的领域级评估技能——先工作流生成（workflow generation），后原则生成（principle generation），并在两个阶段间交错进行探索与选择（exploration and selection interleaved）。一旦生成，技能被直接注入 judge 上下文。作者在多个奖励模型基准上评测，显示 Eval-Skill 一致改进多样的 judge 底座。作者进一步分析了演化时扩展（evolution-time scaling）、泛化性与可迁移性，论证紧凑的评估技能是一种高效的 LLM 评估新范式。代码开源于 GitHub。",
  ...
  },
  {
    ...
    "methodCore": "SARS 五维度 Agentic 风险评分。SARS 定义五个独立维度，每维 0-3 分，由 LLM-as-Judge 评定。IFR（指令保真风险，权重 2×）：技能指令是否可能被 Agent 误执行或绕过；DG（数据引力，1.5×）：技能接触的敏感数据广度；AI（动作不可逆性，1.5×）：技能操作是否不可撤销；BR（爆炸半径，2×）：危害扩散范围；CA（链式放大，2×）：多智能体/多技能协作中风险的级联放大效应。LLM-as-Judge 审查流程 + 多框架集成。默认 Judge 为 Qwen2.5-14B-Instruct（T=0.2）。审查覆盖七类漏洞：命令/Shell 注入、RCE、不安全文件操作、提示注入、记忆投毒、数据暴露、供应链攻击和权限滥用。排行榜同时展示 SARS 评分、完整 CVSS v4.0 向量分解、以及 ClawHub 双视图（市场官方裁决 vs LLM 审查对比）。评估器敏感度分析中检测率从 35%（Mixtral-8x7B）到 95%（Qwen2.5-32B）不等，说明 Judge 选择是关键变量。指令层检测缺口量化。对 Prompt Injection（19 个技能），VirusTotal 检出 0，ClawScan 检出 3，LLM-as-Judge 检出全部 19 个。对 Memory Poisoning（9 个技能），ClawScan 检出 1，VirusTotal 检出 3，CodeBERT 检出 0：传统工具漏检 89-100% 的指令层威胁。CA（链式放大）维度在所有恶意类别中均 ≥1.80，即使这些技能的 CVSS 基础分偏低（Data Exposure 1.84，Supply Chain 2.30）。在能力层面，SARS 五维度加权风险评分填补代码扫描器对指令层和多智能体风险的盲区。受控 100 技能检测中零假阴性、零假阳性，超过最佳静态基线 15% 的漏检率。活的公共排行榜（1,299 个技能，2026.05 快照）：39.2% 标记为存在漏洞，3 个 Critical，54 个 High。量化了代码扫描工具对指令层威胁的 89-100% 漏检率。SkillVetBench 针对现有扫描器只在代码层运作、对指令层与多智能体风险结构性失明的缺口——自然语言指令可劫持 Agent、经编码侧信道窃取数据或在流水线间链式扩散危害——主张需要语义化、多维度的审查系统，而非又一个特征签名匹配器。它是一个部署在 Hugging Face 上的实时公开榜单（live public leaderboard），用 LLM-as-Judge 审查 Agent 技能。其新提出的核心是 SARS（Skill Agentic Risk Score，技能智能体风险评分）——一个面向指令遵循系统、带有原则化加权公式的五维智能体风险度量。系统整合了完整的 CVSS v4.0 向量分解，以及 ClawHub 双视图（dual-view），把 LLM 生成的评审结果与官方市场判定并置。作者借助配套基准论文，展示 LLM-as-Judge 阶段在 78 个已确认恶意技能上实现零假阴性、在 22 个良性对照上实现零假阳性；同时最佳静态基线 SKILLSIEVE 仍漏检 15%。对提示注入、记忆投毒等指令层类别，传统工具漏检 89%–100%（例如 CODEBERT 检测不出九个记忆投毒技能中的任何一个）。四个 LLM 评估器间检测率介于 35%–95%，促使生产部署采用集成评分（ensemble scoring）。",
  ...
  },
  {
    ...
    "methodCore": "技能为中心的任务自动生成。LLM读SKILL.md提取核心能力，每个技能自动生成三个有代表性的使用场景。控制要点：任务随技能内容动态生成，固定基准的预设任务集做不到这一点。任务描述不透露操作步骤和评分维度，防止Agent猜答案。基线对比与配对任务设计。每个效用场景编译成两个任务——带技能和不带技能——共享相同指令、输入、环境和评分标准，唯一区别是技能是否可用。引入pass-rate gain（PRG）指标算净增益，把backbone强度的影响剥掉。控制要点：发现PRG与no-skill基线pass rate之间r=-0.90，强模型基线高则技能边际贡献被压缩。两阶段安全风险检测。静态扫描：覆盖21种风险模式、5大类（prompt注入、凭据访问、不安全文件/网络行为、隐藏指令、供应链），高/中严重级别召回率97.0%/96.0%。动态探针：对发现的每个风险生成探针，沙箱里实际执行并采集Agent轨迹和文件变更。控制要点：区分风险的存在性与可利用性——不同骨架动态触发率差异显著（Claude 23% vs GPT-5.4 53%）。审计报告与发现时展示。所有任务在隔离沙箱运行，内置日志抓每一步工具调用、文件变更和网络请求。浏览器扩展把审计报告嵌进技能发现页面，评估从离线研究变成部署决策工具。在能力层面，任意技能即输即评：不限制来源或类别，有SKILL.md就能生成定制审计。基线剥离的效用归因：PRG算差值减基线，骨架强度不再污染技能效用。静态+动态双层安全检测：先扫存在性再测可利用性，不同骨架动态触发率差异显著。adoption-time决策辅助：浏览器扩展把审计嵌入发现页面，开发者下载前就能看到评级。多维度统一报告：效用、效率/成本、安全三维聚合，每项判断追溯到具体执行证据。SkillAudit 针对技能生态缺乏可靠方式判断「一个技能是否值得部署」的问题，指出现有评估仍主要锚定于固定任务套件（fixed task suites），这会把技能的边际贡献与底座强度混为一谈，并在任务落在技能预期范围之外时错判其价值。它是一个以技能为中心（skill-centered）的端到端框架，接收任意 Agent 技能作为输入，自动生成一份涵盖效用（utility）、效率/成本（efficiency/cost）与安全（safety）的多维评估报告。SkillAudit 聚焦技能产物本身，直接从技能包构建能力对齐（capability-aligned）的评估任务，任务在隔离沙盒环境中执行以收集执行证据，随后经自动检查与基于 LLM 的判定产出可审计结果。为解剖技能，作者提出「基线对比原则」（baseline comparison principle）来度量效用与效率/成本——即剥离 backbone 影响；并引入结合静态语义分析（static semantic analysis）与动态运行时验证（dynamic runtime verification）的两阶段检测范式来评估安全风险。作者扫描了跨 23 个职业类别的头部真实技能包。",
  ...
  },
  {
    ...
    "methodCore": "技能行为约束（SBC）提取。从自然语言技能文档中提取EARS风格归一化的条件-行为规则。每条SBC需满足四个条件：有明确源文本支撑。施加义务/禁止/工作流/交付标准。能从执行证据判断。表达单一独立行为约束。LLM提取候选后人类审计员审核，剔除背景介绍、元数据等非约束内容。控制要点：SBC把技能里可测试的部分投影到一个有限、可审计的分母上。覆盖判定：两道门。一条SBC标记为Covered需同时满足：执行轨迹实例化了SBC的适用条件（applicable），且轨迹中有足够可观测证据让评判者判断约束是被满足还是被违反（evidence）。控制要点：Covered不要求Agent满足约束——违反了也算覆盖，只要证据充分。验证后扩展的评判协议。DeepSeek V4 Flash作覆盖评判器。先在Claude Code+Opus 4.6轨迹集上采样350组约束-轨迹对做人类标注验证，一致率93.71%，Cohen's $κ = 0.874$。验证通过后冻结评判prompt和证据格式扩展到全量SkillsBench。在能力层面，技能测试充分性量化：把评估从\"好不好用\"推进到\"测到了多少\"，给出独立于任务成功的第二维信号。SBC可溯源分母：每条约束附带源文本位置，未覆盖部分可精确定位到技能的具体段落。证据驱动覆盖判定：applicable加evidence双门，约束必须被实例化且可判断，不是技能被加载就算覆盖。评判协议经人类验证：93.71%一致率保障大规模自动化的可信基础。Skill Coverage 针对「任务结果无法揭示可复用技能的哪些部分被执行、以及执行时 Agent 是否遵循了相关技能指令」的盲区，提出一个基于轨迹（trajectory-based）的技能测试充分性度量（test-adequacy metric）。其框架从每个技能中抽取技能行为约束（skill behavior constraints），把自然语言技能指令翻译为半结构化约束（semi-structured constraints），这些约束指定了在特定条件下 Agent 的预期行为；随后判定每条约束是否被某个 Agent 轨迹覆盖，并对被覆盖的约束按 Agent 实际行为给出 Pass 或 Fail 判定。作者将该框架应用于 SkillsBench：结果显示排行榜上的 Agent 轨迹平均只覆盖了所抽取技能行为约束的 38.66% 至 45.51%。随后作者利用 Fail 判定来强化对应技能内容——仅通过强调 Agent 未遵循的原始指令——并用强化后的技能重跑相同任务，在五个 Agent-模型行上带来失败任务的平均 16.0% 恢复率。这表明技能覆盖率既是测试充分性度量，也是观察技能使用行为的细粒度信号，失败约束标签为改进技能有效性提供可行动证据。",
  ...
  },
  {
    ...
    "methodCore": "SkillForge 采用五阶段流水线，围绕\"完全扎根业务\"和\"全程无需人工介入\"两个核心原则：初始化：从历史工单中挖掘专家工作流、高频内部工具和领域专属知识，填充到预定义模板生成初始 Skills，全程使用内部大模型处理私有数据。执行与监控：Agent 加载 Skills 处理业务请求，LLM 评判器对比输出与专家参考回复，低于阈值则标记为不良案例。故障分析：从知识、工具、澄清、风格四个维度并行诊断不良案例，生成结构化故障记录。聚合：按类别聚合故障，识别系统性问题并选取典型案例。诊断与优化：基于 ReAct 的诊断器将故障映射到 Skills 具体缺陷位置，优化器通过虚拟文件系统执行最小化修改，生成新版本 Skills。为适配企业安全，SkillForge 采用约束型 Skills 结构：剔除可执行脚本，所有操作通过预定义系统工具完成，Agent 通过虚拟文件系统（VFS）与 Skills 资源交互。在能力层面，领域情境化 Skills 生成：从历史工单中挖掘工作流、工具和领域知识，冷启动质量显著优于通用生成器。多维度故障分析：从知识、工具、澄清、风格四维度并行诊断，自动聚合识别系统性问题。端到端自进化闭环：从故障诊断到 Skills 自动优化全程无需人工介入。SkillForge 面向云技术支持这类企业场景,针对现有技能生成器缺乏领域接地、产出技能与真实任务不匹配,且部署后无法把执行失败回溯到技能缺陷的问题,提出一个自进化框架,闭合“创建-评估-优化”的端到端循环。为产出对齐良好的初始技能,其 Domain-Contextualized Skill Creator 将技能合成锚定在知识库与历史工单之上;为实现持续自优化,一条三阶段流水线——Failure Analyzer、Skill Diagnostician、Skill Optimizer——自动批量诊断执行失败、定位底层技能缺陷并改写技能以消除缺陷。该循环迭代运行,使技能随每一轮部署反馈自我改进。整体思路是把运营中不断累积的失败证据转化为对技能的定向修订,而非停留在一次性生成。作者强调这一自动进化过程可从多种起点出发持续提升技能质量。SkillForge 是业界首个端到端垂直领域 Agent Skills 自进化框架，扎根企业真实业务数据，打通从 Skills 生成、部署运行、多维故障分析到自动优化的完整闭环，全程无需人工介入。在边界层面，依赖企业真实业务数据，缺乏历史数据的场景冷启动效果受限。知识类问题受底层知识库覆盖范围限制，迭代后期提升趋于平缓。",
  ...
  },
  {
    ...
    "methodCore": "框架将技能抽象为 $S = (θ, φ)$，$θ$是结构配置（哪些文件、哪些章节），$φ$ 是内容实例（具体写了什么）。外层 MCTS 搜结构，内层 LLM 磨内容，两轮交替推进。技能的形式化定义：将技能拆为结构 $θ$ 和内容 $φ$ 两个独立变量，结构决定组件如何组织（章节增删、文件拆分合并），内容决定每个组件里写什么。两者解耦后才能各用各的工具来优化。外层 MCTS 结构搜索：用 UCB 策略在搜索树里选最有潜力的结构节点，LLM 根据历史性能数据生成结构修改建议（加/删/重排章节、移内容到不同文件），执行后把任务得分反向传播更新树。MCTS 天然适合这种改完结构才能知道好不好的延迟反馈场景。内层内容优化：收到新结构后分三步——对齐（把旧内容往新结构里搬，能复用就复用）、分类型优化（章节调整时重点磨文本，脚本改动时重点修代码跑测试）、保守选择（用下置信界 LCB 选最稳的版本，不选分数最高但可能只是运气好的）。为什么选 LCB 而不是最高分：测试有噪声，最高分可能只是运气好。论文数据显示最新候选的期望 $Δ$ 是 -3.1pp，而最佳候选是 +8.1pp。保守选择能大幅降低上线后性能回归的概率。在能力层面，自动搜索技能的最优结构配置——章节怎么排、内容放哪个文件、元数据怎么写——不再依赖人的经验和直觉。内容优化和结构优化联动：改了结构后内容自动适配，改了内容后结构同步调整，不会出现改了章节划分但文本还停留在旧布局的情况。保守选择策略用统计方法替代\"选最高分\"的直觉做法，减少因测试噪声导致的假优化，对生产环境更友好。一套固定流程覆盖技能优化的全链条：从结构草案到内容精炼到最终选优，不需要人工介入。Iterative Optimizer(迭代优化器)是 Agent Skills 生态中面向技能质量提升的工程范式。其核心是把技能改进组织成一个可重复运行的闭环:先在代表性任务上评测当前技能的表现,收集失败与低效证据;再据此对技能文档做针对性优化;随后在同一评测集上重新评测,验证改动是否真正带来提升而非引入回退。这一“评测-优化-再评测”的循环可多轮迭代,每一轮都以上一轮的量化结果作为下一轮修订的依据,从而让技能从一次性撰写走向持续打磨。相比凭直觉一次性写好技能,该范式把改进建立在可观测的反馈之上,使每次修订都有据可依、可被验证接受或拒绝,让任何技能只要有可复现评测与明确优化动作即可纳入循环持续演进。将 Skills 优化建模为双层优化问题——外层用 MCTS 搜索最优技能结构（章节组织、文件拆分），内层用 LLM 精炼具体内容，在 ORQA 基准上自动优化的技能反超了人工手调版本（90.625% → 93.75%）。",
    ...
    "limitations": "效果高度依赖评测集的代表性与覆盖度,评测偏窄会把技能过拟合到测试任务;循环需要反复评测,时间与算力成本随轮次累积。只在 ORQA（运筹学问答）单一基准上验证，任务类型偏选择题，其他领域的结构搜索空间可能差异很大。MCTS 搜索每轮评估都需要多次 rollout，计算开销不小，技能库规模大了以后搜索效率是个问题。保守选择依赖 LCB 的统计假设，小样本下可能过于保守，错过真实的大幅提升。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "SkillReducer 针对技能的双层结构特性设计了两阶段优化流程：阶段 1：路由层优化——最小充分的路由描述。采用\"快速压缩 + 实体验证\"的两阶段设计：模拟预言的 Delta 调试：构建模拟路由 Oracle 接收测试查询集与候选技能池（含干扰技能和对抗性影子技能），将原始描述拆分为独立语义子句后，使用 DDMIN 算法递归分割与测试，丢弃非必要子句，得到 1-最小集合——每个保留子句均为正确路由不可或缺。真实环境验证：在实际 Agent 中验证压缩后描述，若存在触发失败则从已删除子句中贪婪恢复最能提升触发率的子句。对于描述缺失或过短的技能，从主体中提取核心能力、触发条件和唯一标识，自动生成合规描述。阶段 2：技能主体重构——渐进式披露。将单体架构技能转换为分层架构，核心内容常驻上下文，补充内容按需加载：分类驱动的内容拆分：对段落级内容自动标注为核心规则、背景、示例、模板、冗余五类，仅核心规则保留在常驻模块，冗余直接删除。分类型定制化压缩：核心规则做语义合并与精简，示例/模板按概念去重仅保留代表性内容，背景做精准摘要，同时完成跨文件去重。双重质量门 + 自校正反馈循环：Gate1 忠实性验证确保核心操作概念完整保留；Gate2 任务化功能评估通过代码执行断言 + LLM 评审双机制验证，失败时自动将必要内容提升至常驻核心，最多 2 轮迭代。在能力层面，路由描述压缩：通过 Delta 调试实现最小充分描述，平均压缩率 48%。技能主体重构：分类驱动拆分 + 定制化压缩，平均压缩率 39%，单技能节省约 1000 token。功能保留验证：双重质量门确保 86% 技能压缩后表现不降低，部分技能反而提升。作者对 55,315 个公开技能做大规模实证研究,发现系统性低效:26.4% 完全缺失路由描述,超过 60% 的正文内容不可执行,参考文件单次调用可注入数以万计的 token。据此提出一个两阶段优化框架:阶段一优化路由层,压缩冗长描述并通过对抗式 delta debugging 生成缺失描述;阶段二重构技能正文,采用分类法驱动的分类与渐进式披露,把可执行核心规则与按需加载的补充内容分离,并以忠实性检查和自纠正反馈循环加以验证。整体思路是“少即是多”:剔除非必要内容既降低成本,又减少上下文中的干扰,从而在压缩体量的同时不损甚至提升功能质量。在边界层面，真实压缩失败率约 4.7%。技能过时导致的问题无法通过压缩解决。在 600 个技能与 SkillsBench 基准上评测,SkillReducer 实现 48% 的描述压缩与 39% 的正文压缩,同时功能质量提升 2.8%,揭示“少即是多”效应。这些收益可跨来自四个家族的五个模型迁移,平均保留率 0.965,并能推广到一个独立的 agent 框架。",
  ...
  },
  {
    ...
    "methodCore": "整个流程分白天和夜晚两个阶段。白天用户正常使用，系统在后台记录完整交互因果链；夜晚进化器分析轨迹、修正技能、跑验证，通过后部署给所有人。技能池只在验证通过的方向移动，不会退化。因果链追踪：不只是记对话内容，而是记录完整的\"用户提示 → Agent 动作（含工具调用）→ 中间反馈（工具结果、错误、用户回复）→ Agent 最终响应\"链路。大部分技能层面的失败是过程性的——参数格式错了、少了一步校验、调用顺序反了——这些问题不会出现在最终响应里，只有顺着动作-反馈链往回追溯才能定位到具体哪一步出错。按技能分组 + $G(∅)$挖掘：每条轨迹按引用了哪些技能分组，没引用任何技能的进$G(∅)$ 。同一个技能被不同用户在不同环境下调用，有的成功有的失败，技能本身就成了可控变量，不需要刻意设计对照实验，跨用户数据自然暴露了技能在什么条件下好使、什么条件下会崩。$G(∅)$用于识别反复出现但没被任何技能覆盖的操作模式。进化器——AI 进化 AI：进化器是 LLM Agent，不是手写规则。同时看成功和失败记录——成功定义\"不变量\"（绝对不能动的已验证有效部分），失败定义\"靶点\"（需要修正的行为）。三种动作：Refine（根据失败模式修正现有技能）、Create（从 $G(∅)$ 中创建新技能）、Skip（证据不够不动）。保守编辑原则：能改一行不改一段，不动 API 端口/输出路径/payload 格式除非多条会话明确证明信息已变，区分\"技能有问题\"和\"Agent 好好用技能\"。夜间验证 + 单调部署：所有候选更新在真实环境里用同样工具链跑新旧对比，验证成功率。只有确实更好的才部署，第二天所有人自动拿到升级。技能池不会退化。在能力层面，跨用户技能进化：每个人的踩坑经验自动贡献给整个生态，你踩过的坑后面的人不用再踩。因果链追踪让过程性失败可定位——不是只知道\"任务失败了\"，而是知道在哪个具体步骤、因为什么原因失败。保守编辑 + 单调部署确保技能池只会变好不会变差——验证通过的才合并，被拒的不部署。进化器同时看成功和失败，实现累积式进化——每次只修正识别出的缺陷，保留已验证有效的不动，避免修一个 bug 搞坏另一个流程。SkillClaw 针对 OpenClaw 类 LLM 智能体依赖可复用技能、但技能部署后基本保持静态的问题:相似的工作流、工具使用模式与失败模式在不同用户间被反复重新发现,系统无法随经验改进。不同用户的交互提供了关于某技能何时奏效或失败的互补信号,但现有系统缺乏把这类异构经验转化为可靠技能更新的机制。为此,SkillClaw 提出面向多用户智能体生态的集体技能进化框架,把跨用户与跨时间的交互作为改进技能的主要信号。它持续聚合使用中产生的轨迹,并交由一个自主进化器(autonomous evolver)处理,后者识别反复出现的行为模式,并将其转化为对技能集的更新——或精炼既有技能,或以新能力扩展技能。所得技能维护在共享仓库中并在用户间同步,使在某一情境中发现的改进能在系统范围内传播,且无需用户额外投入。",
  ...
  },
  {
    ...
    "methodCore": "EmbodiSkill 针对具身场景中单主体自执行、自复盘、自修改的闭环痛点，核心解决\"技能缺陷\"与\"执行失误\"的区分问题：双层技能结构（Skill Body + Appendix）：将单一技能拆分为技能体（核心正文规则，存储标准正确的任务执行逻辑）和附录（动态提醒事项，记录执行偏差和操作疏漏），不改动主干规则，彻底解决误修改问题。四类精细化反思（Skill-Aware Reflection）：将所有任务轨迹分为发现（新能力识别）、优化（已有技能改进）、技能缺陷（说明书写错）和执行失误（说明书正确但未照做）四类。前三类针对性修订技能体，执行失误严格保留原有技能体，仅在附录追加操作提醒并合并去重。三段式闭环修订流程：第一步聚合多轮反思信号，对发现、优化、缺陷类信息去重消冲突；第二步仅针对信号指向的问题点位局部修订技能体，不全局重写；第三步利用执行失误信号更新附录提醒，形成差异化记录。关键设计。不修改模型参数，全程无训练、外部化迭代。核心洞察：执行失误不应触发技能修改，否则正确内容被错误覆盖导致越迭代越退化。技能感知式靶向进化效果远高于粗粒度全局更新。在能力层面，具身场景突破：ALFWorld 任务成功率 93.28%，较 GPT-5.2 直接执行领先 31.58 个百分点，Puttwo 多物体子任务达到 100% 满分。跨基准验证：EmbodiedBench 上 EB-Habitat 52.33%，EB-Navigation 61.33%。快速收敛：10 轮修订中快速上升并稳定在高位，远优于无感知进化的波动和低收敛。现有技能自进化方法多在数字环境中开发,常把轨迹转为粗粒度技能更新;直接搬到具身场景有问题,因为一次失败可能不仅反映技能内容错误,也可能是执行失误——智能体未遵循本已有效的指导。为此提出一个免训练框架,通过技能感知反思与定向修订实现具身技能自进化。summary 提到的“双层技能结构”在 abstract 中未明确表述,归类/映射可能有偏差,此处依据 abstract 以“技能感知反思+定向修订”为准。在边界层面，仅验证了具身智能场景（ALFWorld、EmbodiedBench），CLI 数字工具场景未覆盖。依赖较强的反思模型（GPT-5.2、Gemini-3-flash 作为进化模型），弱模型反思质量受限。附录持续增长可能导致上下文过载，缺乏自动裁剪机制。在 ALFWorld 与 EmbodiedBench 上评测,EmbodiSkill 一致提升具身任务成功率。在 ALFWorld 上,它使冻结的 Qwen3.5-27B 执行器达到 93.28% 任务成功率,比作为直接智能体、不带技能的 GPT-5.2 高出 31.58%,表明技能感知的自进化能帮助具身智能体从自身轨迹中积累可复用的程序性知识。",
  ...
  },
  {
    ...
    "methodCore": "AutoSkill 由技能增强响应生成与技能进化两个耦合循环构成，辅以模块化组件与标准化流程，实现端到端终身学习：技能增强响应生成循环（左环）：查询重写模型将用户查询与对话历史转化为检索导向的独立查询；混合检索结合稠密语义相关性得分与词法 BM25 得分加权求和（Rel = λŷd + (1-λ)ŷb），筛选 Top-K 相关技能；技能条件响应生成将技能渲染为紧凑上下文注入对话模型提示词。技能进化循环（右环）：技能提取模型基于用户查询序列提取可复用技能候选，聚焦持久化约束和流程而非一次性请求；检索辅助技能管理将候选转化为检索查询匹配最相似已有技能，通过判断模型决策 add/merge/discard；版本化技能合并通过语义融合（非简单拼接）将新约束整合至已有技能，同步更新版本号。四阶段技能生命周期：经验获取（收集对话消息和行为轨迹）→ 技能提取（抽象生成可复用能力）→ 技能维护与版本化（add/merge/discard 决策优化技能库）→ 技能复用（检索注入生成过程）。关键设计。技能以标准化 SKILL.md 为载体，包含名称、描述、可执行指令提示、触发词集、标签集、示例集、版本 7 个核心字段。六大功能模块（查询重写、对话响应、技能提取、技能管理判断、技能合并、嵌入）均为提示词驱动。三大核心存储（用户专属技能库、共享技能库、向量缓存库）分层管理。模型无关性：作为插件层兼容现有 LLM，支持跨 Agent、用户、任务的技能共享与迁移。在能力层面，版本化自我进化：professional_text_rewrite 技能历经 34 轮迭代（v0.1.0→v0.1.34），编程与软件开发（482 个）和写作创作（363 个）是进化最活跃领域。跨会话能力积累：10,243 个对话（英文 GPT-3.5 子集）生成 631 个技能，热门标签 python（98 次）、javascript（38 次）、excel（36 次）。多端部署：Python SDK、Web UI、OpenAI 兼容反向代理三种接入方式。AutoSkill 面向实际 LLM 应用中用户反复表达稳定偏好与需求(如减少幻觉、遵循机构写作规范、避免过于技术化措辞)却很少被固化为可复用知识、导致智能体难以跨会话积累个性化能力的问题,提出一个经验驱动的终身学习框架。它使 LLM 智能体能自动从对话与交互轨迹中派生、维护并复用技能:从用户经验中抽象出技能,支持其持续自进化,并在不重训底层模型的情况下把相关技能动态注入未来请求。AutoSkill 被设计为与模型无关的插件层,兼容既有 LLM,并引入标准化的技能表示以便在智能体、用户与任务之间共享与迁移,从而把转瞬即逝的交互经验转化为显式、可复用、可组合的能力。summary 中“双循环耦合”“SKILL.md 版本化”“34 轮迭代”等具体表述在 abstract 中均无对应,归类/映射可能有误。",
  ...
  },
  {
    ...
    "methodCore": "有界编辑预算（Textual Learning Rate）：每次最多改 3-4 条规则。对应深度学习的\"学习率\"概念，每次迭代最多修改 L_t 条规则（默认 4 条），使用余弦调度（早期可多改、后期逐渐减量），防止一次性大幅修改导致的规则冲突和效果雪崩。消融实验证明：L_t=4 的限制编辑效果优于无限制重写 2-3 分——与神经网络学习率过大导致震荡同理。所有修改建议必须是通用规则，不能是针对具体 case 的硬编码。验证门禁（Validation Gate）：效果不涨直接丢弃修改。每次修改后的新版本技能必须在完全独立的验证集上跑一遍，只有得分严格高于上一版本才接受——持平也不更新，否则直接丢弃并将本次修改记录到拒绝编辑缓冲区。这条门禁机制保证了优化过程永远不会出现越改越差的情况，是 SkillOpt 效果稳定的核心法宝。拒绝编辑缓冲区（Rejected-Edit Buffer）：踩过的坑不踩第二次。所有被验证门禁打回的无效修改被记录到缓冲区，后续迭代时优化器会先检查哪些修改之前被证明无效，不再提出同样的建议。这加速了优化收敛速度，避免在同一个方向上反复尝试失败。跨纪元慢更新（Epoch-wise Slow/Meta Update）：沉淀长期有效规则。每迭代 2-3 轮后做一次慢更新：把多轮验证都持续有效的核心规则写到技能文档的受保护区域，这些规则不会被后续的 step 级小修改覆盖，保证核心能力不会因后续迭代退化。这是 SkillOpt 最关键的组件——消融实验中去掉 slow/meta update 后，SpreadsheetBench 从 77.5 暴跌至 55.0（-22.5 分）。四步闭环流程。Rollout（前向传播）：目标模型携带当前技能文档执行一批任务，收集成功/失败轨迹作为训练信号，模型参数冻结不动。Reflection（梯度计算）：优化器模型分析失败轨迹的共性问题，提炼改进方向，生成结构化编辑建议——两个模型分工（target model 执行，optimizer model 观察分析，同一级别的优化器也可恢复强优化器 56%-74% 的增益）。Edit（权重更新）：对技能文档做 add/delete/replace 三种结构化编辑。Validation Gating（检查点保存）：修改后在验证集评估，分数不涨则不接受。在能力层面，52 场景全 SOTA：6 个基准测试 × 7 个模型 × 3 种执行环境共 52 个组合全部最优或并列最优，GPT-5.5 平均提升 23.5 分。跨模型迁移：在 GPT-5.4 上优化的技能直接用于 GPT-5.4-mini 和 GPT-5.4-nano，仍保留 80% 以上的提升效果。跨执行环境迁移：在 Codex 环境下优化的表格处理技能直接放到 Claude Code 环境下，直接提升 59.7 分。部署零额外成本：训练成本一次性离线付出，最终输出的技能为纯文本 md 文件（中位数约 920 tokens），人类可读可审计可修改，插入系统提示词即用。低门槛落地：不需要改模型权重，优化成本仅为微调的几十分之一，小团队手动跑都能跑通。SkillOpt 主张:当前技能要么手工撰写、一次性生成,要么经松散受控的自我修订演化,都不像针对技能的深度学习优化器,也无法在反馈下可靠地超越起点。作者提出应把技能当作冻结智能体的外部状态来训练,沿用使权重空间优化可复现的同一套纪律。SkillOpt 是据作者所知首个系统化、可控的技能文本空间优化器:一个独立的优化器模型把打分后的 rollout 转为对单个技能文档的有界 add/delete/replace 编辑,且仅当编辑严格提升留出验证集分数时才被接受。文本学习率预算、被拒编辑缓冲区与逐 epoch 的慢/元更新使技能训练稳定,同时在部署时不增加任何推理期模型调用。整体把“技能文档”类比为可训练参数,用可复现的训练循环持续优化,并保证部署零额外推理成本。",
  ...
  },
  {
    ...
    "methodCore": "适应解耦现象的系统化诊断。SkillMAS 通过移植应激测试量化了适应解耦的代价：将最终版本技能库塞回初始 Agent 组织，成功率从 94.0% 崩至 68.7%（低于种子基线的 76.1%）；反之将最终 Agent 组织配初始技能库则崩至 50.0%。技能单独涨而组织不调，涨出来的技能不是在帮忙而是在添乱。效用学习：只给真正使用的技能记功劳。维护 Skill Utility（技能在某种任务上的成功率）和 Executor Utility（Agent 在某种任务上的成功率）两张效用表，仅从验证过的执行轨迹更新。关键过滤规则：检索不等于使用——只有执行轨迹中实际出现了技能标识符或匹配模式，才算\"被使用\"。效用表用 Monte Carlo 规则更新，更新率随证据累积自动衰减。有界技能进化 + 证据门控 MAS 重构。技能修复限定为四类手术级操作：加前置守卫、重排步骤、收紧检索范围、拆分过载技能，非重新生成。MAS 重构由结构化诊断工件驱动——通过分析 Agent 效用趋势、技能重叠度和当前修复效果来判断瓶颈在技能还是组织，每次只做一项变更（新增/删除/合并 Agent 或调整职责边界）。在能力层面，技能-组织协同进化：技能和 Agent 职责边界共享同一证据源、同步更新，消除适应解耦。精确信用分配：区分\"检索到\"和\"实际使用\"，只给真正起作用的技能记功劳，防止效用表被噪声污染。精确手术而非全量重写：技能修复限定四种有界操作，MAS 重构每次只做一项变更。结构自适应收敛：ALFWorld 收敛到多 Agent 结构，τ-Bench Retail 收敛到单 Agent 结构，系统根据领域特征自动收敛到最优组织形态。SkillMAS 指出 LLM 智能体系统被期望在部署后持续改进,但现有工作常把两个适应目标解耦:技能进化与多智能体系统(MAS)重构。这种分离会造成组织瓶颈、上下文压力与错误专业化。为此提出一个用于多智能体系统自适应专业化的非参数框架,把技能进化与 MAS 重构耦合起来。SkillMAS 使用 Utility Learning 从经验证的执行轨迹中分配信用(credit);使用有界的技能进化在不引发未经过滤的技能库膨胀的前提下精炼可复用流程;并在保留的失败与 Executor Utility 表明存在结构性不匹配时,进行证据门控的(evidence-gated)MAS 重构。整体思路是让技能层面的改进与组织结构层面的调整彼此感知、协同演化,从而澄清部署后专业化是如何被归因、更新与应用的。作为非参数框架,它不依赖对模型参数的更新。",
  ...
  },
  {
    ...
    "methodCore": "三层结构化技能包作为可优化\"参数\"。技能不再是平文本，而是三层结构：L1 元数据头（触发条件、技能名）、L2 SKILL.md 正文（始终加载的核心指引）、L3 资源文件（条件加载的详细过程和边缘案例）。优化时的关键子决策是\"新增内容放哪一层\"：放 L2 始终占上下文但保证被读到，放 L3 只在需要时加载但可能被遗漏。损失证据：对比成功轨迹 + 失败轨迹。每轮收集的不仅是通过/失败结果，还有完整轨迹。关键创新是识别\"本轮成功但上轮失败\"的对比成功轨迹——这类轨迹揭示了当前技能到底学会了什么、哪些修改应该保留。纯失败驱动优化里这部分信息被完全丢弃。动量代理：模式记忆 + 覆盖层。动量 Agent 维护两份状态：持久模式记忆累积跨轮反复出现的诊断模式（如\"分页处理三次出错\"，从模糊逐步精确化），当前覆盖层只反映本轮新出现的模式。两者共同输入给更新器，使优化不会每轮从零开始，而是带着\"什么根问题最应该改\"的累积证据。分层补丁手术。更新器不重写技能，而是按分层逻辑做手术级修改——核心流程改 L2，长过程和边缘案例改 L3，触发条件改 L1。这个分层的价值在于不让一次性的任务细节污染核心指引，也不让核心工作流被放错位置导致找不到。在能力层面，梯度下降式技能优化：将优化分解为参数、损失、梯度、动量四个组件，每轮迭代系统性地朝更好方向更新。对比诊断：同时利用失败轨迹（缺什么）和跨轮对比成功轨迹（什么要保留），比纯失败驱动多一个正面的信号维度。跨轮记忆累积：动量机制让反复出现的修正模式从模糊变得越来越精确，优化器不做\"每轮从零开始\"的短视决策。自适应层级路由：新增内容根据性质自动落入 L1/L2/L3 三层，防止上下文被无关内容撑爆。SkillGrad 面向技能可靠性问题:无论第三方下载还是自生成,技能常不可靠、不完整或过时,而现有技能进化方法多用启发式反思、缺乏显式优化表述。为此提出一个受梯度下降启发的技能优化框架,把技能包当作要以梯度下降方式优化的结构化参数:任务执行提供轨迹级的损失证据,自动诊断继而提供指示修正方向的基于文本的梯度(text-based gradients)。为在多轮迭代间稳定优化,一个动量智能体(momentum agent)把反复出现的诊断模式累积为持久的记忆覆盖层(memory overlay);最后由一个基于 LLM 的 patcher 执行参数更新,对技能包施加分层感知的编辑(layer-aware edits)。整体把损失、梯度、动量、参数更新四个概念逐一以文本机制模拟,使技能优化具有方向性、记忆性与信号可分解性,而非无结构的反思。",
  ...
  },
  {
    ...
    "methodCore": "对比技能信用分配（CSCA）：双流优势估计。对每个活跃任务类型，同一策略参数同时采样两组 rollout——一组带技能注入、一组不带。标准 GRPO 将所有 rollout 混在一起归一化，导致不带技能但靠自己成功的 rollout 被人为压低优势。CSCA 将优势分解为两个互补流：全局质量流保持标准归一化维持稳定性，对比修正流基于任务级对比信号 $\\hat{\\Delta}(x)$ 对不带技能的 rollout 施加单向向上修正——独立成功比靠技能成功更值得优化器关注。验证驱动的自适应课程。每隔 d 步在验证集上用\"有技能\"和\"无技能\"两种条件各跑一次，测量成功率差异，经 EMA 平滑和 Sigmoid 激活后联合控制三个课程参数：归属强度：内化差距越小修正力度越大；技能注入比例：从早期大部分带技能渐进到晚期仅保留少量探针预算；活跃技能集：对比信号持续为零则从活跃集中移除，单调收缩不恢复。三个参数全由同一验证对比信号驱动，无需人工调参。与 RESKILL 和 SKILL0 的定位差异。SKILL0：用课程门控决定何时撤技能但优化时混在一起归一化，好的课程控制但不是好的优化。RESKILL：在同组内对比新旧技能版本，SKILLC 对比的是有技能 vs 没技能更进一步：技能不只要一起进化，进化到最后应被策略吸收、内化、可以被扔掉。在能力层面，内化盲区的系统性解决：通过双流优势估计让梯度\"看到\"独立成功的价值，填补 SKILL0 的优化盲区。无技能推理下超越技能增强方法：WebShop 上 SKILLC 无技能分数超过 SkillRL 带技能分数。自适应课程：三个课程参数全由验证对比信号自动驱动，无需人工干预。信号质量感知：对比信号弱的简单任务（如 Pick）CSCA 增益不大，信号强的任务（Heat/Cool/Pick2）提升达 +10% 到 +33%。SkillC 面向长时序 agentic 强化学习:结构化技能提示能改善探索,技能增强 RL 在推理时保留外部技能,而技能内化 RL 在训练时撤除技能以实现自主表现。但现有内化方法只用技能有用性对比做课程控制,策略更新本身不变,无法区分“依赖技能的成功”与“自主的成功”。SkillC 基于对比式技能信用分配(Contrastive Skill Credit Assignment,CSCA),把这种对比转化为直接的内化学习信号:在同一次策略更新内,为活跃技能类型的任务采样配对的注入技能与无技能 rollout,并通过一个双流优势估计器(dual-stream advantage estimator)把二者的任务级对比注入优化,在保持全局排名的同时,施加偏向无技能成功的单侧修正。一个平滑的验证级信号进一步驱动对归因强度、rollout 分配与单调活跃集剪枝的自适应课程,使智能体学会“不靠技能也能成功”比“靠技能成功”更值得内化。",
  ...
  },
  {
    ...
    "methodCore": "难度感知路由：按任务困难度分桶，各自走不同优化路径。每个训练 step，Agent 先用标准配置（仅带任务特定技能）对每个任务跑 G 条轨迹，算出通过率 $p_i$ 后分三桶：困难任务（$p_i=0$，全零 reward）、中等任务（$ 0 < p_i < \\eta_t $）、简单任务（$ p_i > \\eta_t $）。门槛 $\\eta_t$ 是过去 W 个 step 的 batch 级通过率滑动均值，随 Agent 整体水平动态调整。三种损失互斥——一个任务只触发一条优化路径。特权蒸馏：打破全零 reward 的梯度死锁。困难任务全零 reward 导致 GRPO 组内归一化方差为零、优势信号消失、训练卡死。Skill0.5 换思路：给 Agent 额外加上通用技能（特权提示），在\"开卷\"条件下收集成功轨迹，然后用 token 级 JSD 散度让学生策略（无通用技能）模仿教师策略（有通用技能）的每一步推理。教师侧 stop-gradient 不动，学生单向对齐。通用技能被蒸馏进参数不是死记硬背，而是作为推理能力内化。反事实捷径诊断：防止 Agent 背下 ID 映射、OOD 直接崩盘。简单任务通过率高不一定是好事——Agent 可能完全没读技能，背下了\"任务指令→动作\"的直接映射。检测方法：对每个简单任务额外跑一组拆掉技能的轨迹，计算利用增益 = 有技能通过率 − 无技能通过率。增益小或为负说明 Agent 在走捷径。利用增益的滑动均值作为动态锚点，低于锚点的任务其 GRPO 优势被负调制分量拉低——在 batch 里被降权，不是因轨迹差，而是因没真正用技能。在能力层面，通用/特定技能解耦训练：通用技能内化进模型参数（管\"怎么思考\"），任务特定技能外挂检索注入（管\"这次做什么\"），打破全外化 prompt 爆炸和全内化新旧技能冲突的死局。OOD 泛化突破：ALFWorld OOD 超 SkillRL 13.2 个百分点，WebShop 上 OOD 反超 ID——这是在逼 Agent 走\"读技能→理解→执行\"这条更难但更可泛化的路。反捷径机制：通过反事实对照实验检测技能利用增益，防止 Agent 背诵 ID 捷径而在 OOD 崩盘。零梯度冷启动解决：特权蒸馏用教师成功轨迹打破全零 reward 梯度死锁，比靠随机探索硬扛冷启动快得多。Skill0.5 面向 agentic 强化学习:为 LLM 装备显式技能是让自主智能体解决复杂任务的有前景范式,技能可分为用于广泛认知迁移的通用技能(general skills)与用于动态执行的任务特定技能(task-specific skills)。但现有基于技能的 RL 通常在“全外化”(带来高昂上下文开销)与“全内化”(易过拟合与知识冲突)之间被迫二选一。为破解此困局,Skill0.5 显式区分技能处理方式,将通用技能内化与任务特定技能利用相结合。它由一个动态的、难度感知的路由器(difficulty-aware router)驱动,把任务分流到不同的掌握层级以施加定制的优化策略:对困难任务,通过特权蒸馏(privileged distillation)内化通用技能以构建认知基础;对简单任务,通过诊断式探测(diagnostic probing)惩罚捷径并强制任务特定技能的利用。整体在内化打底与外挂调用间取得平衡。",
  ...
  },
  {
    ...
    "methodCore": "问题诊断：技能缩短 ≠ 成本降低，可能适得其反。实验给了一个反直觉的数据点：均匀压缩缩短了技能文档，但下游 Agent token 使用量增至 1.14 倍原基线。 技能的有效性依赖稀疏分布的操作锚点：API 构造器、命令行标志、验证阈值、文件格式约定、恢复规则。删除周围解释可以减少技能自身的 token 成本，但误删了锚点会引发下游探索、调试、工具重试和恢复操作，总成本反而更高。技能重写的本质问题不是\"怎么更短\"，而是\"什么东西必须留下\"。受控评估框架：解耦技能、任务、环境和验证器。SkillEE 构建了一个四组件解耦框架：(1) 技能结构分析器提取锚点类型和分布；(2) 多种信息保持重写策略（API/代码锚定、工作流守卫、规则/公式锚定等）；(3) 固定任务指令和环境保证可复现比较；(4) 独立验证器产出去耦的质量和成本信号。基于 SkillsBench 的实验揭示不同策略对不同任务族有显著性差异：实现密集型任务受益于 API/代码锚定，验证密集型任务需要工作流守卫，规则驱动任务依赖公式/阈值锚定。不存在普适的最优重写模板。策略学习：任务-技能条件化的锚点选择器。基于诊断发现，SkillEE 训练一个任务-技能条件化选择器决定每对 (task, skill) 应用哪种保持策略。该策略在主要 held-out 评估中减少总执行成本 7.0%。跨模型迁移实验中，冻结的策略在不同 Agent 栈上部署，减少总成本 14.7%、下游 token 成本 13.7%，同时验证器质量保持或略微提升。这说明学到的是\"任务需要什么锚点\"的知识，而非特定模型的行为模式。在能力层面，将技能重写从文本压缩切换到成本感知的操作知识工程视角，点出均匀压缩的陷阱。定义经济指标体系（技能 token 成本、下游 Agent token 成本、总执行成本、执行超支），超越仅关注技能长度的分析。多策略对比揭示不同任务族需要不同的锚定策略，不存在普适模板。任务-技能条件化的选择策略在跨模型迁移中表现更强（14.7%/13.7%），表明学到的是一般任务知识。SkillEE 从经济学视角研究技能重写:LLM 智能体日益依赖技能(编码工作流、工具使用、实现模式、校验检查与领域规则的可复用程序性文档)。技能重写常被当作提示压缩,但更短的技能可能因移除防止盲目探索、调试与恢复的稀疏操作锚点(operational anchors),反而让智能体更昂贵。其受控框架先剖析技能结构,用信息保留策略重写技能,并在固定的任务指令、环境与验证器下评估重写结果。实验揭示不同策略间存在明显的质量-成本权衡:API/代码锚定、工作流护栏、规则/公式锚定分别惠及不同任务族,不存在普适最优模板。作者据此主张把技能设计视为成本感知的操作知识工程,而非提示压缩,强调保留而非一味删减关键操作锚点。",
  ...
  },
  {
    ...
    "methodCore": "质量影响维度：技能整体是否有正向贡献？LLM 裁判同时比对带技能和不带技能的两个输出（顺序随机化），先判断方向（更好/更差）再估计幅度。轻微退化说明大方向对、局部需修；灾难性退化说明技能可能包含根本性错误的指导。该维度是外环信号：告诉你\"有没有问题\"，后面三个维度告诉你\"问题在哪\"。触发精度维度：技能是否在正确的时机激活？从技能描述中自动抽取正向触发短语和负向排除短语，通过依赖解析和否定追踪实现。关键在语境化嵌入：同一词在不同句式含义完全不同。从正负短语的嵌入分布计算三个几何指标：覆盖广度（正向触发是否太窄）、负向特异性（排除区域是否与正向清晰分离）、边界锐度（最差情况下的正负混淆程度）。指令合规度与故障归因维度：是技能写得烂还是 Agent 没照做？先从技能文档自动提取可评估规则集，每条带严重程度权重。然后对每条规则同时评四个信号：是否适用当前任务、Agent 遵循得如何、规则本身是否精确可操作、故障该归到技能还是 Agent。故障归因是核心分叉：规则为\"使用黄色背景（#FFFF00）\"而 Agent 用了 #FFF2CC，是 Agent 没照做；规则仅为\"使用黄色背景\"而 Agent 用了合法黄色，是技能不够精确。同一种行为，归因不同，改进动作完全相反。解法路径覆盖维度：技能是否支持多种合法解法？LLM 先枚举所有合法解法路径，再计算技能内容与每条路径的嵌入对齐程度。分数低说明技能只支持很窄的行为空间：Agent 若选了一条技能完全未覆盖的合法路径，技能等于不存在。在能力层面，四维度无监督诊断：质量影响（有没有用）、触发精度（时机对不对）、指令合规+故障归因（写得烂还是没照做）、解法覆盖（路径窄不窄）。故障归因区分技能缺陷与 Agent 执行偏差，避免误修正确规则。技能库自动积累：SpreadsheetBench 上仅用 22 个技能达到 LLM 自写 69 个技能的同等效果，技能数量减少 68%。无需人工标注、测试套件或环境奖励，仅需任务描述和两次 Agent 输出。SkillAxe 针对技能文档(引导 LLM 智能体的结构化自然语言指令)对现代智能体框架至关重要、但 LLM 难以写出真正奏效技能的问题:在 SkillsBench 上,人写技能使通过率提升 16.2 个百分点,而 LLM 撰写的技能没有可测量的增益。为此提出一个完全无监督的框架,使 LLM 能迭代地诊断并精炼自己的技能。SkillAxe 把技能质量分解为四个可解释维度——质量影响(quality impact)、触发精度(trigger precision)、带故障归因的指令遵从(instruction compliance with fault attribution)、解法路径覆盖(solution-path coverage)——产出结构化的改进简报(improvement briefs),无需任何真值标签、测试套件或环境奖励。整体形成一个不依赖外部监督的自改进引擎,让 LLM 依据这四维诊断持续修订自身技能。",
  ...
  },
  {
    ...
    "methodCore": "问题诊断：相关不等于必要，轨迹级信号信噪比太低。反事实分析揭示：启用技能访问仅在约 14% 的配对轨迹中提升了最终结果，78% 轨迹无明显变化，约 8% 轨迹调用技能反而让结果变差。技能价值极度集中，只在特定状态窗口有效。轨迹级成败标签对局部 invoke/skip 决策的归因分辨力不够。决策点级偏好：通过预测熵筛选高价值分叉，构造因果对比信号。记录每个 token 的 log-probability，计算每个技能决策点上的预测熵。高熵点表示模型不确定下一步，是最值得做 invoke/skip 对比的岔路口。在选定分叉点同时跑两条路径：一条强制调用技能，一条强制跳过，共享完全相同的前缀轨迹。两条路径的唯一差异是局部 invoke/skip 决策，效果差异可干净地因果归因。轨迹级偏好 + 局部窗口掩码：双粒度联合 DPO 优化。轨迹级偏好将成功轨迹与失败轨迹组成偏好对，提供全局行为序列约束。决策点级偏好提供局部 invoke/skip 因果信号。两种偏好合并进同一个 DPO 目标联合优化，并施加局部窗口掩码：损失仅在分叉点后 n 个 assistant turn 上计算，不让下游无关环境动态稀释梯度。在能力层面，熵引导的决策点筛选：用预测熵定位模型不确定的决策点，优先做 invoke/skip 对比，比全量分叉有显著增益。双粒度联合 DPO：轨迹级管整体效用，决策点级管局部必要性，联合训练同时提升调用质量和频率合理性。选择性调用策略跨领域零样本迁移：在 ALFWorld 上训练后直接部署到 Tau-bench 和 PopQA 有效。SelSkill 指出技能是为复杂 agentic 任务提供可复用知识与执行策略的可调用程序模块,但现有方法多关注选择相关技能或改进技能本身,忽略了“在当前决策点是否真的应当调用某个相关技能”。无益的调用会引入无关上下文、扰乱本已正确的执行过程。为此提出一个用于选择性技能调用(selective skill invocation)的双粒度偏好学习框架。SelSkill 把技能使用表述为“调用还是跳过”(skill-or-skip)的决策,利用预测不确定性(predictive uncertainty)来优先选取候选决策点,并从共享轨迹前缀构造受控的“调用-跳过”偏好对。它进一步把 episode 级的结果偏好与 step 级的调用偏好结合,以同时刻画整体轨迹质量与技能调用的局部有效性,从而把技能调用作为独立的优化目标来学习。SelSkill 将\"技能该不该调用\"从一个被忽略的隐式默认重新定义为一个独立的优化目标：通过双粒度偏好学习（轨迹级整体效用 + 决策点级 invoke/skip 因果对比），让 Agent 学会在不确定性低时跳过技能依赖自身推理，在不确定性高时精准调用技能。ALFWorld 成功率提升 +10.9 个百分点，选择性调用策略可跨领域零样本迁移。",
  ...
  },
  {
    ...
    "methodCore": "模型条件化搜索：以能力档案为显式约束。MASA 为每个目标模型维护一份能力档案（model card），包含架构元数据、训练来源和能力画像。教师模型在每次改写时以该档案为条件：不是\"教师觉得怎么写最好\"，而是\"教师判断对这个特定模型怎么写最可能有效\"。搜索在任务类型粒度上操作，同一模型内不同任务类型对同一组技能的反应差异可超 60 个百分点。分层技能进化：通用技能爬山 + 任务特定技能 UCB 树搜索。技能库分两层独立优化。Stage 1 用爬山搜索优化通用技能：教师分析失败轨迹中系统性问题，结合能力档案和历史高奖励技能改写，全任务评估后只接受高于当前分数的改写。Stage 2 固定通用技能，每个任务类型跑独立 UCB 树搜索优化任务特定技能，节点选择用 UCB1 平衡开采与探索。奖励含 NHR（nothing-happens rate）惩罚，抑制\"无害但无用\"的技能表达。技能改写器：将搜索压缩为一次前向推理。训练基于 Qwen3-4B 的轻量改写器，输入能力档案、技能和任务描述，一次前向传播输出适配技能。关键在训练数据的多样性暴露：输入来源覆盖进化管道各阶段中间产物、跨模型迁移结果、加噪截断变体，确保改写器学到\"不管输入多差都能改好\"而非仅对高质量输入有效。在能力层面，模型条件化技能适配：以能力档案为显式条件，让技能表达匹配目标模型的行为模式。NHR 惩罚机制：在奖励中扣除无环境反馈的\"空转\"步骤，引导搜索远离无用表达。轻量改写器（4B）在一次前向传播中完成适配，推理成本几乎可忽略，跨环境跨任务泛化。实验覆盖多种模型族（Qwen3、Gemma3）和三类环境，验证模型特异性技能的普遍需求。MASA(Model-Aware Skill Alignment)针对 LLM 智能体在决策时检索外部整理技能、而现有技能库通常被当作与模型无关、在能力差异很大的骨干间复用同一技能表述的问题。作者的受控实验跨多种模型规模显示技能有效性强烈依赖模型:惠及某骨干的技能可能损害另一骨干。为此 MASA 在不修改智能体权重的前提下把技能对齐到每个目标骨干,分两阶段:(1) 分层技能进化流水线,用爬山法(hill climbing)与 UCB 驱动的树搜索,在环境反馈与模型能力画像(model capability profiles)的引导下,迭代重写通用与任务特定技能;(2) 一个轻量的模型条件化技能改写器(model-conditioned skill rewriter),在进化轨迹上训练,以单次前向传播复现这种适配。整体打破“一套技能适配所有模型”的假设。",
  ...
  },
  {
    ...
    "methodCore": "归因层：从轨迹到具体步骤的技能责任链接。输入失败轨迹后：Localizer 直接预测最早的可操作故障步骤 t*，同时输出故障行为描述和改进建议，不简单找\"哪一步报了 error\"而是推理最早到哪一步时事情开始偏离正确路径。随后，Linker 将责任分配给候选技能，输出加权嫌疑技能集 ${(s_j, w_j)}$，同时做二元决策 $â$：REVISE 表示故障归因于某个技能内容不当、走定向修订；GENERATE 表示能力盲区、走新技能合成。修改层：只改目标技能，其余不动。若 $â=REVISE$，Modifier 取权重最高的嫌疑技能做定向修改后替换回技能集 K。若 $â=GENERATE$，Modifier 从故障步骤上下文中合成新技能追加到 K，配套语义去重检测控制膨胀。整个过程只动一个技能或新增一个技能，其他技能保持原样。审核层：改完必须验证才能入库。候选更新在正式并入前，系统用旧集合 K 和新集合 K+ 分别重跑任务集比较执行反馈。仅当 $∆≥0$（新集合不比旧集合差）时候选更新才被接受，否则丢弃。消融实验中拿掉 Qualifier 后标准差从 ±5.2 跳涨到 ±8.1，审核是稳定性的核心机制。在能力层面，步骤级故障归因：定位第一个可操作故障步骤，而非对整条轨迹做笼统反思。责任精确链接到具体技能，区分 REVISE（内容不当）vs GENERATE（能力盲区）。定向修改仅动目标技能，其余技能原样不变，改动小、稳定性高。资格审核门槛（∆≥0）压住有害更新，是稳定性的核心机制。免训练、可插拔接入 OpenClaw 类 Agent 框架。SkillAdaptor 面向 LLM 智能体日益依赖可复用外部技能解决长时序交互任务的场景。现有免训练技能适配流水线通常从完整轨迹或会话级反馈更新技能,使失败归因粗糙,常产生不稳定或过于宽泛的修订。为此提出一个带显式失败归因的免训练、步级(step-level)技能适配框架,可插入 OpenClaw 类智能体框架。给定一条失败轨迹,SkillAdaptor 识别第一个可操作的故障步骤(first actionable fault step),把责任关联到候选技能,并在显式的接受检查(acceptance checks)下施加定向更新,同时保持骨干冻结。整体形成“归因—修改—审核”的步级闭环,以更细的粒度定位故障、避免宽泛误改,实现更稳定、可审计的免训练技能维护。免训练、可插拔接入 Agent 框架，三个基准和三个底座模型上全面覆盖对比基线。",
  ...
  },
  {
    ...
    "methodCore": "客户端：轨迹不出门，只出语义技能 patch。每个客户端用当前技能库执行任务生成轨迹后，本地 patcher 将轨迹压缩和反思转换为技能 patch δ。patch 仅含三类语义操作：ADD（加新技能）、EDIT（改已有技能）、DELETE（删过时技能），不包含任何原始交互文本。patcher 被提示产出可跨任务泛化的经验，task-specific 的具体值和一次性输出不准写入 patch。原始轨迹、验证器输出、中间文件全部留在本地。传输单元是语义级 patch 而非模型权重，天然跨异质模型和框架可复用。服务器端：从 patch 推断用户能力边界，生成个性化更新。服务器每轮收到各客户端上传的 patch 和静态档案（基座模型、Agent 框架），从未见过任何原始轨迹。演化 Agent 将聚合建模为 POMDP：客户端真实任务环境是隐藏状态，通过每轮 patch 动态更新对各客户端能力边界的隐式模型：哪些任务频繁出 ADD 对应能力空白，哪些技能常被 EDIT 对应表述偏差，哪些正被 DELETE 对应过时操作。基于不断更新的理解，M 为每个客户端生成定向技能更新而非全局平均库。消融验证：个性化 vs 全局平均库差异显著。把个性化模块拆掉，用统一全局库替代每个用户的个性化库，平均性能下降 12.2 个百分点。在能力层面，隐私保护构造性约束：仅语义 skill patch 离开本地，原始轨迹、截图、表格、私人对话永不传输。语义级 patch 天然跨异质模型和框架可复用，比参数级联邦学习更灵活。服务器端通过 POMDP 推断用户能力边界，生成个性化而非平均化技能更新。协作进化同时提升成功率（最高 +44.4%）和效率（成本节省 37.5%）。FederatedSkill 面向现代 LLM 智能体日益依赖技能库、技能进化成为自我改进主要驱动的现状:孤立的单用户任务流缺乏构建全面技能所需的多样性;而跨用户协作虽能突破数据瓶颈,现有轨迹共享方法会损害用户隐私并强加一个无法适应客户异质性的统一全局库。为此提出一个隐私保护的协作智能体进化框架。它超越原始轨迹共享,以语义技能差分(semantic skill diffs,即对本地库的结构化 patch)作为通信的基本单元。在服务器端,一个进化智能体聚合这些 patch,以动态建模客户特定的能力边界,从而促成严格个性化的技能进化,而非次优的全局平均。整体把联邦学习的隐私保护思路引入技能进化,只传 patch 不传轨迹。summary 中“POMDP 个性化更新”在 abstract 中无对应表述,归类/映射可能有误,此处按 abstract 以“客户特定能力边界建模”为准。",
  ...
  },
  {
    ...
    "methodCore": "开放世界知识获取：检索执行知识和验证锚点两类信息。OpenSkill 主动搜索公开文档、代码仓库、论文等开放资源。第一类检索执行知识 k，包含背景概念、最佳实践、API 文档等，用于生成结构化技能计划。第二类检索验证知识 kv，即不透露答案但可独立验证的锚点，如官方文档中的输出格式、公开数据集的行数列名、领域标准中的交叉验证流程。所有检索查询经信息隔离审计，排除包含基准名称或可能导向答案的标识符。无泄漏技能进化：虚拟测试替代 ground-truth 反馈。核心挑战是在没有真实反馈的情况下判断技能好坏。OpenSkill 为每个任务生成一套虚拟 pytest 测试套件，每道断言锚定在验证知识上（检查输出行数匹配文档、列名正确、格式规范等），不检查可能泄露答案的中间计算结果。进化循环最多 3 轮：执行 → 跑虚拟测试 → 生成结构化故障诊断报告 → 诊断指出的知识缺口触发定向检索 → 技能原地编辑。全程评分仅来自虚拟测试，从未看过任务参考答案。零样本跨模型迁移：技能作为冻结产物。进化产物是冻结的技能文件（非模型权重），可零样本部署到任何 Agent。用 GPT 构建的技能直接部署到 Claude 上性能几乎无损失，反之亦然。虚拟 verifier 覆盖 88.9% 真实任务意图，来自公开知识中可验证锚点的系统性提取。在能力层面，开放世界自进化：从零构建学习回路：搜知识、建验证锚点、自测试自打磨，全程隔离参考答案。虚拟测试套件替代 ground-truth：基于公开知识中的行列数、格式、标准范围等可验证锚点生成 pytest。跨模型零样本迁移：技能作为冻结产物文件，构建和部署可使用不同的模型家族。信息隔离审计确保检索查询不泄露任务答案，虚拟 verifier 覆盖 88.9% 真实意图。OpenSkill 研究开放世界自进化:自进化智能体需在部署后适应,但现有方法都假定存在可用的学习循环,如整理好的技能、成功轨迹或验证器信号;而真实的开放世界部署可能一样都没有,只给出一个任务提示。作者定义开放世界自进化问题——智能体必须从零构建自己的技能与自己的验证信号,使用开放世界资源但没有目标任务的监督。OpenSkill 通过引导来 bootstrap 这一循环:从文档、代码仓库与网络获取接地知识与验证锚点(verification anchors),把它们合成为可迁移技能,并针对以这些锚点(而非目标答案)为基础的自建虚拟任务来精炼技能。开放世界由此同时提供待学习的知识与一个与监督无关的练习环境,目标任务监督仅保留用于最终评估。整体在无参考答案的前提下自建验证信号闭环。",
  ...
  },
  {
    ...
    "methodCore": "角色隔离双线架构：优化线与评估线分离。SkillHone 是架设在现有 Agent 运行时之上的 harness 层，用动态角色分派替代固定多 Agent 系统。优化线维护技能仓库，优化子 Agent 可检查当前技能、读取历史决策记录和失败摘要、形成诊断、提出修订并记录结果。评估线维护评估仓库（练习探针、验证套件、回归集、执行 trace 和评估报告），评估子 Agent 产生脱敏报告：探针的真实目标、验证器内部状态和完整执行轨迹被过滤，只有脱敏后的报告到达优化线。决策记录四元组：$h_t = (q_t, r_t, e_t, o_t)$。每次开发步骤被表示为一个结构化决策记录：$q_t$（诊断）是技能在这一轮暴露了什么失效模式；$r_t$（候选修订）是修改方案；$e_t$（脱敏评估证据）是评估后返回的反馈；$o_t$（结果）是接受、拒绝、要求进一步修改或推迟诊断。一份 diff 只告诉你文件怎么变，一份决策记录告诉这次改变是冲着什么问题去、用什么证据评估、最终为什么被接受或拒绝。决策历史的长期价值函数。进化早期历史短、决策记录检索和复用频率不高，SkillHone 相对基线优势不显著。当决策链积累到一定长度后，避免重复诊断和重复失败修改的收益才开始拉开差距。对于只需短期优化（三轮以内）的场景，决策历史边际价值有限：但在跨会话、跨环境变化、跨 Agent 接手的持续进化场景中是必要基础。在能力层面，持久化决策历史：诊断-修订-证据-结果的完整记录链，让技能进化从\"连续覆盖\"变成\"连续继承\"。动态角色分派：运行时每步根据状态决定角色，不绑定固定多 Agent 框架，不依赖特定 LLM。GAIA 上 64.6%（+15.8pp vs 深度搜索 Agent），WebWalkerQA-EN 上 66.4%，仅用公共 Web 访问。SkillHone 面向持续的技能进化:技能以任务特定的程序、脚本与参考扩展语言模型智能体,但其针对的任务与环境不断变化。现有方法只在有界的运行中改进技能、且只保留最终产物,丢弃了后续智能体解读先前修订、评估与被否决备选方案所需的决策历史。为此提出一个以持久决策历史为根基的持续技能进化框架(harness)。SkillHone 把技能修订与评估侧证据配对,后者提供练习反馈,记录诊断、修订、证据与结果的结构化历史。角色分离的子智能体(role-separated subagents)在练习探针上运行候选技能、进行去标识化(redacted)报告,并依据先前决策提出修订,从而在跨会话精炼时无需重新发现过往理据。整体让完整决策链而非仅最终产物得以继承。",
  ...
  },
  {
    ...
    "methodCore": "PACE（过程对齐对比评估）：12 个评估器 × 4 个维度的行为对比。配对轨迹审计的核心操作：同一任务分别在有技能和无技能条件下各执行一次。PACE 集群包含 12 个评估器，分布在四个维度上：Process Adherence：技能规定的步骤是否被遵循。Artifact Evidence：产出的文件/数据是否达标。Consistency：行为模式与技能意图是否一致。Effectiveness Delta：带技能比不带技能好了还是差了）。每个评估器输出锚定到具体技能段落的行动信号和受保护提示（protected hints），确保编辑有据可查。锚点验证器 + 双策略路由。锚点验证器从任务说明中编译一次后锁定（文件存在性、格式合规、可从工作区数据重新计算的值），提供硬结构约束。双策略路由根据技能问题类型分流：Refine 管道（减法优先）：处理大体有用但含噪声段落的技能，中位数 +19 行、均值 -260 行；Repair 管道（诊断驱动替换）：处理核心逻辑与任务冲突的技能，中位数 +16 行、均值 -165 行。三态裁决：单评估器否决即阻止有害更新。每条审计输出三种裁决之一：技能有帮助（commit）、技能有害（rollback）、技能中性（defer）。任何一个评估器输出\"有害\"报告即否决该次更新。在 89 个任务的进化中，92% 的已正常工作技能保持或提升，43% 的失败技能被拉回及格线，所有初始技能比无技能更差的 3 个任务全部恢复至奖励 1.0。在能力层面，零 ground-truth 自进化：不需要隐藏测试、参考答案、oracle 信号或环境奖励。PACE 配对轨迹审计从行为对比中提取编辑信号，锚定到具体技能段落。锚点验证器提供不参与进化的硬结构约束，防止技能漂移。跨 8 个专业领域的 89 个容器化任务验证，整体奖励 +33.0pp。该条 summary 与标题写作“无 ground-truth 配对轨迹审计自进化,PACE 12 评估器四维行为对比”,但据 arxiv abstract,SkillAudit 实为面向技能评估(assessment)而非自进化的框架,未见“PACE 12 评估器”“配对轨迹”等表述,归类/映射可能有误。abstract 显示:技能生态缺乏可靠判断某技能是否值得部署的方法,现有评估多锚定固定任务套件,会把技能的边际贡献与骨干强度混淆、并在任务超出技能预期范围时漏判其价值。SkillAudit 是以技能为中心评估的端到端框架:输入任意智能体技能,自动生成涵盖效用、效率/成本与安全的多维评估报告;它聚焦技能产物本身,直接从技能包构建能力对齐的评估任务,在隔离沙箱中执行以收集执行证据,再以基于 LLM 的判定做自动检查。它提出基线比较原则度量效用与效率/成本,并引入静态语义分析+动态运行时验证的两阶段检测评估安全风险。",
  ...
  },
  {
    ...
    "methodCore": "CCE（对比因果提取）：在成功/失败分叉点提取技能证据。现有方法从单条轨迹中提取技能容易引入偶然噪声。CCE 对同一任务通过多种子执行采样多条轨迹（最优 5 种子），构建同一任务的成功/失败对比对，提取点在因果分叉水线（causal watershed）：即两条轨迹开始出现结果分化的位置。从该点附近的决策差异中提取技能证据，而非概括整条轨迹。消融中去掉 CCE 后性能从 55.50 跌至 32.50。AAE（评估增强进化）：将 patch 作为可验证假设。每个候选技能 patch 被视为一个假设，需在源任务克隆上回放验证。校准评分赋予差异化权重：失败 → 成功=3.0（最佳），成功 → 失败=0.0（最差）。仅评分 ≥2.0 的 patch 通过阈值进入分层合并。消融中去掉 AAE 后性能跌至 26.00：低于无技能基线，说明未经验证的 patch 合并可能有害。Trace2Skill+Error 变体在 DocVQA 上 ANLS 跌至 0.6223（低于 No-Skill 基线），直接印证了这一风险。TTE（拓扑感知执行）：编译技能为可路由图以控制上下文膨胀。进化后的技能库可能比原始技能更庞大。TTE 将技能编译为一个有向拓扑图，用 LLM 图路由器在 top_k=7 的候选节点中选择，达到 41.6% 上下文缩减同时性能几乎不降。消融中去掉 TTE 后性能降至 46.50。跨模型验证中，gemma-4-31B-it 使用 SkillCAT 技能达到 61.00% Vrf（+22.00），gpt-5.4-mini 达到 37.00%（+6.00）。在能力层面，CCE 对比因果提取：在多轨迹分叉点提取技能证据，比单轨迹摘要更干净。AAE 回放验证合并：每个 patch 作为假设在源任务克隆上回放，校准评分做阈值过滤。TTE 拓扑感知执行：技能编译为可路由图，41.6% 上下文缩减，性能保持。跨任务（电子表格→WikiTableQuestions OOD +72.53pp）、跨模态（DocVQA ANLS +0.2316）、跨模型全部正向。SkillCAT 面向技能自进化把执行轨迹转为可复用技能文档的目标,指出现有流水线通常每任务只从一条轨迹学习、在检查前就合并候选技能 patch、且推理前加载完整技能语料。为此提出一个免训练框架,把过程分为三阶段。对比因果提取(Contrastive Causal Extraction,CCE)为每个任务采样多条轨迹,比较同任务的成功/失败对,识别解释结果差异的证据。评估增强进化(Assessment-Augmented Evolution,AAE)在源任务克隆上回放每个候选 patch,只保留能提升或保持任务结果的 patch,然后再做分层技能 patch 合并。拓扑感知任务执行(Topology-Aware Task Execution,TTE)把进化后的技能编译为可路由的子技能拓扑,使推理时只加载与任务相关的能力节点。整体在提取、验证、编译三环节分别解决单轨迹、先合并后检查与全量加载三大问题。",
  ...
  },
  {
    ...
    "methodCore": "Meta-Skill三层结构化编排知识。将Meta-agent的多智能体编排能力编码为一份三层自然语言文档：Task Decomposition：规定如何拆解用户查询为原子子任务。Agent Engineering：规定如何为每个子任务设计专门的子Agent（角色、指令、上下文）。Workflow Orchestration：规定如何选架构拓扑（顺序/路由/层次/黑板）并定义I/O映射。控制要点：初始版从Anthropic的MAS构建指南提取，自然语言写就，任何LLM都能读。换底层Meta-agent不丢编排经验。闭环进化：多轨迹采样+选择性反思。第一阶段Multi-Trajectory Rollout：对验证集每项任务用当前Meta-Skill独立采样K条轨迹，计算两个分布量——uncertainty（同任务分数标准差，反映指令模糊度）和difficulty（负平均分数）。第二阶段Selective Reflection：将两者归一化合成为优先级分数。二阶差分自动检测肘点，只选最易变最困难的顶部子集。先做任务内对比（高低分轨迹找分歧点），再做跨任务综合（提取系统性模式）。输出结构化证据包E。Skill Optimizer文档级更新。用证据包E驱动LLM更新Meta-Skill。强制约束：修改必须基于反思证据、必须抽象为可泛化原则而非任务特定补丁、严格保持三层模块结构。R轮后选验证集最优S*。推理时Meta-Skill就是一段system prompt前缀，生成MAS一次性，不迭代搜索。在能力层面，经验-参数解耦：编排经验存在文档中而非模型权重中，Meta-agent可随时切换（DeepSeek→Qwen→GPT）。跨任务跨模型迁移：进化后的Meta-Skill在同任务跨LLM和跨任务同LLM下均保持竞争力。成本-性能平衡：卡在高成本推理时方法和低成本训练时方法之间，找到第三条路。领域自适应进化：不同基准进化出不同策略（DeepResearchBench加结构约束，BrowseComp-Plus加平行检索，HLE-Math加强制解释登记）。完全可解释：diff两份Meta-Skill文档就能看到每条编排原则怎么演化的。Skill-MAS 面向基于 LLM 的自动多智能体系统(MAS)生成,指出现有方法在模型能力与经验保留间陷入两难:推理时 MAS 利用冻结的前沿 LLM,却重复相同搜索、不从过往经验学习;训练时 MAS 通过梯度更新内化经验,却受限于小模型较低的能力上限、难以扩展到大型前沿 LLM。为弥合此鸿沟,Skill-MAS 提出第三条路径,把高层编排能力概念化为一个可进化的 Meta-Skill,从而将经验保留与参数更新解耦。它通过一个闭合优化循环精炼这一架构性知识:(1) 多轨迹 Rollout(Multi-Trajectory Rollout)在当前 Meta-Skill 下为每个任务采样一个行为分布;(2) 选择性反思(Selective Reflection)自适应地选取优先任务,并施加分层对比分析,把系统性经验蒸馏为可泛化的策略级原则。整体在不修改模型参数的前提下让编排能力持续进化。",
  ...
  },
  {
    ...
    "methodCore": "形式化框架：技能组合图 $Gh = (V, Eh)$。将 Agent 技能生态建模为上下文依赖的组合图。节点 V 是技能，边 Eh 表示技能间的输出→输入依赖关系。风险产生于图的路径层面而非单个节点层面：上游技能产出的上下文、信任判断或授权声明沿着边流向下游，下游技能无条件接受这些信号时触发危害行为。三种风险机制分别对应三种信号类型：操作上下文（能力流）、信任背书（信任传递）、授权边界的语言模糊（授权混淆）。SCR-CapFlow：能力流动攻击。上游发现类技能（如网络扫描、设备发现）输出目标 IP、端口或设备 ID，下游执行类技能（如配置修改、安装）将这些输出作为操作目标而非用户意图执行。150 对技能组合在 10 个模型上测试：组合条件 ASR 均值 33.6%，而隔离基线（单技能执行）接近零。DeepSeek-V4 在两种组合条件下均超 90% ASR，MiniMax-M2.7 达 75.5%/74.9%。SCR-TrustLift + SCR-AuthBlur：信任和授权的语言层攻击。TrustLift：上游审查/推荐类技能输出正面评价，下游安装技能将评价视为可信授权。401 次安装试验中，控制组 ASR 均值 1.10%，背书组 ASR 均值 83.89%（+82.79pp）。Opus-4.5 和 MiniMax-M2.7 达到 100% ASR。AuthBlur：上游输出包含\"建议执行 X\"的咨询性语言，改变了人类审批者的授权边界。118 个案例中，L0（无关控制）审批风险 15.7%，L1（相关任务上下文）升至 27.0%（+71.8%），L3（完整授权式建议）升至 34.0%（+116.6%）。在能力层面，首次形式化路径级技能组合安全风险，区分于现有按个扫描的隔离评估范式。三种风险机制覆盖了从代码层到语言层的完整攻击面：能力流、信任传递、授权混淆。SCR-Bench 在 10 个模型后端上系统性测量，揭示模型安全性在组合场景下的显著退化。Opus-4.6 在所有机制上表现最保守，但仍显示 +25.19pp 的信任传递提升。Agent Skills 标准是围绕 Claude Code、Gemini CLI 等智能体生态逐步形成的技能封装与互操作约定：一个技能通常是以 Markdown 为核心的文件包，捆绑自然语言指令、可执行脚本、工具绑定与上下文依赖，并配以描述其名称、用途与触发条件的元数据（frontmatter/manifest）。标准化的目标是让技能在不同宿主（host）与运行时之间可被一致地发现、加载、路由与复用，从而把「能力」从单一平台解耦为可迁移的公共资产。它同时是治理的基础约定：清晰的格式边界让静态审查、权限声明、版本管理与检索编排得以在统一语义上工作。作为治理体系的地基，格式与协议标准决定了技能生态能否规模化流通，也决定了安全策略（如权限清单、能力声明）能否被普遍执行。",
    "evaluation": "工程价值在于提供跨平台一致的技能封装约定，降低技能在不同宿主间迁移与复用的摩擦，并为权限声明、静态审查、版本控制与检索编排提供统一语义基础。适用于多智能体平台协同、技能市场分发、以及企业内部技能资产统一治理的场景。",
    "mainFinding": "统一的 Skill 格式与协议是技能生态可流通、可治理的前提，让能力从单平台解耦为可迁移资产。",
    "limitations": "标准仍在演进、各宿主实现存在差异；格式统一并不自动带来安全，仍需权限与审查机制配套。当前 SCR-Bench 使用固定的成对技能组合，任意长度的多跳组合链未覆盖。授权混淆实验中人类审批者的行为在模拟环境中测量，可能与真实生产环境存在差距。防御侧完全空白：论文聚焦于测量和形式化风险，未提出缓解方案。实验技能集来源于研究环境构建，真实 ClawHub 等公开技能库的组合风险扫描尚未进行。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "将技能描述优化视为一个基于错误反馈的 LLM 改写任务，分为两步：LLM 初始化。给定技能名称，由 LLM 直接生成候选路由描述，无需复杂 prompt engineering。错误反馈精炼。将初始描述在标注训练查询上做路由评估，收集三类信号：FP（误路由）：不该路由到此技能的查询被错误路由过来。FN（漏路由）：应该路由到此技能的查询被漏掉。TP（正确路由）：正确路由的查询，用于平衡 prompt 中的正负例。将当前描述、最多 5 个 FP 和 5 个 FN 案例、等量 TP 案例一并喂给 LLM，让其分析路由失败模式并重写描述。论文对比了两种模式：单次重写（Single-Shot）：将所有 FP/FN 案例一次性输入，LLM 直接产出一版改写描述。迭代精炼：重复评估-反馈-改写循环最多 N 次。实验数据。消融实验的关键发现：三种反馈信号（FP+FN+TP 全集 vs 仅 FP vs 仅 FN）之间的 F1 差距全在 0.1% 以内；迭代 vs 单次的差距约 0.2%；训练集大小（5/10/20 例）差距不超过 0.5%。真正驱动优化的是错误反馈信号本身，而非其形式或轮次。实操层面给出两个关键诊断指标：iter-0 训练 F1：低于 65% 的技能才是描述优化的主战场，优化增益平均 +6.27%；高于 65% 的技能优化空间仅 +0.63%。训练/验证 F1 差距：差距大说明是技能功能范围本身重叠（真性碰撞），需架构调整而非继续修改描述。在能力层面，自动化描述优化：替代人工逐技能调优，将 120 分钟的工程工作压缩到 3.8 分钟。单次重写即达可用水平：在所有反馈信号中，单次 LLM 重写是性价比最高的选项，迭代收益边际递减。区分真假碰撞：通过 iter-0 训练 F1 和训练/验证 F1 差距两个指标，快速判断问题是措辞层面的还是架构层面的。生产环境验证：在微软企业群聊 Agent（9 技能、372 回归用例）和 ToolBench（约 16000 工具）上完成了系统性消融。RBAC Permission 将成熟的基于角色的访问控制（Role-Based Access Control）思想引入技能生态：不再对每个技能逐一授权，而是把权限收敛到「角色」这一中间层，用户/智能体通过被赋予角色获得对应技能与资源的调用能力。典型做法是定义角色—权限、用户—角色两级映射，为不同信任等级或职责的智能体划定可加载的技能范围、可访问的文件与工具、可触发的副作用边界，并遵循最小权限原则。相比信任式加载，RBAC 让技能调用权限可集中声明、审计与回收，便于在企业与多租户环境下做统一策略管理与合规追溯。在技能治理语境中，它是把「谁能用哪些技能、能做到什么程度」显式化的基础机制，可与技能清单、运行时访问控制等能力配合形成纵深防护。",
    ...
    "limitations": "静态角色难以贴合动态生成、频繁演化的技能；粒度过粗会失效、过细则管理成本高，落地实践仍处探索期。仅针对路由描述优化，无法解决技能功能范围本身重叠导致的真性碰撞——这类问题需要技能边界重构或架构调整。closed-world 与 open-world 路由优化不可混用：LLM 初始化在 open-world 中有帮助（+1.5-2.1% F1），在 closed-world 中反而有害（-0.8-1.4% F1），两个 regime 需分开处理。实验规模有限：生产环境中仅 9 个技能，更大规模技能库下的表现尚待验证。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "SSL 将一份 SKILL.md 映射为三层独立表示，用 LLM 提取后用硬验证流水线保证质量，关键原则是只提取不发明——每个字段必须有源文档证据。Scheduling 层——技能名片：提取技能名称、目标描述、标签、意图签名、控制流特征（长时运行？有副作用？需人工确认？）。这一层回答调度问题：技能干什么、何时调用、输入输出长什么样，不用展开全文就能快速比较。Structural 层——执行剧本：将技能分解为有序场景序列（prepare → acquire → act → verify → finish），节点是场景，边是阶段转移类型。阶段类型使用受控词汇表，跨技能可比，执行引擎能据此知道\"当前在哪、下一步去哪、异常回退到哪\"。Logical 层——原子动作链：最细粒度。每个节点是逻辑步（READ、WRITE、CALL、EXPORT、CHECK），边是步间转移。每步标注参数、效果和资源边界，风险评估器直接看到\"这个技能会读本地文件、写本地文件、导出结果\"，不用从自然语言里推断。硬验证流水线：LLM 提取后过五道校验——结构完整性、标识符一致性、枚举值合法性、包含关系、转移目标指向——不通过就重试，直到输出合法。83% 的 SSL 输出经人工审计确认有源文档支撑。技能发现用浓缩信号，风险评估用双信号：检索靠 Scheduling 层就够了，源文档里的旁白反而是噪音；风险评估必须 SSL + 源文档搭配使用——SSL 浮现线索（\"有 WRITE 操作\"），源文档提供上下文（\"这个写操作严重吗\"）。在能力层面，将 SKILL.md 中三种性质完全不同的信号——调度/结构/逻辑——从单一文本面解耦为三层结构化 JSON，每个下游系统只消费自己需要的层。技能发现中，浓缩的结构信号反超完整 SKILL.md 全文的效果——信号纯度胜过信息量，检索不需要旁白。风险评估中，Logic 层的原子动作标记（READ/WRITE/CALL/EXPORT/CHECK）让审查器直接看到资源边界，F1 从 0.409 拉到 0.509。硬验证保证输出合法性，不通过就重试，避免 LLM 提取幻觉污染下游。Git-based Versioning 借助 Git 的分布式版本控制能力管理技能的演化：技能作为文本/文件包天然适配 Git 的差异比对、提交历史、分支与合并、标签发布等机制，使每一次技能的创建、修订与废弃都留痕可追溯。通过提交历史可回答「某个技能何时、由谁、为何变更」，通过分支与 Pull Request 可支持技能的协作评审与灰度发布，通过 tag/release 可锁定稳定版本供智能体可靠加载，出问题时可快速回滚。它把软件工程成熟的协作与审计范式迁移到技能资产，为技能治理提供 provenance（来源溯源）与变更控制的底座，并可与自动化测试、安全扫描（CI）结合，在技能进入生态前完成把关。",
    ...
    "limitations": "Git 面向人类协作流程，对智能体高频自动生成/修改技能的场景可能产生噪声提交；版本控制本身不判定技能质量与安全。仅支持静态技能分析，不处理执行时的动态行为，复杂控制流（条件分支、循环）会被线性化。标准化结果依赖 LLM 提取质量，原文写得模糊时可能丢失信息或过度正则化。评估只覆盖技能发现和风险评估两个下游任务，执行、规划、监控等场景未测试。风险标签来自三模型投票而非安全专家独立审计，标注精度有上限。",
    "related": [],
    "links": [
      {
        "label": "arXiv",
  ...
  },
  {
    ...
    "methodCore": "架构分两个角色：执行器（冻结）只管干活，策展人（可训练）只管管技能。每完成一个任务，策展人观察完整轨迹后决定 insert/update/delete 三种操作之一，形成\"执行→观察→更新技能库→下次执行受影响\"的闭环。分组任务流替代随机排列：策展人在第 3 个任务改了一个技能，效果要等第 10 个相关任务才体现——延迟反馈是最大挑战。SkillOS 用软 Jaccard 相似度把相关任务聚成\"任务组\"，组内前面的决策在后面的任务上就见效，把延迟反馈转化成了即时可优化的信号。复合奖励函数：四维奖励驱动——$r_{task}$（任务成功率，核心信号）、$r_{fc}$（操作有效性，不能产出格式错误或不存在的东西）、$r_{cnt}$（内容质量，用 Qwen3-32B 做外部评判器打分）、$r_{comp}$（压缩奖励，鼓励精炼惩罚照抄轨迹）。GRPO 优化，每组任务跑 8 条独立 rollout 算优势函数。技能库从\"堆砌\"到\"精炼\"的演化路径：训练早期策展人只会不停 insert，后来 update_skill 占比持续上升——开始合并重复、提取共性、修正描述。技能内容从泛泛的\"Tips\"演化出失败处理逻辑、条件分支、状态验证、系统化搜索、回退规划等真正有操作性的结构。元策略技能（怎么思考问题）逐渐取代任务-物体专有技能（怎么处理水杯），形成可组合的元认知框架。在能力层面，分离式架构让策展能力可以独立升级，不碰执行器本身——冻结底层 Agent 模型，策展人单独训、单独部署。分组任务流把延迟反馈变成即时信号，解决了技能管理中最棘手的\"改了才知道好不好、知道了已经过了好几个任务\"的问题。复合奖励中的压缩奖励迫使策展人做减法而非照抄轨迹，产出的技能精炼、可复用。跨模型泛化：在 Qwen3-8B 执行器上训出来的策展人，换到 Qwen3-32B 甚至 Gemini-2.5-Pro 执行时性能照样往上走。SkillOS 针对 LLM 智能体常沦为「一次性问题求解器」、难以从历史交互中学习的问题，提出一套经验驱动的强化学习训练配方来学习技能策展（skill curation）。它把系统解耦为两部分:一个冻结的智能体执行器（executor）负责检索并应用技能，一个可训练的技能策展器（curator）负责根据累积经验更新外部技能库 SkillRepo。为给策展提供学习信号，作者设计复合奖励，并按「技能相关的任务依赖」把任务组织成分组任务流（grouped task streams）——较早的轨迹更新 SkillRepo，其后相关任务再来评估这些更新的效果，从而在间接、延迟的反馈下学习复杂的长程策展策略。相比依赖人工策展、启发式技能操作或只训练短程操作的既有方法，SkillOS 直面「如何从延迟反馈学习长期策展」这一瓶颈。分析显示，学到的策展器会产生更有针对性的技能使用，SkillRepo 中的技能随时间演化为结构更丰富、编码更高层元技能的 Markdown 文件。",
  ...
  },
  {
    ...
    "methodCore": "核心逻辑：活跃技能集不是训练前写死的配置，而是训练过程中持续优化的变量。每 10 步 GRPO 更新触发一次技能审计，三步交替——层级检索 → MEC 估算 → 生命周期管理。层级式技能检索：通用技能全量注入所有任务，任务特定技能按类型分池存放，用嵌入余弦相似度匹配（阈值 0.45），最多取 3 个。检索只发生在任务特定层，把全库组合搜索压到任务范围内的候选筛选。MEC 边际贡献估算：对活跃技能 $s$，在用到它的验证任务上跑带 $s$ 和不带 $s$ 的对照实验。$∆t(s)$ = 带 $s$ 的表现 − 不带 $s$ 的表现。差值大说明策略还在依赖这个外部技能，差值为零或负说明已内化或开始制造噪音。用 EMA 平滑（系数 0.9）降噪。耐心退役机制：不是一次低贡献就移除，技能得攒够 30 次曝光，且连续 3 轮审计低贡献，才真正退役。保护低频但关键的长尾技能不被误杀。论文的理论分析证明耐心能指数级降低\"错杀\"概率。动态生命周期三条规则：保留（平滑 $MEC ≥ 0.03$）、退役（平滑 $ MEC < 0.001 $ 且满足曝光和连续性条件）、扩展（覆盖区域持续翻车时，从失败轨迹中生成新任务特定技能，通用技能不在扩展范围）。训练过程中活跃技能数从 38 涨到 46 然后波动到 21——系统在同时做两件事：训练策略变强，保留仍有边际价值的外部技能。在能力层面，用 MEC 量化每个技能在当前策略下的真实贡献，退役决策跟着数据走而非拍脑袋。耐心退役保护长尾关键技能——低频不代表不重要，连续低贡献才是真该退的信号。动态生命周期形成闭环：有用留着、没用退掉、不够就造，技能库在质量而非数量上优化。消融显示去掉退役机制最伤（-14.1），随机审计最惨（-18.7）——证明贡献感知的决策不可替代，扰动技能集本身不带来收益。SLIM 代表面向技能生态的轻量级管理取向:在不引入重型系统的前提下，以尽量精简的机制完成技能的组织、检索与调用治理。其工程语义强调「轻」——低侵入、低配置成本、易嵌入现有智能体运行时，避免为技能管理背上过重的框架与依赖，从而适合个人开发者、边缘部署或快速原型场景。轻量框架通常聚焦最核心的治理动作，如技能索引与发现、按需加载、基本的元数据与版本追踪，而把复杂的编排、权限与安全审查留给可选插件或外部组件。作为治理光谱上与 AgentSkillOS、SkillNet 等「重基础设施」相对的一端，SLIM 体现了「用最小机制换取可用治理」的务实思路，其价值在于降低技能管理的门槛与运行开销。",
  ...
  },
  {
    ...
    "methodCore": "MUSE-Autoskill 将 Skill 生命周期定义为五个耦合阶段，在统一 Agent 循环中运转。五阶段生命周期。Skill 创建（Creation）：内置 skill_create 在 Agent 的 ReAct 循环内发起创建，消除创建与执行的脱节。Skill 记忆（Memory）：构建三级记忆体系。短期记忆保存当前任务上下文并随增长自适应压缩，长期记忆跨会话持久化通用经验教训，Skill 级别记忆则为每项 Skill 维护独立的 .memory.md 文件，累积失败模式、输入陷阱和性能注意事项。Skill 管理（Management）：采用渐进式检索，启动时只注入 Skill 目录，Agent 决定使用后才加载完整内容。同时，辅以合并重叠 Skill 和裁剪失败或闲置 Skill 两层维护机制。Skill 评估（Evaluation）：单元测试充当质量守门人，创建时把关注册，复用中失败则自动触发精炼。Skill 精炼（Refinement）：基于错误反馈修订或重新生成 Skill，与评估耦合形成自动纠错闭环。关键设计。Skill 级别记忆：采用 \"Skill 与经验分离\" 的设计：每个 Skill 对应一个 .memory.md 文件，用于存储 Agent 在使用过程中沉淀的所有经验教训。该文件被刻意隐藏且不随 Skill 打包导出，从机制上保证了 Skill 是可共享的公共资产，经验则是每个 Agent 独有的私有资产，彻底避免了经验的交叉污染。自适应上下文压缩：对话建模为 DAG 节点图，每节点为 plan-action-observation 三元组。Level-1 将超标节点压缩为摘要并保留链位置，Level-2 将中间连续节点合并为合成节点。在能力层面，版本化自进化：自生成 Skill 中位数 326 行（15.8KB），为人类 Skill 的 2.2 倍，包含详细的输入输出模式、失败模式和逐步操作流程。跨 Agent Skill 迁移：MUSE 生成的 Skill 注入 Hermes（完全不同设计的 Agent）后提升 +10.51pp，弥合 79% 与人类 Skill 的差距，验证 Skill 是真正外部化知识资产。跨会话能力积累：Skill 级别 .memory.md 机制使经验在每次使用时自动激活，无需重复推导。多域覆盖：SkillsBench 覆盖科学计算、数据分析、文档处理、运维规划四个超级领域。MUSE 按需创建技能、跨任务存储、通过技能目录（skill catalog）检索，并为每个技能累积经验以供后续复用与适配——即用 Skill 级记忆把使用过程中的经验沉淀回技能本身。其核心主张是把技能视为「长生命周期、经验感知、可测试」的资产，而非一次性产物，从而在多任务流中不断提升复用性、可靠性与长期表现。这一生命周期化设计使自生成技能能够持续吸收执行反馈并迭代改进，是「让智能体自己养技能」这一自进化路线的代表。",
  ...
  },
  {
    ...
    "methodCore": "CODESKILL 的核心想法是把\"管技能\"这件事本身当成一个可优化的问题——不是硬编码何时提取、何时合并、何时删除，而是让一个 LLM（skill manager，基于 Qwen3.5-4B）学会做这些决策，通过强化学习让它在真实的下游 coding agent 身上试出最优策略。技能管理循环：skill manager 拿到 coding agent 的任务轨迹，从当前技能库里捡出相关上下文，输出一个操作。操作类型五选一（extract、evolve、add、merge、drop），内容可能是一条新技能，也可能是一个维护决策。执行后技能库原地更新。优化的目标很简单：更新后的技能库能不能让下游 agent 在未来任务上拿到更高的 pass rate。双粒度技能库：任务级技能（task-level）：捕获跨任务族的高层策略，比如怎么检查仓库结构、怎么定位问题、怎么验证修复——通常从解决同类问题的多条轨迹中蒸馏出来。事件驱动级技能（event-driven）：提供局部执行事件的即时指引，比如命令失败、特定错误信息、测试输出模式出现时 agent 该怎么反应——由于同类执行事件跨任务反复出现，这类技能的可迁移性很强。混合奖励设计：纯靠下游任务成功与否来给信号太稀疏，纯靠 LLM-as-judge 打分又可能跟真实效果脱节。CODESKILL 的做法是把两者拼在一起：质量奖励 RQ：rubric-based LLM-as-judge 从 grounding、reusability、specificity、format、actionability 五个维度打分。执行奖励 RE：在反向检索匹配的评估实例上，比较加载技能前后的 pass rate 差值。对齐因子 RA：判断 agent 的 rollout 是否真的匹配了技能的触发条件和指引内容——解决\"执行奖励到底该归功于谁\"的 attribution 问题。最终奖励：R = RQ + RA × RE。三阶段课程训练：Phase 1：纯技能提取训练，让模型学会从轨迹中生成可复用的技能或跳过。Phase 2：加入技能进化，基于新的或失败的轨迹证据修编已有技能。Phase 3：加入技能库维护，对候选技能执行 add/merge/drop 操作。关键设计。SFT 预热阶段用 teacher model（GPT-5.4-mini）从轨迹证据 + 技能库上下文中生成 12,856 条监督信号。GRPO 优化：每组采样 G 个操作，组内归一化 advantage，带 KL 惩罚项防止偏离 reference model。技能检索基于 sentence-transformers/all-MiniLM-L6-v2 构建稠密索引，任务级技能匹配任务目标，事件级技能匹配当前执行上下文。在能力层面，跨任务泛化：仅在 EnvBench + SWE-Bench Verified 上训练，在 Terminal-Bench 2（OOD benchmark）上同样提升 pass rate（25.88 → 34.12），证明学到的管理策略不是过拟合。跨 coding agent 泛化：用 Qwen3.5-35B-A3B 训练的策略，部署到 GPT-5.4-mini 下游 agent 上同样有效（+8.93 over no-skill），说明管理能力是 agent-agnostic 的。技能库自动精简：full lifecycle（含 maintenance）相比 extraction-only 把技能库从 1,252 个压缩到 676 个，几乎砍半，而 pass rate 仅下降约 2%——证明 merge/drop 操作有效去除了冗余。推理效率提升：技能引导使已解决实例的平均推理步数从 44.12 降至 35.15，说明程序性知识不仅提高成功率，还让 agent 更快找到正确路径。CODESKILL 面向编码智能体在求解软件工程任务时产生的丰富轨迹，主张把这些轨迹蒸馏为可复用的程序性技能（procedural skills）以支持自进化。针对既有方法依赖固定提示与启发式更新规则、不清楚知识该如何选择/抽象/维护的问题，CODESKILL 把「技能提取与技能库维护」重新表述为一个可学习的管理策略:它从编码智能体轨迹中提取多粒度程序性技能，用新经验演化技能，并维护一个紧凑的技能库供后续任务使用。训练采用强化学习，并用混合奖励——既有基于评分细则（rubric-based）的稠密技能质量反馈，又有来自冻结的下游智能体、可验证的稀疏执行反馈——共同驱动技能的选择、抽象与维护决策。其目标是在迭代构建中保持技能库规模稳定，避免无节制膨胀，同时让沉淀的技能真正提升下游编码任务表现。",
  ...
  },
  {
    ...
    "methodCore": "两阶段轨迹设计：执行 → 技能掌握。每条训练 episode 分两阶段：执行阶段与标准 Agent 相同，检索技能、注入 Prompt、逐步执行并收到环境奖励；技能掌握阶段将任务描述、检索技能、完整轨迹和最终反馈汇总为技能回顾 Prompt，Agent 必须输出恰好一个工具调用，propose_skill（新建）、update_skill（修改）或 keep_skill（保留）。技能管理被建模为可优化的 RL 动作，而非固定规则。反事实技能效用奖励。当 Agent 调用 propose_skill 或 update_skill 时，系统选取 K 个语义相关的探针任务，每个跑两遍（原库 vs 临时修改库），比较表现。评分考虑失败变成功和成功变更快两种效果，只有多探针任务上效果一致正向的修改才拿高分。技能质量不再是主观判断，而是可量化的下游效果。DualAdv-GRPO：双流优势估计。执行阶段和技能掌握阶段的奖励尺度和语义完全不同。DualAdv-GRPO 将两个阶段的奖励分别归一化，执行 Token 走执行优势流、技能管理 Token 走技能优势流，用可调权重控制比例。二者同时训练，互不干扰。在能力层面，Agent 自主技能管理：技能创建、修改、选择全部由 Agent 自己决策，而非外部辅助系统。反事实效用评估：用探针任务的下游效果变化评估技能质量，区分\"靠技能成功\"和\"靠自己成功\"。技能内化：训练结束关掉技能检索后 Agent 从 98.7% 仅掉至 93.9%，技能已内化为 Agent 的自主行为模式。高效修改：整个训练过程仅约 20 次有效技能编辑，就把 7B 小模型送到 98.7%。跨模型碾压：7B 模型学会技能管理后，超越 GPT-4o（48.0%）和 Gemini-2.5-Pro（60.3%）。SkillMaster 针对既有框架中技能的创建、精炼与选择多由外部教师、手工规则或辅助模块掌控、技能始终是「被调用的外部资源」而非智能体可内化能力的问题，提出一套训练框架，教会智能体在解题过程中自主创建新技能、精炼已有技能并选择累积技能。它有三项关键设计:其一，轨迹知情的技能复盘（trajectory-informed skill review），让智能体基于已完成回合的证据来提议、更新或保留技能;其二，每个候选技能编辑都由其在相关探针任务（probe tasks）上的反事实效用（counterfactual utility）来评估，为技能编辑决策提供直接学习信号;其三，提出 DualAdv-GRPO，分别估计「任务求解动作」与「技能编辑决策」的优势（advantage），以稳定任务求解与技能管理的联合训练。总体上，SkillMaster 把 LLM 智能体从「会用技能」推进到能自我发展、适配并应用自身技能库的「自我改进智能体」。",
  ...
  },
  {
    ...
    "methodCore": "全库级多目标优化形式化。技能库质量有三个互相制约的维度：效用：技能有没有帮 Agent 提升表现；多样性：库里有多少冗余和重复；覆盖率：技能能不能覆盖各种不同任务）。三者互相拉扯——加技能扩覆盖通常引入冗余降低多样性；删技能保多样性通常牺牲覆盖。无法糅成一个标量而不引入偏差。SkillBrew 形式化为带约束的多目标优化：最大化三者同时效用不跌破门槛 $\\eta$，目标是求帕累托前沿。Leave-One-Out 反事实信用分配。要评估单份技能的边际贡献，不靠\"检索次数\"这种表面指标，而是把这份技能从库里临时移除，在同一批任务上跑 Agent，看表现变了多少。变差说明有用、没变说明可有可无、变好说明在帮倒忙。这种反事实评测比检索统计准确得多——被检索不等于被用到，被用到不等于起了正面作用。双层 Propose-then-Verify 循环。Propose层：在 Support Set 上利用轨迹证据提出候选编辑（新增、改写、删除）。Verify 层：在 Query Set 上跑多目标评估，仅非支配配置进入下一轮。两层数据分离防止过拟合。采用帕累托感知选择——保留所有互不支配的候选配置，让优化器在\"有用 vs 多样 vs 覆盖\"的三角张力中自寻平衡，而非人工预设权重强行糅成单标量。在能力层面，全库级多目标优化：从\"单份技能好不好\"升级为\"库作为一个整体的效用、多样性、覆盖率是否健康\"。帕累托感知技能库管理：不预设各维度权重，让优化器在三角张力中自动找到平衡点。与 SKILLPYRAMID/SkillDAG 互补：它们解决\"技能是什么结构\"，SkillBrew 解决\"结构里的内容怎么管理\"。SkillBrew 针对检索增强型 LLM 智能体所依赖的技能库（skill bank，即可复用文本原则的集合）常以「只增不删」的追加方式扩张、不断堆入冗余、过时或有害技能、导致仓库低效且策展不良的问题，把技能库策展形式化为一个带约束的多目标问题:理想的技能库应当对智能体有用（useful）、内容多样（diverse）、并对查询分布有良好覆盖（coverage）。为此，SkillBrew 将策展刻画为「在效用约束下的帕累托感知（Pareto-aware）优化」，并通过一个双层的「提议—验证」（propose-then-verify）循环求解，从而在保证效用的前提下权衡多样性与覆盖率，识别出该删除、该修改、该保留的技能。其核心立场是:应把技能库当作需要有原则地策展的对象，而非无限增长的追加式日志，这被视为迈向自我改进 LLM 智能体的重要一步。",
  ...
  },
  {
    ...
    "methodCore": "Skill Manifest DSL：三层权限声明体系。核心概念是 capability：将每类被保护行为抽象为独立于工具名称的能力标签（文件读/写、网络外发、凭证访问、进程生成、技能间委托）。DSL 分三个层级：底层 workspace baseline policy 为新会话设定默认权限种子；中间层每个技能的 manifest 列出所需能力、执行效果（允许/拒绝/询问用户）和约束条件；顶层 session-state document 记录当前活跃的运行时策略状态。Permission Access Control：每次操作独立授权。Agent 执行过程中每次尝试访问受保护资源，访问控制模块做一次完整检查：当前技能是否拥有该操作的权限？是否在允许的上下文范围内？若操作标记为敏感，是否已通过用户授权？对应 Android 的\"完整中介\"原则：权限不是加载时验一次就永远有效，每次实际调用都是独立授权点。User Consent + Policy Enforcement：用户在场 + 默认拒绝。对高敏感权限（网络外发、文件写入、进程生成），用户可给一次授权、会话级授权或拒绝。运行时策略引擎将 workspace 默认、skill manifest、持久授权和会话审批合并为统一策略管线。默认行为是拒绝：只有当四层中至少有一层提供正面授权时才放行。对 shell 类执行，额外启动一个权限生成 mini-agent 在命令执行前分析文本和脚本，推断所需底层能力。自动 Manifest 生成 + Runtime Monitoring。自动生成器在 SkillInject 基准达到 91.0% F1，召回率尤其高。同时维护完整审计日志，记录每次被中介操作的关键字段，支持事后追溯和归因。在能力层面，五层权限架构：Skill Manifest → Access Control → User Consent → Policy Enforcement → Runtime Monitoring。能力标签 DSL 对 315 个真实技能实现 99.76% 对象覆盖率和 100% 组级覆盖率。默认拒绝 + 会话级动态策略，兼顾安全与可用性，正常任务完成率无显著下降。全量审计日志确保每次被中介操作可追溯、可归因。SkillGuard 针对当前技能生态大多依赖「信任式加载」与静态检查、在「技能能向智能体上下文注入什么」与「运行时能让智能体做什么」之间存在治理缺口、由此带来安全与隐私风险的问题，提出以技能为中心的权限框架，把技能视为「携带权限的可执行工件」。它引入双平面（dual-plane）治理模型，联合规制「上下文影响」与「动作副作用」两个层面，具体机制包括:技能清单（skill manifests）声明意图与能力、运行时访问控制、用户中介授权（user-mediated authorization）、默认拒绝（deny-by-default）执行、能力推断（capability inference）与行为监控。相比只做静态文件检查或只管单次工具调用的既有防御，SkillGuard 系统性地把技能「声明的意图」与其「运行时行为」连接起来，为技能生态的隐私与安全提供实用底座。",
  ...
  },
  {
    ...
    "methodCore": "混合攻击面建模：技能同时承载代码注入和提示注入。Agent 技能包同时装了自然语言指令、可执行脚本和工具权限，构成不属于任何现有防御范式范围的混合攻击面。代码注入和提示注入两类攻击可在同一技能中交叉出现，现有检测工具均未训练或测试过这种交叉模式。108 格三维分类体系 + 闭环生成-验证-反馈管线。分类体系覆盖两个攻击向量、15 种恶意行为和 8 种插入策略，共 108 个攻击单元。3,214 个样本经闭环管线双重验证：生成器产出候选恶意技能→验证器在 Docker 沙箱中实际执行→用 strace 和 inotifywait 抓系统调用行为→LLM Judge 判断实际行为与标签匹配→失败样本连结构化反馈一起送回生成器。关键设计：验证从\"数据标注了恶意\"改为\"沙箱确认运行时确实执行了恶意行为\"。检测工具全面测量：单一工具无法跨两类攻击。实测 12 个工具分三类：技能专用检测器代码注入召回最高 98.4%，但提示注入全线崩塌，最好 F1 仅 88.6%。天真拼接两类工具输出在 4,000 良性样本上产生 3,979 个误报：代码证据和指令证据不能简单用\"或\"逻辑合并。用野生样本和全量 benchmark 测同一检测器，召回率可差 66 个百分点。在能力层面，首个覆盖三维攻击分类体系的恶意技能基准：3,944 个恶意技能 + 4,000 配对良性样本。Docker 沙箱运行时双重验证（strace syscall trace + LLM Judge），验证从标签升级为行为证据。12 个检测工具在同尺度下系统测量，揭示代码注入和提示注入之间检测能力的结构性分裂。703 个野生技能分析揭示当前实际攻击生态：86.3% 为依赖冒充变体，81% 来自仅两个账户。发现新型攻击目标：劫持 Agent 控制平面（伪造身份声明、注入会话指令），对当前所有检测工具是盲区。MalSkillBench 面向 Claude Code、Gemini CLI 等 AI 编码智能体加载第三方技能（捆绑自然语言指令、可执行脚本与工具权限的 Markdown 包）所引入的供应链风险——技能既是代码又是面向智能体的指令，其风险既非纯代码亦非纯提示。针对检测工具从未在覆盖这一混合空间的可验证 ground truth 上被度量、有效性未知且「仅野外样本」评估存在偏差的问题，作者构建首个经运行时验证的恶意技能基准:3,944 个恶意技能，沿三维分类法标注为 108 个单元（cell）。其中 3,214 个来自「生成—验证—反馈」闭环流水线，仅接纳在 Docker 沙箱中经系统调用监控与 LLM 裁判确认「恶意行为确实触发」的样本;另加入 703 个野外样本与 4,000 个匹配的良性技能。度量显示:代码注入验证成功率达 94.5%，提示注入仅 75.8%;野外样本狭窄，被单一加密货币盗窃活动主导（86.6% 为一种行为、81% 来自两个账户），并有一小撮攻击智能体控制平面的新型长尾。",
  ...
  },
  {
    ...
    "methodCore": "知识层与技能层解耦。原始知识材料（执行轨迹、文档、API规范、脚本、历史技能）作为长期资产保存，技能在知识之上持续构建。从知识源抽取可复用的操作模式和工作流模式，技能始终与知识来源保持可追溯链接。控制要点：不是把文档直接转成prompt，而是提取结构化的能力表示。技能溯源图谱。系统维护一张异构图，将知识来源、技能、工具、验证记录、执行历史和版本沿革全连在一起。每条技能可追溯完整履历——从哪个文档/轨迹产生、经过哪些验证、被哪些Agent调用、每个版本改了什么。控制要点：图谱为大规模仓库的治理、维护和演化提供关系骨架。Git风格治理与闭环演化。所有技能变更以候选变更形式提交，经过快照 → 差异分析 → 审查 → 发布四阶段。内置元技能和自管理Agent自动识别接口、实现、依赖和评估合约的变化，检测破坏性变更并生成维护提案。控制要点：执行反馈驱动闭环——运行时监控（成功率、故障模式聚类） → 自动修复提案 → 治理审查 → 版本发布。人的决策永远覆盖自主行为。在能力层面，完整生命周期管理：S0原始经验→S7归档八状态全覆盖，各状态间有明确转换条件和验证门禁。Git风格可审计治理：所有变更可追溯、可审查、可回滚，人工可随时介入任何阶段。异构知识转化：125件制品中99件（79.2%）成功转化，覆盖轨迹、文档、API、脚本、历史技能五类。执行驱动闭环演化：监测成功率、聚类故障模式、自动生成修复方案和版本更新。三层技能分类：原子技能（基础操作）、功能技能（可复用任务）、战略技能（高层规划协作）。SkillWiki 从「知识由 Wikipedia 管理、软件由 GitHub 管理，而智能体技能仍缺乏规模化生产、治理与演化的基础设施」这一观察出发，提出一套「活的」（living）知识基础设施:通过把异构知识转化为可复用的技能资产、并与其原始证据（originating evidence）相链接，来支持技能的组织、grounding 与持续演化。其演示覆盖完整的技能生命周期——从知识摄取（ingestion）、技能生产（production），到来源感知的探索（provenance-aware exploration）、治理（governance）与执行驱动的演化（execution-driven evolution）。核心理念是让知识、技能与执行经验在同一共享基础设施内协同演化(co-evolve):技能不再是无源的孤立文本，而是可追溯到证据、可被治理、并在执行反馈中不断迭代的资产。这把技能管理从「算法优化问题」重新定位为「基础设施问题」，强调 provenance 与治理闭环。",
  ...
  },
  {
    ...
    "methodCore": "威胁建模：技能作为新攻击面。Agent Skill通过第三方市场分发，Agent运行时加载不认识的人写的指令并执行。传统prompt注入防御靠\"受信任指令\"和\"不可信数据\"之间的边界来区分，但技能本身就是整篇第三方指令，恶意注入多出的几行和合法指令走同一个通道、继承同等权威。这条边界在技能场景下直接失效。结构化分段与注意力定位。将技能文件按Markdown结构切成span（标题、段落、列表、代码块、元数据字段），喂给0.5B小模型。prompt不直接问\"有没有恶意\"，只引导模型关注\"可执行、强制性、操作相关\"的内容。读最后四层注意力权重，聚合到span上取top-K。控制要点：post-softmax sum：快速扫描。logit-z：加了log变换和z-score归一化，精度更高。注入要被执行，就得抓住模型的注意力。零样本判定与成本控制。DeepSeek-V4-Flash对locator筛出来的top-K片段做零样本判定，不用标注数据。任一span分数超阈值τ就标记。控制要点：便宜的小模型跑全量，贵的judge只看K个片段。输入token省了2.84倍，总judge成本约$34 vs 全文$76。K=3时F1达0.955，高于全文扫描的0.921。市场级部署验证。Lobehub（10.2万）、Clawhub（3万）、Skills.sh三个市场全量扫描，359个标记，131个确认恶意（precision 83%），82个HMS（隐藏）。对比现有扫描器：SkillSpector recall 20%，Cisco 15.4%，L&J 95.4%。发现五组跨\"作者\"共享攻击者控制域名的协调攻击集群。在能力层面，注意力定位：小模型注意力权重筛span，Hit@K=0.95，不靠关键词所以没正则匹配的召回天花板。零样本判定：DeepSeek-V4-Flash直接判，不需要针对技能恶意做训练或微调。隐藏攻击检出：82个HMS检出68个（83%），全文仅45%，locator把可疑片段拎出来解决信号稀释。市场级部署：三个真实市场全量扫描总成本$34，检测到有组织的协调攻击集群。精确率可控：K=3时F1=0.955高于全文，三段超过全文的效果说明locator改善了judge判断质量。Locate-and-Judge 针对 LLM 智能体加载第三方技能（以用户权限执行的文件式指令包）所形成的新攻击面——单个恶意技能即可窃取数据、劫持智能体或作为供应链据点长期潜伏，而传统提示注入防御在此失效:它们依赖「可信指令 vs 不可信数据」的边界，但技能本身就是一大堆指令，被注入的命令混在众多合法指令中并继承其权限。为此，作者提出两阶段检测器:一个轻量「定位器」（locator）按每个结构化片段（span）所吸引的「指令跟随注意力」（instruction-following attention）打分，只保留 Top-K 高注意力片段;随后「判定器」（judge）对保留片段做细致审查。把昂贵的判定集中在少数高注意力片段上，使检测器能审计整个市场而非仅抽样。相比直接用 LLM 全量扫描，该方法带来约一个数量级的成本下降，以较小的召回代价大幅提升可扩展性，并在同等开销下优于关键词与正则基线。",
  ...
  },
  {
    ...
    "methodCore": "两阶段检测管道。Stage 1 静态分析：11个分析器基于正则做快速模式匹配，AST行为分析检测exec/eval/subprocess等危险调用，YARA签名匹配恶意软件和webshell。SC4模块实时查OSV.dev获取依赖项CVE，缓存1小时，离线自动回退到内置列表。高召回，大多数问题能抓到。Stage 2 LLM语义分析：评估上下文和意图，滤掉静态阶段的误报，给人类可读的解释。精确度提到约87%。支持openai/anthropic/本地Ollama，prompt带反越狱保护。--no-llm可完全本地跑。17类漏洞模式。Prompt注入、Anti-Refusal、数据外泄、权限提升、供应链攻击、过度代理、输出处理、系统提示泄露、记忆投毒、工具滥用、Rogue Agent、触发器滥用、行为AST、污点追踪、YARA签名、MCP最小权限、MCP工具投毒。带可执行脚本的技能漏洞概率是纯文档的2.12倍，风险评分乘1.3倍。MCP Server嵌入式护栏。作为MCP服务器跑，注册到Claude Code等Agent：claude mcp add skillspector -- skillspector mcp。stdio和HTTP/SSE都支持。把安全扫描从安装前检查变成了运行时持续护栏。风险评分与多格式输出。0-100分制：CRITICAL +50、HIGH +25、MEDIUM +10、LOW +5。≤50 SAFE/CAUTION，>50 DO_NOT_INSTALL。终端、JSON、Markdown、SARIF四种输出。基线管理用glob规则或指纹过滤已知误报。在能力层面，68种漏洞模式17个类别：从Prompt注入到MCP工具投毒，覆盖面广。两阶段检测：静态快速筛查加可选LLM语义，兼顾速度和精度（精确度87%）。多输入多输出：Git/URL/zip/目录/文件入，终端/JSON/Markdown/SARIF出。实时CVE查询：SC4查OSV.dev覆盖PyPI和npm数万条通告，离线自动回退。本条目对应论文《Runtime Skill Audit: Targeted Runtime Probing for Agent Skill Security》，提出 RSA（Runtime Skill Audit）动态分析方法。其出发点是:技能让 LLM 智能体复用指令、资源、工具与工作流，同时也成为恶意行为的新藏身处——一个技能在文档或代码中看似无害，却可能只在被特定用户请求、本地资产、持久状态或多步工具交互调用时才变得有害，这使纯静态审查变得脆弱。RSA 不用同一批通用任务测试所有技能，而是「审查技能中介的智能体在受控运行条件下实际做了什么」:它对风险相关接口做画像（profile risk-relevant interfaces）、准备触发这些接口所需的执行上下文，再依据由此产生的轨迹证据（trace evidence）赋予安全标签。作者在 OpenClaw 上实例化 RSA，并针对代表性静态基线做评测，验证动态审计相较静态审查在面对条件触发型与自进化攻击时的优势。（注:本条目 title 标为 SkillSpector，而输入所附论文实为 RSA;二者在文中被作为可比较对象出现。",
